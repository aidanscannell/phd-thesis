...
\renewcommand{\expertKernelNN}{\ensuremath{\mathbf{K}_{\modeInd \numData \numData}}}
\renewcommand{\expertKernelnM}{\ensuremath{\mathbf{k}_{\modeInd \numData \numInducing}}}}
\renewcommand{\expertKernelNM}{\ensuremath{\mathbf{K}_{\modeInd \numData \numInducing}}}}
\renewcommand{\expertKernelMM}{\ensuremath{\mathbf{K}_{\modeInd \numInducing \numInducing}}}
\renewcommand{\expertKernelMn}{\ensuremath{\mathbf{k}_{\modeInd \numInducing \numData}}}
\renewcommand{\expertKernelMN}{\ensuremath{\mathbf{K}_{\modeInd \numInducing \numData}}}
\renewcommand{\expertKernelsM}{\ensuremath{\mathbf{k}_{\modeInd * \numInducing}}}}
...
\renewcommand{\gatingKernelNN}{\ensuremath{\hat{\mathbf{K}}_{\modeInd \numData \numData}}}
\renewcommand{\gatingKernelnM}{\ensuremath{\hat{\mathbf{k}}_{\modeInd \numData \numInducing}}}
\renewcommand{\gatingKernelNM}{\ensuremath{\hat{\mathbf{K}}_{\modeInd \numData \numInducing}}}
\renewcommand{\gatingKernelMM}{\ensuremath{\hat{\mathbf{K}}_{\modeInd \numInducing \numInducing}}}
\renewcommand{\gatingKernelMn}{\ensuremath{\hat{\mathbf{k}}_{\modeInd \numInducing \numData}}}
\renewcommand{\gatingKernelMN}{\ensuremath{\hat{\mathbf{K}}_{\modeInd \numInducing \numData}}}
...
\Delta \state_{\timeInd} &= \mode{\mathbf{g}} (\state_{\timeInd}, \control_{\timeInd} ; \Delta t = t_*) + \mode{\bm\epsilon}
...
= \frac{\sum_{\modeInd=1}^{\ModeInd} \allGatingLikelihood \gatingPrior \allExpertLikelihood \expertPrior}{\evidence}
\end{align}
#+END_EXPORT
where the denominator is the marginal likelihood from Eq. ref:eq-marginal-likelihood-factorised.
...
be evaluated analytically whilst accurately modelling the complex dependencies.
Further to this, the marginal likelihood is extremely expensive to evaluate $\mathcal{O}(\ModeInd^2 \NumData^{4})$
...
The set of all inducing inputs and the set of all inducing variables associated with the experts have been
denoted $\expertsInducingInput$ and $\expertsInducingOutput$ respectively.
Similarly for the gating network, the set of all inducing variables has been denoted $\gatingsInducingOutput$.
...
\end{align}
#+END_EXPORT
Moving the marginalisation over the gating functions outside of the marginalisation over the expert indicator
variable is possible because the gating functions are assumed to be independent
i.e. the mixing probabilities are dependent on *all* of the gating functions and
...
Moving the marginalisation over each experts latent function outside of the marginalisation over the expert
indicator variable corresponds to changing the underlying model.
Instead of mixing the GPs associated with each expert this approximation would simply lead to mixing their associated
noise models.
...
variational posterior is given by,
#+BEGIN_EXPORT latex
\begin{align}
%\label{eq--variational-posteriors-functional}
...
Importantly, this variational posterior $\gatingsVariational$
marginalises $\gatingsInducingVariational$ in closed-form,
...
$(\ModeInd +1)\NumInducing\InputDim$ optimisation parameters.
...
$\mathcal{O}\left( \NumData_{b} \ModeInd \NumInducing^{2} \right)$ to evaluate.
...
\todo{check complexities}

*** Predictions
For a given test input $\x_*$, this model makes probabilistic predictions following
...
#+BEGIN_EXPORT latex
\newcommand{\predictivePosterior}{\ensuremath{p(\y_* \mid \x_*)}}
\newcommand{\predictiveProb}{\ensuremath{\Pr(\modeVar_*=\modeInd \mid \x_*)}}
\newcommand{\predictiveExpert}{\ensuremath{q(\y_* \mid \modeVar_*=\modeInd, \x_*)}}
\newcommand{\predictiveExpertLikelihood}{\ensuremath{p(\y_* \mid \mode{f}(\x_*))}}
\newcommand{\predictiveExpertPrior}{\ensuremath{q(\mode{f}(\x_*))}}
\newcommand{\predictiveExpertsPrior}{\ensuremath{q(\mathbf{f}(\x_*))}}
\newcommand{\predictiveGatingLikelihood}{\ensuremath{\Pr(\modeVar_*=\modeInd \mid \mathbf{\gatingFunc}(\x_*))}}
\newcommand{\predictiveGatingLikelihoodBernoulli}{\ensuremath{\Pr(\modeVar=1 \mid \gatingFunc_1(\x_*))}}
\newcommand{\predictiveGatingPrior}{\ensuremath{q(\mathbf{\gatingFunc}(\x_*))}}
\newcommand{\predictiveGatingPriorBernoulli}{\ensuremath{q(\gatingFunc_1(\x_*))}}
...
is obtained by taking the expectation of its Gaussian likelihood $\predictiveExpertLikelihood$
...
#+END_EXPORT
where the predictive posterior $\predictiveExpertPrior$ is given by,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-predictive-expert}
\label{eq--variational-posteriors-functional}
q(\mode{\latentFunc(\x_*)}) &= \mathcal{N} \left( \mode{\latentFunc}(\x_*) \mid
\mode{\mathbf{A}} \mode{\mathbf{m}},
\expertKernelss
+ \mode{\mathbf{A}}
...
\end{align}
#+END_EXPORT
with  $\mode{\mathbf{A}} = \expertKernelsM \expertKernelMM^{-1}$.
...
The two lower bounds ($tightBound$ and $\furtherBound$),
