...
This chapter assumes access to a data set $\dataset$ of state transitions, that
...
\Delta \state_{\timeInd} &= \mode{\mathbf{g}} (\state_{\timeInd}, \control_{\timeInd} ; \Delta t = t_*) + \mode{\epsilon}
&\text{if} \quad \modeVar_{\timeInd} = \modeInd \\
&= \underbrace{\nominalDynamicsK(\state_t, \control_t ; \Delta t = t_*)}_{\text{nominal mode \modeInd}}
+ \underbrace{\unknownDynamicsK(\state_t, \control_t ; \Delta t = t_*)}_{\text{unknown error mode \modeInd}}
+ \mode{\bm\epsilon}(t)
...
i.e. $\fk(\cdot)$ is assumed to be $\mode{\latentFunc}(\cdot)$.
...
where the mapping switches between $\ModeInd$ different functions $\unknownDynamicsK$.
This is a regression problem,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-multimodal-dynamics-disc}
\underbrace{\Delta \state_t - \nominalDynamicsK(\state_t, \control_t ; \Delta t = t_*)}_{\text{\singleOutput}}
= \underbrace{\unknownDynamicsK(\state_t, \control_t ; \Delta t = t_*)}_{\text{\mode{\latentFunc} (\singleInput)}}
+ \mode{\epsilon}
...
$\modeVar$ must be learned.
A single output is denoted as
$\singleOutput = \Delta \state_t - \nominalDynamicsK(\state_t, \control_t ; \Delta t = t_*)$.
...
Learning representations of dynamical systems for controlling in dynamical systems
using probabilistic models has shown much promise.
For example, see
...
1. can /identify/ the true underlying dynamics modes accurately,
   - this could be achieved by providing handles for encoding domain knowledge,
...
In contrast, we seek enhanced identifiability that learns only the two true dynamics modes.
...
over the input and output space $p(\mathbf{y}, \mathbf{x})$,
...
where each Gaussian cluster $z=\{1,\ldots, K\}$
...
and a softmax link function, enables complex nonlinear dependencies between the expert indicator
...
More accurately modelling this dependency will result in superior generalisation,
...
This is indicated by worse Negative Log Predictive Density (NLPD) scores than a SVGP on 6 out of 7 the
...
\todo{cite trappDeep2020,nguyenStochastic2018,gaddEnriched2020}
