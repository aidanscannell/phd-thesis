...
%For example, controlling a quadrotor subject to unoperable dynamics modes that are
...
%constraints may be violated i.e. the system may enter unoperable dynamics modes.
...
\caption{\label{fig-problem-statement}
...
The goal is to find trajectories from a start state $\state_0$, to the target state (red star),
...
As such, the quadcopter does not have access to a historical data set for model learning.
...
# \begin{figure}
# \centering
# \includegraphics[width=0.8\textwidth]{./images/point-mass-problem-statement-scenario-4.pdf}
# \caption{\label{fig-point-mass-motivation}
# Diagram showing a top down view of an environment, representing a quadcopter
# subject to two dynamics modes: 1) a turbulent dynamics mode
# induced by a fan (green) and 2) a non-turbulent dynamics mode everywhere else (blue).
# The goal is to find trajectories from a start state $\state_0$, to the target state (red star),
# whilst avoiding 1) the turbulent dynamics mode and 2) regions of the learned dynamics that
# cannot be predicted confidently due to lack of observations.}
# \end{figure}
# #+END_EXPORT
#+BEGIN_EXPORT latex
\begin{figure}[t!]
...
white indicates the turbulent dynamics mode and red indicates the desied (operable) mode.
...
\end{figure}
#+END_EXPORT
This chapter is motivated by the illustrative example in cref:illustrative_example, i.e.
flying the quadcopter from an initial state $\state_0$, to a target state $\state_{f}$.
See cref:fig-problem-statement for a schematic of the environment and details of the problem.
The turbulent dynamics mode is subject to higher drift in the negative $y$ direction.
This is due to the wind field created by the fan.
It is also subject to higher diffusion (or higher process noise) resulting from the turbulence induced by the fan.
Although the exact turbulent dynamics are not known, they are believed to be difficult to control due to high process
noise, which may lead to catastrophic failure.
...
this chapter seeks to find trajectories from a start state $\state_0$, to a target state $\targetState$,
whilst avoiding the turbulent dynamics mode.

Given a historical data set of state transitions from a system, a common approach is to first
learn a single-step dynamics model using a Gaussian process.
Following standard methodologies, trajectories can be found by minimising the expected cost under the state distribution,
resulting from cascading single-step predictions through the GP dynamics model.
A cost function consisting of a terminal state cost term ensures that trajectories end at the target state.
It is also common to include a quadratic integral control cost term to regularise the controls to
encode the notion of "minimal effort" trajectories.

# cref:fig-traj-opt-over-gating-mask-svgp-all-baseline shows results of performing trajectory optimisation using a SVGP
# dynamics model after training on state transitions sampled from both dynamics modes.
cref:fig-traj-opt-over-gating-mask-svgp-all-baseline demonstrates the shortcomings of simply performing trajectory
optimisation in a SVGP dynamics model after training on state transitions sampled from both dynamics modes.
It shows that the SVGP is not able to learn a representation of the dynamics which is true to the underlying system.
This is because it cannot model the mulitmodal behaviour present in the environment.
The optimised controls successfully drive the SVGP dynamics from the start state $\state_0$,
to the target state $\targetState$.
However, as the SVGP dynamics model does not correspond to the true underlying dynamics,
applying the optimised controls in the environment
does not result in the trajectory successfully navigating to the target state $\targetState$.
...
Conveniently, the MoSVGPE method from cref:chap-dynamics can be used to learn a factorised representation of the underlying
dynamics modes.
...
algorithms after training a MoSVGPE dynamics model on the historical data set of state transitions.
...
#      in closed form.
#      We seek to learn an operatable mode with one expert and explain away the inoperable mode
...
As such, it seeks to correctly identify the underlying dynamics modes whilst inferring
...
This work derives novel evidence lower bounds based on sparse GP approximations, that provide
well-calibrated uncertainty estimates and scalability via stochastic gradient methods.
The method presented in this chapter is named
Mixtures of Sparse Variational Gaussian Process Experts (MoSVGPE).
...
The work in this chapter is implemented in GPflow/TensorFlow and is available on GitHub[fn:1:Code accompanying this Chapter can be found @ [[https://github.com/aidanscannell/mogpe][\icon{\faGithub}/aidanscannell/mogpe]].]
cite:matthewsGPflow2017,GPflow2017,tensorflow2015-whitepaper.
...
MoGPE model cite:trespMixtures2000a. Our marginal likelihood is given by,
#+BEGIN_EXPORT latex
...
cref:fig-graphical-model-sparse shows the graphical model of our augmented joint probability space.
...
and used to obtain a variational lower bound, aka Evidence Lower Bound (ELBO).
In this section, we derive three ELBOs.
...
The model was trained using the model and training parameters shown in cref:tab-params-quadcopter.
...
*Identifiability*
These results show that our gating network is capable of turning a single expert
...
Motivated by synergising model learning and control, in particular, 1) correctly identify the
underlying dynamics modes and 2) inferring latent structure that can be exploited for control,
...
is used to derive three ELBOs that provide a nice coupling between the
optimisation of the experts and the gating network, by efficiently marginalising the expert indicator variable for single
data points.
The three different ELBOs are compared on the Motorcycle data set cite:Silverman1985.
...
* Mode Remaining Trajectory Optimisation via Riemannian Manifolds label:chap-traj-opt
** Maths :ignore:
#+BEGIN_EXPORT latex
...
The MoSVGPE method from cref:chap-dynamics was intentionally formulated with latent variables
...
a single-step predictive dynamics model learned using the MoSVGPE method from cref:chap-dynamics.
...
an ODE that implicitly encodes the mode remaining behaviour.
The second approach is a direct control method that resembles standard Gaussian process control methods
with a geometric objective function that encodes the mode remaining behaviour.
...
The work in this chapter is implemented in JAX and TensorFlow and is available on GitHub
...
\label{eq-traj-opt-gating-network-gp}
\caption{
Visualisation of the GP posterior mean (left) and posterior variance (right), associated with the
desired mode's gating function after training the MoSVGPE
...
\end{figure}
#+END_EXPORT
The work in this chapter first uses the MoSVGPE method from cref:chap-dynamics to learn a factorised
...
the MoSVGPE method from \cref{chap-dynamics}.
\end{assumption}
#+END_EXPORT
...
#+BEGIN_EXPORT latex
\begin{definition}[$\delta$-mode remaining] \label{}
Let $\controlTraj$ be a sequence of controls with corresponding open-loop system $\controlledDynamicsFunc$.
Let $\state_0 \in \desiredStateDomain$ and $\delta \in (0,1]$.
A system is said to be $\delta$-mode remaining under the controls $\controlTraj$ iff:
\begin{align}
\Pr( \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd] : \controlledDynamicsFunc(\state_{\timeInd}) &\in \desiredStateDomain,
...
cref:fig-traj-opt-gating-network visualises this GP posterior after training on a historical data set of state
transitions from a simulator of the illustrative example in cref:illustrative_example.
...
Given this model, this work seeks to find a control trajectory $\controlTraj = \control_{0:\TimeInd-1}$,
to navigate from an initial state $\state_{0}$, to a target state $\targetState$,
...
cref:eq-traj-opt-gating-network-gp shows the GP posterior associated with the desired mode's gating function
after training
MoSVGPE on the historical data set of state transitions from the quadcopter experiment in cref:illustrative_example.
...
It then details two control methods that exploit the geometry of the MoSVGPE model to obtain mode remaining behaviour.
...
The relevant concepts from Riemannian geometry are now introduced.
...
This algorithm further exploits the fact that geodesic trajectories are solutions to a 2^{nd} order ODE and projects
the optimisation onto this ODE.
# #+BEGIN_EXPORT latex
# \begin{remark}
# \end{remark}
# #+END_EXPORT
...
Solving the 2^{nd} order ODE in cref:eq-2ode with the expected metric from cref:eq-expected-metric-weighting,
...
\end{subequations}
#+END_EXPORT
with a quadratic cost function to regularise the controls,
#+BEGIN_EXPORT latex
...
The nonlinear program in cref:eq-collocation-problem is solved using Sequential Least Squares
Programming (SLSQP) in SciPy.
...
*** Remarks :ignore:
#+BEGIN_EXPORT latex
\begin{remark}[Dynamics Constraints]
This approach does do not necessarily find trajectories that satisfy the dynamics constraints,
as it projects the problem onto the geodesic ODE.
\end{remark}
#+END_EXPORT
#+BEGIN_EXPORT latex
\begin{remark}[Stochasticity]
This algorithm ignores much of the stochasticity inherent in the problem.
\end{remark}
#+END_EXPORT
It does not consider the full distribution over state-control trajectories.
...
This is because the geodesic ODE will become a Stochastic Differential Equation (SDE).
...
# This is because the geodesic ODE will become a Stochastic Differential Equation (SDE).
#+BEGIN_EXPORT latex
\begin{remark}[Differential Flatness]
This method is only applicable in systems where the state and control can be expressed in terms of a flat output.
\end{remark}
#+END_EXPORT
Perhaps the biggest limitation of this method, is its dependence on
differential flatness to obtain the controls from the optimised state trajectory and geodesic ODE.
...
1) is not limited to dynamical systems which are differentially flat,
2) guarantees trajectories satisfy the dynamics constraints,
3) principally handles the uncertainty associated with the dynamics.
...
Calculating multi-step predictions in the MoSVSGPE dynamics model cannot be calculated in closed-from.
This is because the distribution over the state difference after the first time step is a Gaussian mixture and
...
#+END_EXPORT
# *Approximate Inference for Dynamics Predictions*
Constructing approximate closed-form solutions for multi-step predictions using MoSVGPE is difficult,
...
all of the modes, over a trajectory of length $N$, would lead to the final state consisting of
$\ModeInd^N$ Gaussian components.
\end{myquote}
#+END_EXPORT

This work sidesteps this issues by assuming that the controlled system remains in the desired dynamics mode.
#+BEGIN_EXPORT latex
\begin{assumption}[Mode Remaining Dynamics] \label{}
Given a desired dynamics mode $\desiredMode$, defined by its state domain $\desiredStateDomain \subseteq \stateDomain$,
an inital state $\state_0 \in \desiredStateDomain$, and a sequence of controls $\controlTraj$,
...
the desired mode's GP dynamics model, i.e. recursively calculating the following integral,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-state-unc-prop}
p(\state_{\timeInd+1} \mid \state_0, \control_{0:\timeInd}, \modeVar_{0:\timeInd}=\desiredMode)
&= \int \transitionDistK p(\state_\timeInd \mid \state_{0}, \control_{0:\timeInd-1}, \modeVar_{0:\timeInd-1}=\desiredMode) \text{d}\state_{\timeInd}
\end{align}
#+END_EXPORT
...
2) The expected Riemannian energy in cref:eq-quadratic-cost-with-energy has no closed-form expression,
   i) Due to the metric's $\metricTensor$ dependence on the state, via the Jacobian.



The expected Riemannian energy in cref:eq-quadratic-cost-with-energy is given by,
#+BEGIN_EXPORT latex
...
\end{align}
#+END_EXPORT
which has no closed-form expression, due to the metric's $\metricTensor$ dependence on the state, via the Jacobian.
Motivated by obtaining a closed-form expression it is possible to approximate the expected energy.
...
However, this work simplifies the problem and approximates the full Jacobian distribution over the trajectory,
to be along the state mean trajectory, i.e.
...
\end{align}
#+END_EXPORT
Given this approximation for the expected energy of a trajectory, the only
...
As such, the expectation in cref:eq-mode-soc-problem-geometry is taken w.r.t. the joint distribution
over the state and the metric tensor over a trajectory, $p(\stateTraj, \metricTensor, \mid \state_0, \controlTraj)$
given controls $\controlTraj$ and starting state $\state_0$.
...
#+END_EXPORT
*Approximate Inference for Dynamics Predictions*
As this work is interested in remaining in a single dynamics mode $\desiredMode$,
multi-step predictions can be made using the single dynamics GP.
...
#+BEGIN_EXPORT latex
\begin{align}
\transitionDistK &= \expertVariational \nonumber \\
&=
\mathcal{N} \left( \mode{\latentFunc}(\singleInput) \mid
\mode{\mathbf{A}} \mode{\mathbf{m}},
\expertKernel(\singleInput, \singleInput)
...
To obtain long-term predictions, one-step predictions can be cascaded.
...
Given this method for making long term predictions, the distribution over state trajectories is
given by,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
p(\stateTraj, \controlTraj \mid \state_0)
= \prod_{\timeInd=0}^{\TimeInd-1} p(\state_{\timeInd+1} \mid \state_0, \controlTraj).
\end{align}
#+END_EXPORT
...
*** Real-World Quadcopter Experiments label:sec-traj-opt-results
#+BEGIN_EXPORT latex
\begin{figure}[!t]
\centering
\includegraphics[width=0.7\textwidth]{./images/geometric-traj-opt-over-prob.pdf}
\caption{Trajectory optimisation results after solving the nonlinear program in \cref{eq-collocation-problem}
using mode one's $\modeVar=1$ gating function from the MoSVGPE (with $\ModeInd=2$ experts)
after training on the real-world velocity
controlled quadcopter data set.
The intial (cyan) and optimised trajectories' -- for two settings of $\lambda$ -- are
overlayed on the desired mode's mixing probability.}
...
The trajectory optimisation is evaluated on the real-world quadcopter control problem from the illustrative example
in cref:fig-problem-statement.
In this example, mode 1 represents the operable dynamics mode whilst mode 2 represents the inoperable turbulent
dynamics mode.
...
A subset of the observations were removed from the data set to
test the trajectory optimisation's ability to avoid regions of high epistemic uncertainty (Goal 2).
...
We engineered our desired dynamics mode to have a noise variance that we deemed operable.
We then used the other dynamics mode to explain away all of the inoperable modes.
...
*** Simulated Quadcopter Experiments label:sec-traj-opt-results
#+BEGIN_EXPORT latex
...
#+END_EXPORT
This section evaluates the performance of ... using the illustrative example from cref:illustrative_example.
Due to coronavirus and limited access to the laboratory the methods are tested in a simulator representing the
environment from cref:fig-problem-statement.
...
and deterministic controls (no maximum entropy control).
The optimised controls are rolled out in the desired mode's GP dynamics (magenta) and in the environment (cyan).
The state trajectories are overlayed on (\subref{fig-riemannian-energ-highty-traj-opt-over-prob})
each mode's mixing probability
and (\subref{fig-riemannian-energy-hight-traj-opt-over-svgp})
the posterior mean (left) and posterior variance (right) associated with desired mode's gating function.}
\label{fig-riemannian-energy-traj-opt}
\end{figure}
#+END_EXPORT
...
**** Riemannian Energy
Minimising the Riemannian energy of a trajectory  principally encodes both goals
whilst also providing a mechanism to decouple them.
Similarly to cref:sec-traj-opt-geometric, the expected metric tensor is augmented with a weight
parameter $\lambda$ that can be adjusted by the user to determine the relevance of the covariance term.
This augmented expected metric decouples the mode remaining behaviour and the uncertainty avoiding behaviour.

cref:fig-riemannian-energy-low-traj-opt shows results after performing trajectory optimisation using
the approximate Riemannian energy cost function in cref:eq-approximate-trajectory-riemannian-energy
with $\lambda=0.1$.
This method used deterministic controls, that is, no maximum entropy control.
...
In a scenario where the main goal is to remain in the desired dynamics mode and avoiding regions of high
epistemic uncertainty is a secondary goal, this may be a desirable behaviour.
However, as mentioned previously, this is not desirable in a risk-averse setting.
...
The right plots show the trajectories avoiding regions of high posterior covariance.
Note that the Riemannian energy contains the covariance of the Jacobian of the gating function, not the
covariance of the gating function itself.
Both of these $\lambda$ settings find trajectories that avoid the region of high uncertainty.
They appear to initially avoid the region of high posterior covariance and then attempt to follow the
posterior mean's contours, successfully remaining in the desired dynamics mode.
...
which lead to the clearance from the turbulent dynamics mode reducing significantly.
One downside to the Riemannian energy approach is that the $\lambda$ parameter must be tuned to obtain
the desired behaviour.

\todo{modify metric to have lambda again}
**** Mode Chance Constraints
An alternative approach is to obtain mode remaining behaviour by optimising subject to the chance constraints
in cref:eq-mode-chance-constraint.
However, in this particular scenario, the optimisation was not able to converge successfully
when using a satisfation probability $\satisfactionProb=0.7$.
It is also worth noting that incorporating these chance constraints into the optimisation significantly slows it
down.
This is because each evaluation of the chance constraints requires simulating a trajectory in the desired mode's
dynamics GP.
For this reason, I believe it is best to use the chance constraints to validate trajectories found by the
optimiser -- before deploying them in the environment -- as opposed to using them to constrain the optimisation.
...
* Mode Constrained Trajectory Optimisation via Probabilistic Inference label:chap-traj-opt
** Intro :ignore:
This chapter is concerned with controlling /unknown/ or /partially unknown/, multimodal dynamical systems,
given a historical data set of state transitions $\dataset$, sampled from the system.
In particular, it is concerned with /mode constrained/ trajectory optimisation, which attempts to
...
# That is, find trajectories that also avoid entering regions of the transition dynamics that
...
This chapter introduces two different approaches to performing /mode constrained/ trajectory optimisation.
Both approaches are model-based techniques that leverage the MoSVGPE method from cref:chap-dynamics to learn a
single-step predictive dynamics model.
The first method exploits concepts from Riemannian geometry -- extended to probabilistic manifolds -- to
obtain mode remaining behaviour.
The second approach extends the control as inference framework to multimodal dynamical systems, which
conditions on the goals and infers the optimal controls.
...
** Problem Statement label:sec-problem-statement
This chapter assumes access to a dynamics model learned using the MoSVGPE model from cref:chap-dynamics and the
historical data set of state transitions.
Further to this, it assumes that a desired dynamics mode $\desiredMode$ is /known a priori/.
The desired mode's transition dynamics are given by
$\desiredDynamicsFunc: \desiredStateDomain \times \controlDomain \rightarrow \stateDomain$,
with continuous states $\state \in \stateDomain \subseteq \R^\StateDim$ and controls
...
#+BEGIN_EXPORT latex
\begin{definition}[\delta-mode remaining] \label{}
Let $\controlTraj$ be a control trajectory with corresponding open-loop system $\controlledDynamicsFunc$.
Let $\state_0 \in \desiredStateDomain$ and $\delta \in (0,1]$.
A system is said to be $\delta$-mode remaining under the controls $\controlTraj$ iff:
\begin{align}
\Pr( \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd] : \controlledDynamicsFunc(\state_{\timeInd}) &\in \desiredStateDomain,
...
# $\controlledDynamicsFunc: \desiredStateDomain \times \controlDomain \rightarrow \desiredStateDomain$,
This work seeks to find the control trajectory $\controlTraj = \control_{0:\TimeInd-1}$,
to navigate from an initial state $\state_{0}$, to a target state $\targetState$,
over a horizon $\TimeInd$,
whilst minimising a cost function, $\costFunc: \stateDomain \times \controlDomain \rightarrow \R$
and keeping the system in the desired dynamics mode $\desiredMode$.
...
$\valueFunc$, for the expected cost.
For $\timeInd = 0,\ldots, \TimeInd-1$ the value function is given by,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-bellman-equations}
\valueFunc(\state) &= \min_{\control} \E\left[
\costFunc(\state, \control) + \valueFunc(\dynamicsFunc(\state, \control)) \right],
\end{align}
#+END_EXPORT
and the terminal value is given by the expected terminal cost
$\valueFunc(\state_\TimeInd) = \E \left[ \costFunc(\state_\TimeInd) \right]$.
...
*** intro :ignore:
The MoSVGPE method from cref:chap-dynamics can be used to learn a factorised representation of the underlying
dynamics modes.
...
\centering
\includegraphics[width=\textwidth]{./images/traj-opt/mode-remaining/mosvgpe/desired_gating_gp_no_traj.pdf}
\subcaption{Desired mode's gating function GP.}
\label{eq-traj-opt-gating-network-gp}
\end{minipage}
\caption{\label{fig-traj-opt-gating-network}
Visualisation of the latent variable posteriors associated with the gating network after training the MoSVGPE
(with $\ModeInd=2$ experts) on the historical data set of state transitions from the 2D quadcopter environment
in the illustrative example from \cref{illustrative_example}.
...
\costFunc_{\text{mode}}(\state_{\timeInd}, \control_{\timeInd}) = - \Pr (\modeVar_{\timeInd} = \desiredMode),
\end{align}
#+END_EXPORT
...
from cref:eq-predictive-gating. Note the dependence on the state input $\state_{\timeInd}$ is reintroduced here,
...
Due to the stochastic nature of our problem, they must be formulated as a chance constraint,
#+BEGIN_EXPORT latex
...
and recursively propagating each component through all of the modes.
...
**** ELBO :ignore:
*Evidence Lower BOund* Variational inference seeks to optimise $q(\stateTraj, \controlTraj \mid \state_0)$
...
&+ \sum_{\timeInd=0}^{\TimeInd-1} \underbrace{\E_{\controlVarDist q(\state_{\timeInd})}
...
**** Trajectory Optimisation :ignore:
*Optimisation*
Maximum entropy control is achieved by parameterising the control at each time step to be Normally distributed
${\controlVarDist = \mathcal{N}(\control_{\timeInd} \mid \controlMean, \controlCov)}$.
In practice, we assume that the control dimensions are independent so $\controlCov$ becomes diagonal.
...
# *Dynamics Configuration* The model from cref:chap-dynamics was instantiated with $\ModeInd=2$ experts, one to
# represent the operatable dynamics mode and one to represent the mode subject to high levels
...
*Mode Conditioning*
- compare max entropy vs no max entropy
...
In the MoSVGPE model, this form of epistemic uncertainty is modelled by the gating function's posterior variance.
