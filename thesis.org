...
* Quadcopter Experiments - Mode Remaining Control label:chap-traj-opt-results :noexport:
** maths :ignore:
#+BEGIN_EXPORT latex
...
\end{definition}
#+END_EXPORT
Notice that the controller $\policy : \stateDomain \times \mathbb{Z} \rightarrow \controlDomain$
is defined generally.
That is, it may take the form of a deterministic feedback policy or an open-loop set of controls
...
%\modeVar_{\timeInd} = \desiredMode \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd] \\
%&\state_{\timeInd} \in \desiredStateDomain \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd] \\
&\state_{0} = \state_0, \quad \state_\TimeInd = \targetState,
\end{align}
\end{subequations}
#+END_EXPORT
...
It may be used to balance behaviours such as speed and efficiency.
cref:eq-mode-explore-problem resembles the problem statement from cref:chap-traj-opt-inference,chap-traj-opt-geometry,
except that it does not assume /a priori/ access to the environment.
...
This model can be used to learn an initial controller, however,
it may not work outside of the initial state domain $\initialStateDomain$ and may not be able to find
$\delta-\text{mode remaining}$ trajectories to the target state $\targetState$, due to the model's epistemic uncertainty.
...
This section provides an overview of existing exploration strategies and how they fit into
the general \acrfull{mbrl} procedure shown in  cref:alg-mbrl.

*Greedy exploitation*
One of the most commonly used exploration strategies is to select the controller that maximises
the expected performance under the learned dynamics model $\dynamicsModel(f \mid \mathcal{D}_{0:i})$.
...
\pi_i^{\text{greedy}} = \arg \max_{\pi \in \Pi} \E_{f \sim \dynamicsModel(f \mid \mathcal{D}_{0:i})} \left[ J(f, \pi) \right].
\end{align}
#+END_EXPORT
...
*$\delta-\text{mode remaining}$ exploration*
The exploration strategy is primarily interested in exploring regions of the gating network that are uncertain.
This work proposes a trajectory optimisation algorithm that principally ensures solutions are
$\delta-\text{mode remaining}$ via chance constraints (cref:eq-mode-chance-constraint-explore).
...
\end{subequations}
#+END_EXPORT
where $\controlCostMatrix$ is a user defined, real, symmetric, positive definite matrix.
...
As such, this work samples a fake target state from the initial data set $\dataset_0$ and
optimises the initial trajectory using the cost function in cref:eq-quadratic-cost-control.
Experiments show that this procedure finds trajectories where all of time steps are
in the desired mode with high probability.
...
\desiredGatingMeanFunc(\state, \state') &=
...
This arises because the \acrshort{mosvgpe} model uses sparse \acrshort{gp}s and
...
data set of state transitions $\dataset_0$ from the desired dynamics mode.
...
At the core of this method is the exploration strategy.
This work uses the exploration strategy from cref:sec-exploration to construct an explorative controller.
...
It should be noted that extending the method in cref:sec-exploration to use a feedback policy should not
be too difficult.
The trajectory optimisation algorithm could be used for guided policy search cite:levineGuided2013
or the objective could be modified to account for infinite horizon with the chance constraints implemented via
lagrange multipliers.
It is also straightforward to check that trajectories under a feedback policy are $\delta-\text{mode remaining}$.
