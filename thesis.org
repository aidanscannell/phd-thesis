...
Given this definition of a mode remaining controlled system, this work seeks to formally solve,
#+BEGIN_EXPORT latex
...
** Preliminary Results
This section presents some initial results on the illustrative quadcotper navigation
problem from cref:illustrative_example using the simulator from cref:sec-traj-opt-results-simulated.
In particular, \modeOpt is tested in Environment 1 from cref:sec-traj-opt-results-simulated.
See cref:sec-traj-opt-results-simulated for more details on the environment and the simulator set up.

cref:fig-explorative-traj-opt-7-initial shows the start/target states $\state_0/\targetState$,
...
It tends to $0.5$ away from the data, i.e. maximum entropy for a Categorial distribution.
cref:fig-explorative-data-over-svgp-7-initial shows the \acrshrot{gp} posterior over the desired mode's gating function.
The posterior variance (right) is low around the initial data set and high everywhere else.
These figures are useful for visualising \modeOpt.
To further aid visualisation, the $1-\delta$ contour is marked on all plots (purple line).
The experiments use $\delta=0.2$ which corresponds to the $\Pr(\modeVar=\desiredMode \mid \state) =0.8$ contour.

*** Model Learning
# To begin with, $\NumData$ state transitions were sampled from the simulator with $\Delta \timeInd = 0.25s$.
Following the experiments in cref:sec-traj-opt-results
...
The initial data set was collected from $15$ random trajectories from the start state $\state_0$ that were
terminated when they left the initial state domain $\initialStateDomain$.
This resulted in an initial data set with $118$ elements.
...
\centering
%\includegraphics[width=0.78\textwidth]{./images/mode-opt/exploration/trajectories_over_mixing_probs_step_\stepNum.pdf}
\adjincludegraphics[trim={{.54\width} 0 0 0},clip, width=0.6\textwidth]{./images/mode-opt/exploration/state-diff-only/trajectories_over_mixing_probs_step_19.pdf}
...
*** state diff vs entropy vs :ignore:
#+BEGIN_EXPORT latex
\begin{figure}
\centering
\begin{minipage}[r]{0.48\columnwidth}
...
Intuitively, a strategy which favours searching regions of the state space which have not previously observed
could likely avoid the local optima in cref:fig-explorative-state-diff-traj-opt-over-prob-7.
This intuition motivates the joint gating entropy term in cref:eq-explorative-traj-opt.
...
This section compares the joint gating entropy term in cref:eq-explorative-traj-opt with the sum of individual
entropy's over the trajectory, which is referred to as the factorised entropy.
cref:fig-entropy-comparison-traj-opt-7 shows the trajectories found at the first iteration
when using the two entropy objectives.
The factorised entropy objective, show in
cref:fig-explorative-mvn-diag-traj-opt-over-prob-7, shows that the entire trajectory collapses onto a single state
of high entropy.
This is an undesirable behaviour because it does not maximise the information gain along the entire trajectory.
In contrast, the result in cref:fig-explorative-mvn-full-cov-traj-opt-over-prob-7 consider the joint entropy over
...
*Balancing*
These results suggest that balancing the state difference and the joint gating entropy terms in
...
cost matrices, $\stateCostMatrix$ and $\controlCostMatrix$, resulted in the algorithm converging in the
experiments.
...
** Discussion & Future Work
This section discusses the method presented in this chapter and proposes some directions for future work.
...
environments with larger state domains. In these environments, the MoSVGPE’s
