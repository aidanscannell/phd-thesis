...
#+END_EXPORT
** To-Do List :noexport:
- [X] Check multiline Eqns only have one eq number
- [ ] Add link to youtube video - TODO https://www.youtube.com/watch?v=VqjkbfIsghM&ab_channel=AidanScannell
- [X] Finish writing up mcycle results
...
- [-] Finish writing up mcycle results
  - [X] compare bounds
    ...
    - [X] compare complexity of bounds? or speed to run?
    - [X] compare how they treat gating network
      + max entropy in indicator var dist (further) vs gating function dist (tight)
  - [X] compare batch size
  - [X] finish writing two experts
  - [X] finish writing three experts
  - [X] update mcycle table with correct NLML
    + NLML/num_data on test_dataset not full_dataset
  - [X] test num inducing points?
...
- [ ] example showing why we want GP based gating network?
  ...
  + or sigmoid kernel?
- [X] Update quadcopter results
  + [X] Add quiver
- [X] Add comment about not optimising inducing points leading to better performance
- [ ] Add discussion
...
to represent the uncertainty in the mode indicator variable $\modeVar$.
...
*Cost Function - Length Minimising Trajectories*
...
The $g_{\text{mode}}$ term in cref:eq-cost-trajectory can be expressed as finding length minimising
trajectories on the desired mode's gating function.
...
Note that the mode indicator variable is a random variable
$\Pr(\modeVar_{\timeInd} = \desiredMode ) \geq \delta_{\modeVar} \quad \forall \timeInd \in \mathbb{Z} \cap [1,\TimeInd]$

However, none of these approaches provide a principled approach for weighting either of the goals.


*Gating Network Posterior*
- *Cost* Minimise length of trajectory on manifold parameterised by the desired mode's gating function,
  + Note that this function is probabilistic,

latent /geometry/ inferred by the GP-based gating network from Chapter ref:chap-dynamics.
