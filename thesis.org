...
\dot{\mathbf{x}}(t) &= \f(\state(t), \control(t)) + \epsilon(t) \\
&= \fk(\state(t), \control(t)) + \mode{\epsilon}(t) \quad \text{if} \quad \alpha(t)=\modeInd, \\
\mode{\epsilon}(t) &\sim \mathcal{N}\left(0, \Sigma_{\mode{\epsilon}} \right),
\end{align}
#+END_EXPORT
...
$\mode{\epsilon}(t) &\sim \mathcal{N}\left(0, \Sigma_{\mode{\epsilon}} \right)$, where
$\Sigma_{\mode{\epsilon}} = \text{diag}\left[ \left(\noiseVarOneK\right)^{2}, \ldots, \left( \noiseVarDK \right)^{2} \right]$,
are selected by a discrete mode indicator variable,
$\alpha(t) \in \modeDomain = \{1,\ldots,\ModeInd\}$, at any given time $t$.
This work considers systems that are stochastic (i.e. subject to process noise), but
are not subject to observation noise.

This work assumes access to historical data comprising state transitions from $\NumEpisodes$ trajectories,
of length $N$, sampled  with a  fixed  time step $\Delta t=t_{*}$.
The data set has ${T=EN}$ elements and we abuse notation by appending the independent trajectories along
time to get the data set $\mathcal{D}=\{(\state_{\timeInd-1},\control_{\timeInd-1}),\Delta \state_t\}^T_{t=1}$.
...
\singleOutput = \fk (\singleState, &\singleControl ; \Delta t = t_*) + \mode{\epsilon}
\quad \text{if} \quad \modeVarK,
\end{align}
#+END_EXPORT
where
$\singleOutput = \state_{\timeInd} - \state_{\timeInd-1}$ is the change in state between time $\timeInd-1$
and $\timeInd$ and
...
# Given the data set $\mathcal{D}$, the goal is to learn a predictive model that can predict the
...
the integral in Eq. ref:eq-gating-posterior can be calculated in closed-form,
if the sigmoid function is selected
...
\mode{\K}
\right), \\
\singleLatentGatingGivenInducing &= \mathcal{N}\left( \mathbf{\gatingFunc}(\singleInput) \mid
...
Instead of collapsing the inducing variables as seen in citep:titsiasVariational2009,
...
# \newline
*Lower Bound 1*
Following a similar approach to cite:hensmanGaussian2013,
...
(Eq. ref:eq-lower-bound-tight) can be further bounded to remove the $\NumInducing$ dimensional integrals.
...
*** Optimisation
The bound in Eq. ref:eq-lower-bound-further meets the necessary conditions to perform stochastic
gradient methods on $\expertsInducingVariational$ and $\gatingsInducingVariational$.
Firstly, it contains a sum of $\NumData$ terms corresponding to input-output pairs, enabling optimisation
with mini-batches.
Secondly, the expectation over the log-likelihood is calculated using Monte Carlo samples.
...
#+BEGIN_EXPORT latex
\newcommand{\predictivePosterior}{\ensuremath{q(\output_* \mid \input_*)}}
...
# , and finally, a data set collected from a simulator of a 2D point mass.
...
- test batch size?
- compare both bounds?
  + we know tighter bound should be better so
  + need to compare complexity of bounds? or speed to run?
- update mcycle table with correct NLML
...
- Finish writing up mcycle results
  - compare bounds
  - compare batch size
- add artificial dataset
  - copied from nerurips paper
...
The two variational lower bounds, $\mathcal{L}_{1}$ and $\mathcal{L}_2$, from Section ref:sec-inference,
are compared by training the model -- with the same model and training parameters -- for each lower bound.
MoSVGPE is compared against a Gaussian process (GP) and a sparse variational Gaussian
process (SVGP),
which use Squared Exponential (SE) kernels with automatic relevance determination (ARD) and
a Gaussian likelihood in all experiments.
...
With this in mind, the model and inference are evaluated by instantiating the model with both two $\ModeInd=2$
and three $\ModeInd=3$ experts.
The two lower bounds derived in Section ref:sec-inference are compared by training each instantiation of
the model -- with the same model and training parameters -- with each lower bound.
...
Motorcycle data set \citep{Silverman1985}, with $\mathcal{L}_1$ (left column) and with $\mathcal{L}_2$ (right column).
...
even though it has been trained with observations up to $x=2.5$.
...
The gating network serves as a probabilistic decision boundary that is dependent on both the mean and
the variance of the GP over $\gatingFunc$.
