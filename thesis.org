...
\newcommand{\expertPrior}{\ensuremath{p\left(\mode{f}(\{\singleInput : \singleModeVarK\}) \right)}}
...
#+END_EXPORT
** TODO :ignore:
- [ ] Check multiline Eqns only have one eq number
...
\E_{p\left(\mode{\latentFunc}(\allInputK) \mid \allInputK \right} \left[
\prod_{\numData=1}^{\NumData}
...
See cref:fig-graphical-model-comparison for a graphical model representation.
...
%   \resizebox{0.8\columnwidth}{!}{
    \begin{tikzpicture}[
      ...
      \node[const] (x) {$\singleInput^{\modeInd}$};

      \node[latent, left=of x, yshift=-1.4cm] (f) {$\mode{\latentFunc}(\allInput^{\modeInd})$};

      \node[const, left=of f, xshift=0.4cm] (thetak) {$\expertParamsK$};

      \node[latent, below=of x, xshift=0.0cm] (a) {$\modeVarn^{\modeInd}$};

      \node[const, right=of a, xshift=-0.4cm] (phik) {$\gatingParams$};

      \node[obs, below=of f, yshift=0.4cm] (y) {$\allOutput^{\modeInd}$};
...
are generated by mapping the inputs $\allInput = {\allInputK}$ through the latent process.
  An input assigned to expert $\modeInd$ is denoted $\singleInput^{\modeInd}$
  and the sets of all $\NumData_{\modeInd}$ inputs and outputs assigned to expert $\modeInd$ are denoted
  $\allInput^{\modeInd}=\allInputK$ and $\allOutput^{\modeInd}=\allOutputK$ respectively.
...
and sampling a an expert mode indicator variable $\singleModeVar$.
...
which involves evaluating the $\ModeInd$ latent gating functions $\GatingFunc$
...
\frac{1}{Z}
\prod_{\numData=1}^{\NumData}
 p(\modeVarn \mid \GatingFunc(\singleInput)) \right]}_{\text{GP gating network}}
...
Fig. [[ref:fig-graphical-model]] shows the graphical model where the $\ModeInd$ latent gating functions $\GatingFunc$
...
In contrast to the experts, partitioning of the data set is not desirable for the gating function GPs,
...
= \frac{\sum_{\allModeVar} \allGatingLikelihood \gatingPrior \allExpertLikelihood \expertPrior}{\evidence}
\end{align}
#+END_EXPORT
...
*** message :noexport:
Ello ello, totally thought I had replied, apologies!
So a little late now, but I am wondering if you are on any contraception?
...
\begin{figure}[t]
  ...
  The assignment of observations to experts is "softly" handled by the
...
\end{align}
#+END_EXPORT
where the set of all inducing inputs has been denoted $\expertsInducingInput$
and the  set of all inducing variables as $\expertsInducingOutput$.
Introducing separate inducing inputs from each expert's GP can loosely be seen as "partitioning"
the observations between experts, i.e. the inducing points can be seen as approximating the data partition as
$\expertInducingOutput \approx \allOutputK$ and $\expertInducingInput = \allInputK$.
...
set of $\NumInducing$ inducing variables from its corresponding GP prior,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-experts-inducing-prior}
...
Our approximation assumes that the marginalisation over every possible assignment
of data points to experts, can be factorised over data given the inducing points.
...
whilst still capturing the complex coupling between the gating network and experts.
...
This removes $\ModeInd$ of the undesirable approximate $M$ dimensional integrals from
...
The two lower bounds ($\furtherBound$ and $\furtherBoundTwo$),
...
# $(\allTestInput, \allTestOutput) = \{(\testSingleInput, \testSingleOutput)\}_{\numTest=1}^{\NumTest}$,
...
The data set is represented by the black crosses in cref:fig-y-mcycle-two-vs-three-experts.
...
\todo{An evaluation on the effect of the number of inducing points, batch size, and the different ELBO's is then
detailed,
...
**** Two Experts
The two further lower bounds are first compared by instantiating the model with two experts $\ModeInd=2$
and comparing their performance.
The results are show in cref:fig-y-mcycle-two-experts,fig-latent-mcycle-two-experts,
...
of data points $(\NumInducing \lll \NumData)$, to obtain improved computational performance.
...
The model was then trained using the model and training parameters shown in Table ref:tab-params-quadcopter.
