...
Given a dynamical system subject to two dynamics mode, the method in cref:chap-dynamics is used to
...
varying wind (and it's associated turbulence).

To begin with, $\NumData$ state transitions were sampled from the simulator.
...
the dynamics model cannot predict confidently.
...
However, this can easily be modified on an application basis.

In all experiments the trajectory optimiser was initialised with a horizon of $\TimeInd=10$.
