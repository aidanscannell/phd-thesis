...
\newcommand{\singleGatingPosterior}{\ensuremath{\Pr\left( \singleModeVarK \mid \singleInput, \gatingParamsK \right)}}
...
#+end_export
The marginal likelihood in Eq. ref:eq-marginal-likelihood is extremely expensive to evaluate $\mathcal{O}(K^2N^{4})$
...
\mathcal{X} &= \{ \state \in \R^\StateDim \mid \Pr\left[ \modeInd = k \right] \} \\
...
\begin{align} \label{eq-}
\traj, \utraj &= \\
\text{s.t.} \quad &\state_{t+1} = f(\state_t, \control_t) \\
&\{ \gatingFunc(\state_t, \control_t) > 0 \}
\end{align}
#+END_EXPORT
** Results
** Conclusion
