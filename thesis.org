...
\acrfull{soc} problem in cref:eq-mode-soc-problem, via a geometric objective function.
...
# The first method is an indirect control method that projects the trajectory optimisation onto the geodesic ODE
...
The methods encoded mode remaining behaviour via the latent geometry of the \acrshort{mosvgpe}'s gating network.
...
\todo{mention control as inference here?}
...
This chapter shares the problem statement in cref:sec-problem-statement
but it is briefly recapped in cref:sec-problem-statement-inference-traj-opt.
cref:sec-inference-background recaps the necessary background and related work.
cref:sec-traj-opt-inference then details the trajectory optimisation algorithm.
The method is evaluated in two simulated quadcopter environments in cref:chap-traj-opt-results.
...
** Problem Statement label:sec-problem-statement-inference-traj-opt
...
given a learned \acrshort{mosvgpe} dynamics model.
# That is, given a learned representation of the dynamics using \acrshort{mosvgpe}
# and a desired dynamics mode $\desiredMode$,
...
#   - in the desired dynamic mode,
#   - in the gating network.
More formally, this problem is given by,
#+BEGIN_EXPORT latex
\begin{subequations} \label{eq-mode-soc-problem-inf}
\begin{align}
\min_{\controlTraj} &\E\Bigg[
\sum_{\timeInd=0}^{\TimeInd} \costFunc(\state_\timeInd, \control_\timeInd) \Bigg] \\
\text{s.t.} \quad &\state_{\timeInd+1} = \dynamicsFunc(\state_\timeInd, \control_\timeInd) + \epsilon,
...
trajectory $\controlTraj = \control_{0:\TimeInd-1}$,
to navigate from $\state_{0}$, to  $\targetState$, over a horizon $\TimeInd$,
whilst minimising a cost function, $\costFunc: \stateDomain \times \controlDomain \rightarrow \R$
and keeping the system in the desired dynamics mode $\desiredMode$.
...
#+END_EXPORT
*** intro :ignore:
This chapter unleashes the power of the probability mass function over the expert indicator variable to encode
...
# The method boasts a closed-form expression for the \acrshort{elbo} that is obtained
...
* Quadcopter Experiments - Mode Remaining Control label:chap-traj-opt-results
** maths :ignore:
#+BEGIN_EXPORT latex
...
#+END_EXPORT
** intro :ignore:
The trajectory optimisation algorithms presented in
cref:chap-traj-opt-inference,chap-traj-opt-geometry are evaluated
...
show the results using the direct control method in cref:sec-traj-opt-energy and
...
**** Table :ignore:
#+begin_table
#+LATEX: \caption{\textbf{Results in simulated environments} Comparison of the  indirect control method from \cref{sec-traj-opt-collocation}, the direct geodesic control method from \cref{sec-traj-opt-energy} and the control-as-inference method from \cref{sec-traj-opt-inference}. All methods are evaluated in the two simulated environments. The performance measures are summed over the collocation points for the indirect geodesic control method and over each time step for the direct control method and the control-as-inference method. The "Mode prob" column calculates the probability at each time step without marginalising the state and gating function uncertainty whereas the "Mode prob with uncertainty" marginalises both the state and gating function uncertainty at each time step.}
#+LATEX: \label{tab-results-sim-envs}
#+LATEX: \resizebox{\textwidth}{!}{
#+ATTR_LATEX: :center t :placement [!b] :align clcccccc
|-----+---------------------------+-----------+---------------------------------+----------------------------------+--------------------------+------------------------------------------------------+--------------------------------------------------+---|
...
All of the direct control and control-as-inference experiments shown in
...
All of the direct control and control-as-inference experiments used a horizon of $\TimeInd=20$ time steps and
...
cref:tab-results-sim-envs summarises the direct control and the control-as-inference results in Environment 2.
...
indicate that all of the direct control and control-as-inference
...
Regardless, this is a big limitation that makes the indirect control method fail in some environments.

Overall, the direct control approach performed significantly better than the indirect control method in the experiments.
...
In conclusion, the direct control method is the preferred geometry-based method for finding mode remaining
trajectories.

*How to set $\lambda$?*
Although increasing $\lambda$ generally leads to more uncertainty avoiding behaviour in the gating network,
this is no not necessarily the case.
Further to this,
...
The experiments also showed that the maximum entropy control obtained from using Normally distributed controls
improves the mode remaining behaviour and works well in practice.
Both the direct control and the control-as-inference methods are competitive approaches for finding
mode remaining trajectories.

*** Discussion & Future Work
This section compares the three control methods presented in
cref:sec-traj-opt-collocation,chap-traj-opt-geometry,chap-traj-opt-inference
...
#+begin_table
#+LATEX: \caption{Comparison of the indirect control via latent geodesics method from \cref{sec-traj-opt-collocation}, the direct control via Riemannian energy method from \cref{sec-traj-opt-energy} and the control-as-inference method from \cref{chap-traj-opt-inference}.}
#+LATEX: \label{tab-geometry-control-comparison}
#+LATEX: \resizebox{\textwidth}{!}{
#+ATTR_LATEX: :center t :placement [!t] :align lccc
|                                                    | Control as Inference | Indirect Control | Direct Control |
|----------------------------------------------------+----------------------+------------------+----------------|
...
# Following the same approach as the direct control method,
...
The direct control and control-as-inference methods required control regularisation to prevent state transitions that
...
An interesting direction for future work is to extend the direct control and control-as-inference methods
...
the direct control method from cref:sec-traj-opt-energy and the control-as-inference method from
...
In contrast, the direct control and control-as-inference methods work well in practice,
...
From the experiments in this chapter, it can be concluded that both the direct control
...
# Overall, both the direct control and the control-as-inference methods performed well in the experiments.
...
* Mode Remaining Model-Based Reinforcement Learning label:chap-active-learning
#+begin_export latex
...
In contrast to previous chapters, it does not assume prior access to a historical data set of state transitions.
...
This chapter seeks to solve this navigation problem by combining all of the work presented thus far in this
dissertation.
...
This chapter presents some initial results and details future work.
...
$\{\mode{\latentFunc} : \stateDomain \times \controlDomain \rightarrow \stateDomain \}_{\modeInd=1}^\ModeInd$,
and associated noise models
$\mode{\bm\epsilon} &\sim \mathcal{N}\left(\mathbf{0}, \bm\Sigma_{\mode{\bm\epsilon}} \right)$, where
...
\begin{definition}[Mode Remaining] \label{def-mode-remaining-explore}
Let $\modeInd$ denote a dynamics mode, defined by its state domain $\mode{\stateDomain} \subseteq \stateDomain$.
Given an inital state $\state_0 \in \mode{\stateDomain}$, and a controller $\policy$,
the controlled system is said to be mode-remaining iff:
\begin{align} \label{eq-mode-remaining-def-explore}
\dynamicsFunc(\state_{\timeInd}, \policy(\state_{\timeInd}, \timeInd))
&\in \mode{\stateDomain} \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd]
\end{align}
\end{definition}
#+END_EXPORT
Given this definition of a mode remaining controlled system,
the problem can be formally stated as,
#+BEGIN_EXPORT latex
\begin{subequations} \label{eq-mode-explore-problem}
\begin{align}
\min_{\policy \in \Pi} &J(\policy) \\
\text{s.t.} \quad &\state_{\timeInd+1} = \dynamicsFunc(\state_\timeInd, \policy(\state_\timeInd, \timeInd)) + \epsilon,
\quad \epsilon \sim \mathcal{N}(\mathbf{0}, \Sigma_{\epsilon}) \\
%&\text{\cref{eq-mode-remaining-def-explore}} \\
&\dynamicsFunc(\state_{\timeInd}, \policy(\state_{\timeInd}, \timeInd))
\in \desiredStateDomain \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd-1] \\
%\modeVar_{\timeInd} = \desiredMode \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd] \\
%&\state_{\timeInd} \in \desiredStateDomain \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd] \\
&\state_\TimeInd = \targetState,
\end{align}
\end{subequations}
#+END_EXPORT
where $\policy : \stateDomain \times \mathbb{Z} \rightarrow \controlDomain$
denotes the controller.
It may take the form of a deterministic feedback policy or an open-loop set of controls without state feedback.

This resembles the problem statement from cref:chap-traj-opt-inference,chap-traj-opt-geometry.
However, this chapter does not assume /a priori/ access to the environment.
As such, it does not have access to a historical data set of state transitions which it can use to learn a
predictive dynamics model.
Instead, it must incrementally explore the environment, without violating the mode remaining constraints.
However, given that the underlying dynamics modes and how the system switches between them are /not fully known a priori/,
it is not possible to guarantee that the controlled system remains in the desired dynamics mode.

However, this requirement can be relaxed to keeping the system in the desired mode with high probability.
#+BEGIN_EXPORT latex
\begin{definition}[$\delta$-mode remaining] \label{def-delta-mode-remaining}
Let $\dynamicsFunc : \stateDomain \times \controlDomain \rightarrow \stateDomain$ denote a multimodal dynamical system
and $\desiredMode$ a desired dynamics mode defined by its state domain $\desiredStateDomain \subseteq \stateDomain$.
...
$\dataset_0 = \{(\state_n, \control_n), \Delta \state_n\}_{n=1}^{N_0}$ collected from around the start state.
This data set can be used to learn a predictive dynamics model which is accurate around the start state $\state_0$.
cref:fig-mode-opt-initial-gating-network shows the initial data set overlayed on the \acrshort{mosvgpe}'s
gating network posterior after training on it.
This model can be used to learn an initial controller which is asymptotically stable around
the start state.
However, this controller may not work outside of the initial state domain and will not keep the system
in the desired dynamics mode.
# Given this data set,

# such that $\state_0 \in \initialStateDomain$.
# $\initialStateDomain \subseteq \desiredStateDomain$ around the start state,
# such that $\state_0 \in \initialStateDomain$.
...
\begin{minipage}[r]{\columnwidth}
\centering
\includegraphics[width=\textwidth]{./images/mode-opt/scenario_7/explore/initial_mixing_prob.pdf}
\subcaption{Mixing probabilities}
\label{fig-mode-opt-over-prob}
\end{minipage}
\begin{minipage}[r]{\columnwidth}
\centering
\includegraphics[width=\textwidth]{./images/mode-opt/scenario_7/explore/initial_gating_gp.pdf}
\subcaption{Gating function's GP posterior}
\label{fig-mode-opt-over-svgp}
\end{minipage}
\caption{\mathbf{Initial gating network} Gating network posterior after training on the initial data set $initialDataset$
...
(\subref{fig-mode-opt-over-prob}) shows the probability mass function over the mode indicators variable and
(\subref{fig-mode-opt-over-svgp}) shows
the posterior mean (left) and posterior variance (right) associated with the desired mode's gating function.}
\label{fig-mode-opt-initial-gating-network}
\end{figure}
#+END_EXPORT
...
\begin{algorithmic}[1]
\Require{Explorative controller $\explorativeController$, mode remaining controller $\modeController$, dynamics model $\dynamicsModel}$, data set $\dataset$}
\For{$n = 1, 2, \ldots $}
    \State Train model $\dynamicsModel}$ on $\dataset$ using \acrshort{elbo} from \cref{eq-lower-bound-further}
    ...
    \State Collect state transition data set in environment using $\explorativeController$; add to $\dataset$
\EndFor
\end{algorithmic}
\end{algorithm}
#+END_EXPORT
...
This chapter proposes a method for solving the mode remaining navigation problem that combines all of
the work in this dissertation.
At is core, the method learns a single-step dynamics model $\dynamicsModel$ using the \acrshort{mosvgpe}
model from cref:chap-dynamics.
Given this model, the mode remaining control methods in cref:chap-traj-opt-geometry,chap-traj-opt-inference
...
Ideally, this exploration strategy will have some guarantee on remaining in the desired dynamics mode.
...
The performance of the algorithm depends on the controllers ability to explore.
This section provides an overview of existing exploration schemes.

*Greedy exploitation*
One of the most commonly used exploration strategies is to select the controller that maximises
the expected performance under the learned dynamics model.
This greedy strategy is given by,
#+BEGIN_EXPORT latex
\begin{align}
\pi_t^{\text{greedy}} = \arg \max_{\pi \in \Pi} \E_{f \sim p(f \mid \mathcal{D}_{1:t})} \left[ J(f, \pi) \right].
\end{align}
#+END_EXPORT
This approach is used in PILCO citep:deisenrothPILCO2011 and GP-MPC citep:kamtheDataEfficient2018
where the dynamics are represented using \acrshort{gp}s and the moment approximation is used to cascade
single-step predictions.
Similarly, PETS citep:chuaDeep2018 uses this exploration strategy but represents the dynamics
using ensembles of probabilistic neural networks.

*Thompson sampling*
An alternative and theoretically grounded strategy is Thompson sampling.
This approach samples a single model  $f \sim p(f \mid \mathcal{D}_{1:t})$ at every iteration
and uses the sampled model to optimise the controller.
This is given by,
#+BEGIN_EXPORT latex
\begin{align}
\pi_t^{\text{thompson}} = \arg \max_{\pi \in \Pi} J(f_t, \pi) \quad \text{s.t.} \quad f_t \sim p(f \mid \mathcal{D}_{1:t}).
\end{align}
#+END_EXPORT
# This contrasts the greedy approach that averages over the
In general it is intractable to sample from $p(f \mid \mathcal{D}_{1:t})$, although methods do exist
...
#+BEGIN_EXPORT latex
\begin{align}
\pi_t^{\text{active}} = \arg \max_{\pi \in \Pi} \matahbb{H} \left[ f \mid \mathcal{D}_{1:t} \right].
\end{align}
#+END_EXPORT
...
Their exploration strategy considers
the differential entropy of the posterior \acrshort{gp} associated with the dynamics model and they
use the \acrshort{gp} classifier to
...
The exploration strategy presented in this chapter is primarily interested in exploring
regions of the gating network that are uncertain.
This work proposes a trajectory optimisation algorithm that principally ensures $\delta\text{mode remaining}$
trajectories via chance constraints.
The objective combines information based exploration of the
gating network with the main goal of navigating to the target state $\targetState$.
The trajectory optimisation algorithm that this work solves is given by,
#+BEGIN_EXPORT latex
\begin{subequations} \label{eq-explorative-traj-opt}
\begin{align}
%\arg \max_{\controlTraj} &\sum_{\timeInd=0}^{\TimeInd}
\arg \max_{\pi} &\sum_{\timeInd=0}^{\TimeInd}
...
(\state_{\timeInd} - \targetState)}_{\text{state difference}} \right] \\
...
Conditioning the entropy on all of the other time steps makes each time step aware of the
information gained by rest of the trajectory.
...
\end{align}
#+END_EXPORT
where the mean and variance are obtained via the properties of multivariate Normal distributions,
#+BEGIN_EXPORT latex
\begin{align}
\desiredGatingMean &=
\desiredGatingKernel(\state_{\timeInd},\stateTraj_{\neg\timeInd})
\desiredGatingKernel(\stateTraj_{\neg\timeInd},\stateTraj_{\neg\timeInd})^{-1}
\desiredGatingFunction(\stateTraj_{\neg\timeInd}) \\
\desiredGatingVariance &=
\desiredGatingKernel(\state_{\timeInd}, \state_{\timeInd}) - \desiredGatingKernel(\state_{\timeInd},\stateTraj_{\neg\timeInd})
\desiredGatingKernel(\stateTraj_{\neg\timeInd},\stateTraj_{\neg\timeInd})^{-1}
\desiredGatingKernel(\stateTraj_{\neg\timeInd}, \state_{\timeInd})
\end{align}
#+END_EXPORT
where
$q(\GatingFunc(\state_{\timeInd}) \mid \state_{\timeInd}) = \mathcal{N}(\state_{\timeInd} \mid  \mu_{\timeInd}, \sigma^2_{\timeInd})$
is the approximate posterior over the
gating functions from cref:eq-predictive-gating.

Assuming that the gating function values associated with each state $\state_{\timeInd}$ in the trajectory
are from the gating function's GP, gives the distribution over a gating function value as,
#+BEGIN_EXPORT latex
\begin{align}
p(\GatingFunc(\state_{\timeInd}) \mid \state_{\timeInd}) =
\mathcal{N}(\mode{\hat{\mu}}(\state_{\timeInd}), (\state_{\timeInd}, \state_{\timeInd}))
\end{align}
#+END_EXPORT

#+BEGIN_EXPORT latex
\begin{align}
\arg \min_{\controlTraj} &\sum_{\timeInd=0}^{\TimeInd}
\mathcal{H} \left[ p(\desiredGatingFunction(\state_{\timeInd}) \mid \state_{0:\timeInd-1}, \dataset) ) \right] \\
\text{s.t.} \quad &\state_{\timeInd+1} = \dynamicsFunc(\state_\timeInd, \control_\timeInd) + \epsilon,
\quad \epsilon \sim \mathcal{N}(\mathbf{0}, \Sigma_{\epsilon}) \\
\modeVar_{\timeInd} &= \desiredMode \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd] \\
\state_{\TimeInd} &= \targetState,
...
#+BEGIN_EXPORT latex
\begin{align}
\arg \min_{\controlTraj \in \controlDomain} &\objectiveFunc \\
\text{s.t.}
&\Pr(\modeVar_{\timeInd} = \desiredMode \mid \state_{0}, \control_{0:\timeInd}, \bm\modeVar_{0:\timeInd-1}=\desiredMode)
\geq 1-\delta \quad \forall \timeInd \in \mathbb{Z} \cap [0,\TimeInd].
\end{align}
...
- Adding new inducing points
- Fixed gating kernel parameters
...
*** ModeOpt
The algorithm presented in this chapter is detailed in cref:alg-mode-opt.
The algorithm is initialised with a data set of state transitions, which are around the start state and
known to belong to the desired dynamics mode.

*\acrshort{mosvgpe} dynamics model $\dynamicsModel$*
At is core, the method learns a single-step dynamics model $\dynamicsModel$ using the \acrshort{mosvgpe}
model from cref:chap-dynamics.
This model learns a factorised representation of the underlying dynamics modes as well as a gating network
which infers valuable information regarding how the system switches between the modes.

*Mode remaining controller $\modeController$*
The mode remaining control methods in cref:chap-traj-opt-geometry,chap-traj-opt-inference
can be used to construct a mode remaining controller $\modeController$ that finds trajectories
to the target state $\targetState$.
The mode chance constraints from cref:eq-mode-chance-constraint
can then be used to check if these trajectories are $\delta-\text{mode remaining}$.

*Explorative controller $\explorativeController$*
# *$\delta-\text{mode remaining}$ exploration*
In order to implement such an algorithm, this chapter first proposes a method for exploring the environment
with mode remaining guarantees.
That is, am controller that will explore the environment whilst ensuring the controlled system remains in the
desired dynamics mode with high probability, i.e. satisfy cref:eq-mode-remaining-def-explore.
