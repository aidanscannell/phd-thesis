...
It is assumed that knowledge of how the transition dynamics switch between modes is also /not known a priori/.
In essence, the safety constraints are /not known a priori/ and should be .

and thus,
neither are the safety constraints.
...
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
\optimalProb &= \monotonicFunc( -\costFunc(\state_\timeInd, \control_\timeInd)).
\end{align}
#+END_EXPORT
