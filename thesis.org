...
This often leads to better NLPD scores as well.
...
this work formulated a gating network based on input-dependent gating functions with GP priors.
The GP-based gating network can be used to constrain the set of admissible functions through the placement of informative
...
every possible set of assignments to experts -- of which there are $\ModeInd^{\NumData}$ possibilities
...
These inducing points are used to factorise the joint distribution over every possible set of assignments to experts
across data and derive a variational lower bound also factorised across data.
The proposed lower bound provides a nice coupling between the
optimisation of the experts and the gating network by efficiently marginalising the expert indicator variable.
Our variational inference scheme principally handles uncertainty and
provides scalability as it can be optimised with stochastic gradient methods.

Three different EBLOs are derived and compared on the Motorcycle data set cite:Silverman1985.
