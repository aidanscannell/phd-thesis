...
Dynamical systems give rise to data arriving sequentially $\state_{1:\TimeInd} = \{\state_1, \ldots, \state_\TimeInd\}$.
...
inherently stochastic, due to lack of mathematical tools for model certain phenomena
e.g. turbulence.
...
state, $\state_g$,
in expectation over the
stochastic dynamics,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
\min_{\controlTraj} &\E\left[ \costFunc_\TimeInd(\state_\TimeInd)
+ \sum_{\timeInd=1}^{\TimeInd-1} \costFunc(\state_\timeInd, \control_\timeInd) \right] \\
...
#+END_EXPORT
*** Intro :ignore:
The trajectory optimisation algorithm presented here
...
For simplicity, this work sticks to the exponential transform.

\todo{cite cite:watsonStochastic2021}

*** Joint Probability Model :ignore:
\newline
The joint probability for an optimal trajectory, i.e.
