...
the method presented in this chapter is applicable with a wider range of \acrshort{mogpe} dynamics models.
...
p(\stateTraj, \controlTraj \mid \optimalVarTraj, \state_0) =
Z^{-1}
...
Note that $\optimalVarTraj_{0:\timeInd}$ denotes the first $\timeInd$ elements of $\optimalVarTraj$, i.e.
$\optimalVarTraj_{0:\timeInd} = \{ \optimalVar_i=1\}_{i=0}^{\timeInd}$.
...
q(\state_{\timeInd+1} \mid \state_0, \modeVarTraj)
\controlVarDist.
\end{align}
#+END_EXPORT
with $p(\state_0) = \delta(\state_0)$.
...
w.r.t. the variational lower bound, aka \acrfull{elbo}.
...
and the variational dynamics simplified to
$q(\state_{\timeInd})=q(\state_{\timeInd} \mid \state_0, \modeVarTraj_{0:\timeInd-1})$.
...
\newcommand{\modeProbTraj}{\ensuremath{\Pr(\allModeVarK \mid \stateTraj)}}
#+END_EXPORT
The control-as-inference method presented in this chapter is evaluated in the simulated environments from
cref:sec-traj-opt-results-energy.
The simulation environments represent a velocity controlled quadcopter subject to spatially
varying wind fields.
See cref:sec-traj-opt-results-energy for more details of the simulated environments
To aid with comparison against the methods presented in cref:chap-traj-opt-geometry,
the experiments leverage the same learned dynamics model as the experiments in cref:sec-traj-opt-results-energy.
That is, the model from cref:chap-dynamics was instantiated with $\ModeInd=2$ experts, one to
...
Sequential Least Squares Programming (SLSQP) in SciPy citep:2020SciPy-NMeth.

*** Performance Metrics :noexport:
...
*** Table :ignore:
#+begin_table
#+LATEX: \caption{\textbf{Results in simulated environments} Results for the control as inference method in the two simulated environments. The performance measures are summed over the collocation points for the indirect geodesic control method and over each time step for the direct control method. The "Mode prob" column calculates the probability at each time step without marginalising the state and gating function uncertainty whereas the "Mode prob with uncertainty" marginalises both the state and gating function uncertainty at each time step.}
#+LATEX: \label{tab-results-sim-envs-inf}
#+LATEX: \resizebox{\textwidth}{!}{
#+ATTR_LATEX: :center t :placement [!b] :align clcccccc
|-----+---------------------------+-----------+---------------------------------+----------------------------------+--------------------------+------------------------------------------------------+--------------------------------------------------+---|
| Env | Method                    | $\lambda$ | Mode prob                       | Mode prob with uncertainty       | State uncertainty        | Gating uncertainty                                   | Riemannian energy                                |   |
|     |                           |           | cref:eq-mode-probability-no-unc | cref:eq-mode-probability-all-unc | $\mathbb{V}[\stateTraj]$ | $\mathbb{V}[\gatingFunc_{\desiredMode}(\stateTraj)]$ | cref:eq-approximate-trajectory-riemannian-energy |   |
|-----+---------------------------+-----------+---------------------------------+----------------------------------+--------------------------+------------------------------------------------------+--------------------------------------------------+---|
|     | Inference                 | N/A       | $20.94$                         | $\mathbf{19.99}$                 | $8.74$                   | $\mathbf{208.62}$                                    | $287.52$                                         |   |
|     | Inference (deterministic) | N/A       | $\mathbf{20.96}$                | $19.58$                          | $\mathbf{8.73}$          | $258.27$                                             | $237.32$                                         |   |
|-----+---------------------------+-----------+---------------------------------+----------------------------------+--------------------------+------------------------------------------------------+--------------------------------------------------+---|
|     | Inference                 | N/A       | $\mathbf{20.98}$                | $\mathbf{20.72}$                 | $1.75$                   | $\mathbf{596.67}$                                    | $442.88$                                         |   |
|     | Inference (deterministic) | N/A       | $\mathbf{20.98}$                | $20.61$                          | $\mathbf{1.56}$          | $644.35$                                             | $534.39$                                         |   |
#+LATEX: }
#+end_table
...
*Multiple desired modes*
Although not tested here, the control-as-inference method presented in this chapter should be able to remain
in more than one desired dynamics modes.
This can be achieved by slightly altering the mode conditioning to condition on being in a set of modes.
...
the control-as-inference method is applicable in systems where
the
It would be interesting to extend this work to systems where the modes are governed by both state and control.
For example, flying at high speed through a wind field may be deemed an operable dynamics mode, whilst
flying at low speed may not.
...
Both the control-as-inference method presented in this chapter and the direct Riemannian energy approach presented in
cref:sec-traj-opt-energy find solutions that satisfy the dynamics constraints.

The mode chance constraints from cref:eq-mode-chance-constraint can be used with the control-as-inference
method to validate that trajectories are $\delta \text{-mode remaining}$.
...
*Dynamics models*
The method presented in this chapter can be deployed in a wider range of dynamics models than the geometry-based
methods.
...
variable.
In contrast, the geometry-based methods are limited to \acrshort{mosvgpe} because of it's GP-based
gating network.
...
*Maximum entropy control*
The maximum entropy control that arises from using Normally distributed controls improved the mode
remaining behaviour in all experiments.
