...
This will reduce the model's epistemic uncertainty.
...
#+BEGIN_EXPORT latex
\begin{align}
\pi_{\text{greedy}} = \arg \max_{\pi \in \Pi} \E_{f \in p(f \mid \mathcal{D}_{1:t})} \left[ J(f, \pi) \right].
\end{align}
#+END_EXPORT
...
#+BEGIN_EXPORT latex
\begin{align}
\pi_{\text{thompson}} = \arg \max_{\pi \in \Pi} J(f, \pi) \quad \text{s.t.} \quad f \sim p(f \mid \mathcal{D}_{1:t}).
\end{align}
#+END_EXPORT
...
#+BEGIN_EXPORT latex
\begin{align}
J(\stateTraj, \controlTraj) =
\underbrace{(\state_{\TimeInd} - \state_0)^T \mathbf{H} (\state_{\TimeInd} - \state_0)}_{\text{terminal cost}}
+ \underbrace{\sum_{\timeInd=1}^{\TimeInd-1} (\state_{\timeInd} - \targetState)^T \mathbf{Q} (\state_{\timeInd} - \targetState)}_{\text{state difference}}
+ \underbrace{\sum_{\timeInd=0}^{\TimeInd-1} \control_{\timeInd}^T \mathbf{R} \control_{\timeInd}}_{\text{control cost}}
\end{align}
#+END_EXPORT

#+BEGIN_EXPORT latex
\begin{align}
p(\desiredGatingFunction(\state_{\timeInd}) \mid \state_{0:\timeInd-1}, \dataset)
=
\end{align}
#+END_EXPORT
where
...
\mathcal{N}(\mode{\hat{\mu}}(\state_{\timeInd}), \mode{\hat{k}}(\state_{\timeInd}, \state_{\timeInd}))
\end{align}
#+END_EXPORT
