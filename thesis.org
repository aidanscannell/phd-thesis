...
\renewcommand{\marginalLikelihood}{\ensuremath{p(\optimalVar_{0:\TimeInd} = 1, \modeVar=\desiredMode \mid \state_0)}}
...
\end{align}
#+END_EXPORT
Further setting the variational dynamics equal to the learned dynamics,
${q(\state_{\timeInd+1} \mid \state_{\timeInd}, \control_\timeInd)=\transitionDist}$
...
&= \text{log} \E_{\trajectoryVarDist} \left[ \frac{\jointDist}{\trajectoryVarDist} \right] \\
&= \text{log} \E_{\trajectoryVarDist} \left[
\frac{\prod_{\timeInd=0}^{\TimeInd-1} \optimalProb \cancel{\transitionDist} \controlDist}{\prod_{\timeInd=0}^{\TimeInd-1} \cancel{\transitionDist} \controlVarDist}
\right] \\
&\geq \E_{\trajectoryVarDist} \left[
\sum_{\timeInd=0}^{\TimeInd-1} \text{log} \optimalProb - \sum_{\timeInd=0}^{\TimeInd-1} \text{log}
\frac{\controlVarDist}{\controlDist} \right] \\
&= \sum_{\timeInd=0}^{\TimeInd} \E_{\controlVarDist q(\state_{\timeInd})} \left[ \costFunc(\state_\timeInd, \control_\timeInd) \right]
...
\newcommand{\singleInputMean}{\ensuremath{\hat{\bm\mu_}}}
...
Given a deterministic state, $\state_\timeInd$, and a deterministic control, $\control_\timeInd$,
concatenated into a single state-control input, $\singleInput=(\state_\timeInd,\control_\timeInd)$,
the transition density, $\transitionDist$, can be calculated using the
desired mode's dynamics function,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
\state_{\timeInd+1} &= \mode{\latentFunc}(\state_\timeInd, \control_\timeInd) + \state_\timeInd +  \mode{\epsilon}.
\end{align}
#+END_EXPORT
Each mode's dynamics function is modelled using a sparse variational GP, which
captures the joint distribution of the data in the mode's corresponding
inducing variables $\expertInducingOutput$.
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
p(\Delta\state_{\timeInd+1} \mid \state_\timeInd, \control_\timeInd, \modeVarK) &=
\E_{\expertVariational} \left[\singleExpertLikelihood \right] \\
\expertVariational &=  \E_{ \expertInducingVariational} \left[  \singleLatentExpertGivenInducing \right] \\
\expertInducingVariational &= \mathcal{N}\left( \expertInducingOutput \mid \mode{\mathbf{m}}, \mode{\mathbf{S}} \right)
\end{align}
#+END_EXPORT
The functional form of \expertsVariational is given by,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
\expertsVariational &= \prod_{\modeInd=1}^\ModeInd \mathcal{N} \left( \mode{\latentFunc}(\singleInput) \mid
\mode{\mathbf{A}} \mode{\mathbf{m}},
...
Given a Gaussian distributed state,
${\state_{\timeInd} \sim \mathcal{N}(\stateMean, \stateCov)}$,
and control,
${\control_{\timeInd} \sim \mathcal{N}(\controlMean, \controlCov)}$,
the distribution over the successor state

As both the state and control at a given time step are Gaussian distributed,
${\state_{\timeInd} \sim \mathcal{N}(\stateMean, \stateCov)}$,
and control,
${\control_{\timeInd} \sim \mathcal{N}(\controlMean, \controlCov)}$,
...
Consider the problem of predicting the desired modes dynamics function value, $\desiredDynamicsFunc(\singleInput)$,
given a Normally distributed input,
${\singleInput \sim \mathcal{N}(\singleInputMean, \singleInputCov)}$,
where $\desiredDynamicsFunc \sim \mathcal{GP}$.
This prediction problem corresponds to calculating the following integral,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
p(\desiredDynamicsFunc(\singleInput) ) = \int  \underbrace{p(\desiredDynamicsFunc(\singleInput) \mid \singleInput)}_{\text{GP predictive dist}}
p(\singleInput) \text{d}\singleInput.
\end{align}
#+END_EXPORT
The mean and variance of the GP predictive distribution $p(\desiredDynamicsFunc(\singleInput) \mid \singleInput)}$
