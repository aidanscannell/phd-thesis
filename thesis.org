...
A real-world state transition data set was collected by flying a quadcopter aroudnd the lab with constant controls.
It was used to train the \acrshort{mosvgpe} model in \cref{sec-brl-experiment}
...
However, this constant controls data set cannot be used to learn a dyanmics model for the control methods
in \cref{chap-traj-opt-inference,sec-traj-opt-energy} (due to the lack of controls).
It was also not possible to collect a new data set, due to lab closures.
