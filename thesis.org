...
This work derives novel \acrfull(elbo)s based on sparse \acrshort{gp} approximations, that provide
well-calibrated uncertainty estimates and scalability via stochastic gradient methods.
The method presented in this chapter is named \acrfull{mosvgpe}
...
$\{\unknownDynamicsK : \stateDomain \times \controlDomain \rightarrow \stateDomain \}_{\modeInd=1}^\ModeInd$ and associated noise models,
$\mode{\bm\epsilon} &\sim \mathcal{N}\left(\mathbf{0}, \bm\Sigma_{\mode{\bm\epsilon}} \right)$, where
$\bm\Sigma_{\mode{\epsilon}} = \text{diag}\left[ \sigma_{\modeInd,1}^{2}, \ldots, \sigma_{\modeInd,\StateDim}^2 \right]$,
are selected by a discrete mode indicator variable,
...
obvious how the notion of mode remaining/avoiding can be encoded into control algorithms.
#+BEGIN_EXPORT latex
\begin{remark}
...
Without such information, it is not obvious how to encode the notion of mode remaining/avoiding behaviour.
...
2. has a convenient /latent space/ for planning/control,
...
**** Gating Networks and Identifiability :noexport:
...
This work adopts a GP-based gating network resembling a GP classification model, similar to that used in the original
\acrshort{mogpe} model citep:trespMixtures2000a. Our marginal likelihood is given by,
#+BEGIN_EXPORT latex
...
*Latent spaces for control* With regards to constructing convenient /latent spaces/ for control,
formulating the gating network GPs
with differentiable mean and covariance functions, enables techniques from Riemannian geometry
to be deployed on the gating functions citep:carmoRiemannian1992.
