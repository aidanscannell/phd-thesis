...
In particular, it walks through the lower bounds (used to perform inference) that are the building blocks
...
\marginnote{covariance and mean function}
...
\marginnote{Gaussian process prior}
...
\marginnote{noise-free predictions}
...
p(\mathbf{f}, \Fu) &= p(\mathbf{f} \mid \Fu) p(\Fu) \\
...
p(\mathbf{y}, \mathbf{f}, \Fu) &=  p(\mathbf{y} \mid \mathbf{f}) p(\mathbf{f} \mid \Fu) p(\Fu)
\end{align}
#+END_EXPORT
...
\text{log}p(\mathbf{y}\mid\Fu) \geq \E_{p(\F \mid \Fu)} \left[ \text{log} p(\mathbf{y} \mid \F) \right].
\end{align}
#+END_EXPORT
This bound is then substituted into the marginal likelihood
$p(\mathbf{y}) = \int p(\mathbf{y} \mid \Fu) p(\Fu) \text{d}\Fu$
to obtain a tractable lower bound cite:titsiasVariational2009,
\marginnote{Titsias' bound}
#+BEGIN_EXPORT latex
\begin{align} \label{eq-titsias-bound}
\text{log}p(\mathbf{y}) \geq \text{log}\mathcal{N}(\mathbf{y} \mid \mathbf{0}, \mathbf{Q}_{nn} + \sigma^{2} \mathbf{I})
- \frac{1}{2\sigma^2}\text{tr}(\K_{nn} - \mathbf{Q}_{nn}),
\end{align}
#+END_EXPORT
...
with a set of $M$ inducing points for each GP.
However, instead of collapsing these inducing points they are explicitly
represented as a variational distribution, as seen in cite:hensmanGaussian2013.
...
#+BEGIN_EXPORT latex
\begin{align*} \label{eq-variational-dist}
\text{log} \pYGivenU \geq \E_{\pFkGivenUk} \left[ \text{log} \sum_{k=1}^{K} \PrA \pYkGivenFk \right]
\end{align*}
#+END_EXPORT

Let us first consider a bound on each expert's conditional given its inducing points,
#+BEGIN_EXPORT latex
