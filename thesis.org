...
%In this scenario the dynamics modes vary spatially (over the state space).
...
This work first considers learning representations of multimodal dynamical systems, assuming
...
high \textit{epistemic uncertainty}, as well as remining in the desired dynamics mode.
...
into a geometric objective function.
...
Finally, this dissertaion considers the active learning setting, where it does
...
are believed to be /unoperatable/ or /unsafe/.
...
Perhaps the other modes are hard to control due to stability, or the mode switch itself is hard to control.
Or perhaps it is desirable to remain in a desired mode for /efficiency/ or /performance/ reasons.
...
This is because it cannot predict the trajectory confidently, and thus, if any safety
constraints have been violated.
In our case, it does not know if the trajectory will remain in the desired dynamics mode.
...
The goal is to find trajectories between $\mathbf{x}_0$ and $\mathbf{x}_f$ that either prioritise
...
\end{figure}
#+END_EXPORT
This dissertation is interested in controlling a DJI Tello quadcopter with partially unknown dynamics,
in an environment
...
This Chapter introduces two different approaches to performing trajectory optimisation that achieve Goals 1 and 2.

** Trajectory Optimisation via Latent ODE Collocation
The trajectory optimisation algorithm presented in this Section exploits the latent /geometry/ inferred by the
Gaussian process based gating network in the dynamics model presented in Chapter ref:chap-dynamics.
...
ODE (the geodesic ODE) whose solutions implicitly minimise the objective function.
...
**** A Geometric Cost Function :ignore:
*Geometric Cost Function* The geometry of the latent gating functions can also be used to formulate a cost term
...
** Trajectory Optimisation as Probabilistic Inference :noexport:
#+BEGIN_EXPORT latex
...
* Trajectory Optimisation in Desired Modes as Probabilistic Inference label:chap-active-learning
** Intro :ignore:
#+BEGIN_EXPORT latex
...
** Results
** Conclusion
* Active Learning in Multimodal Dynamical Systems
** Intro :ignore:
#+BEGIN_EXPORT latex
\newcommand{\timeInd}{\ensuremath{t}}
\newcommand{\TimeInd}{\ensuremath{\MakeUppercase{\timeInd}}}
...
\newcommand{\inputDomain}{\ensuremath{\hat{\mathcal{X}}}}

\renewcommand{\dataset}{\ensuremath{\mathcal{D}}}
\renewcommand{\params}{\ensuremath{\bm\theta}}
\renewcommand{\input}{\ensuremath{\control}}
...
data point environment belongs to either the safe or unsafe regions.
...
and the outputs $\output \in \outputDomain$.
...
\end{align}
#+END_EXPORT
However, in general, solving this problem is NP-hard,
so it is common to approximate it with a myopic greedy approximation.
cite:krauseNearOptimal2008 show that a myopic policy can perform near-optimally.
...
Intuitively, the first term $H(\output \mid \input, \dataset)$) seeks to
...
cite:houlsbyBayesian2011 formulate Bayesian active learning for Gaussian process
binary classification with a Bernoulli likelihood.
Their approach approximates Eq. ref:eq-output-space-entropy in the context of
GP classification.
...
The exploration strategy introduced here is a selective
