...
This chapter introduces two different approaches to performing /mode constrained/ trajectory optimisation.
Both approaches are model-based techniques that leverage the dynamics model from cref:chap-dynamics.
The first method exploits concepts from Riemannian geometry to obtain mode remaining behaviour.
The second approach extends the control as inference framework to achieve mode remaining behaviour
via conditioning on the mode indicator variable.

cref:sec-problem-statement formally states the problem and then cref:sec-traj-opt-geometric,sec-traj-opt-inference
...
\begin{subequations} \label{eq-mode-soc-problem}
\begin{align}
\min_{\controlTraj} &\E\left[
\sum_{\timeInd=0}^{\TimeInd} \costFunc(\state_\timeInd, \control_\timeInd) \right] \\
\text{s.t.} \quad &\state_{\timeInd+1} = \desiredDynamicsFunc(\state_\timeInd, \control_\timeInd) + \epsilon_{\desiredMode},
%\text{s.t.} \quad &\state_{\timeInd+1} = desiredDynamicsFunc\mode{\latentFunc}(\state_\timeInd, \control_\timeInd) + \epsilon_\modeInd,
\quad \epsilon_{\desiedMode} \sim \mathcal{N}(\mathbf{0}, \Sigma_{\epsilon_{\desiredMode}})
\end{align}
\end{subequations}
#+END_EXPORT
...
Although the turbulent dynamics are not known, they are believed to be difficult to control due to high process
...
# Given a dynamical system subject to two dynamics modes, the method in cref:chap-dynamics is used to
# learn a factorised representation of the underlying dynamics modes.
# The goal of this chapter is then to leverage the latent structure of this model, to
# find trajectories from a start state $\state_0$, to a target state $\targetState$,
# whilst avoiding both the turbulent dynamics mode and regions of the dynamics that cannot be predicted confidently,
# i.e. have high /epistemic uncertainty/.
...
\centering
\includegraphics[width=1.0\textwidth]{./images/traj-opt/mode-remaining/baseline-svgp-traj-opt/trajectories_over_desired_gating_mask.pdf}
...
\end{figure}
#+END_EXPORT
cref:fig-traj-opt-over-gating-mask-svgp-baseline demonstrates the shortcomings of simply
learning a transition dynamics model using a standard SVGP.
...
This demonstrates some of the difficulties of finding trajectories
from the start state $\state_0$, to the target state $\targetState$,
without avoiding the turbulent dynamics mode.
It is worth noting here that the underlying dynamics modes and how the system switches between them,
are /not fully known a priori/.
However, the MoSVGPE method from cref:chap-dynamics can be used to learn a factorised representation of
the underlying dynamics modes.
The model also infers informative latent structure that can be used for control.
...
\centering
\includegraphics[width=1.0\textwidth]{./images/traj-opt/mode-remaining/baseline-traj-opt/trajectories_over_desired_prob.pdf}
...
(\subref{eq-traj-opt-over-prob-baseline}) shows the trajectories overlayed on each mode's mixing probability
...
# In this example the dyn


and regions of the dynamics that cannot be predicted confidently,
i.e. have high /epistemic uncertainty/.
For example, because they have not been observed.

It is clear from cref:fig-traj-opt-over-gating-mask-svgp-desired-baseline
that dynamics mode one $\modeInd=1$ is subject to higher drift in the
negative $y$ direction. This is due to the wind field created by the fan.
It is also subject to higher diffusion (or higher process noise) resulting from the turbulence induced by the fan,
however, this is not visualised in cref:fig-traj-opt-baseline.
In such an environment, it is desirable to find trajectories that avoid entering such regions.
The main goals of the trajectory optimisation in this Chapter can be summarised as,
- Goal 1 :: remain in a desired dynamics mode $\desiredMode$ where possible, label:to-goal-mode
- Goal 2 :: avoid regions of the learned dynamics with high /epistemic uncertainty/, i.e. that cannot be predicted confidently (due to limited training observations). label:to-goal-unc
...
# The state trajectories are overlayed on the gating mask, indicating where each mode governs the dynamics.
# White indicates the turbulent dynamics mode and red indicates the operatable mode.
# The hashed box indicates a subset of the environment that has not been observed.}
# \end{figure}
# #+END_EXPORT
...
$\manifoldInducingOutput = \manifoldFunction(\manifoldInducingInput) \in \R^{\NumInducing \times 1}$,
...
** Indirect Control via Latent Geodesics label:sec-traj-opt-geometric
...
*** Discussion
This method provides an elegant solution to finding trajectories that satisfy Goals 1 and 2.
...
As such, an interesting direction for future work is to consider different methods for
incorporating the dynamics constraints.
The simplest approach would be to rollout the dynamics, ignoring uncertainty propagation.
A more elegant approach may consider the full stochastic dynamics by using stochastic collocation methods
based on generalised polynomial chaos.

*Stochastic Dynamics*
Further to this, this algorithm ignores much of the stochasticity inherent in the problem.
In particular, it does not consider the full distribution over state-control trajectories,
as it assumes point estimates for the state and control at each time step.
Without the inclusion of our probabilistic dynamics model, it is impossible
to consider the full distribution over state-control trajectories.
Uncertainty propagation through a single dynamics mode is straight forward and future work could implement this
alongside the collocation constraints, ensuring trajectories are solutions to both the geodesic ODE and the dynamics.
However, including the state distribution in the collocation constraints is not easy, as the geodesic ODE
will become a Stochastic Differential Equation (SDE).
The trajectory optimisation algorithm presented in the next chapter addresses this issue by
minimising the Riemannian energy in cref:eq-trajectory-energy, instead of minimising the Riemannian length.
Importantly, cref:eq-trajectory-energy retains a quadratic form (ignoring the metrics dependence on state)
and calculating expected quadratic costs subject to Normally distributed states and controls is trivial.
# However, it is interesting to consider what would be gained by considering the full distribution.
# The expected Riemannian metric encodes the notion of avoiding regions of the dynamics with high
# epistemic uncertainty, whilst its mode remaining behaviour keeps the dynamics in regions with low process noise.
# However, this assumption is common even when consider GP based dynamics models cite:buisson-fenetActively2020
# \todo{add more references about only using GP mean function.}

*Stochastic Geodesics*
In reality, the geodesic ODE in cref:eq-2ode is an SDE, because
both the state and Riemannian metric are random variables. Our method exploits the fact that
the expected Riemannian metric encodes both of our goals, so that the SDE can be simplified to a deterministic ODE.
It is much easier to solve the deterministic ODE and in practice it achieves both of our goals.
However, it would be interesting to consider the full SDE in future work.
Perhaps stochastic collocation methods could be deployed on both the stochastic dynamics and the geodesic SDE.

*Differential Flatness*
Perhaps the biggest limitation of this method, is its dependence on exploiting
differential flatness to obtain the controls from the state trajectory and geodesic ODE.
...
In practice, we assume that the control dimensions are independent and $\controlCov$ becomes diagonal.
...
**** Latent Geometry
Given this control as inference framework, it is interesting to consider how the
gating network's latent geometry can be exploited.
It turns out that length minimising trajectories on a Riemannian manifold $\manifold$,
are also energy minimising trajectories
citep:carmoRiemannian1992.
...
\end{align}
#+END_EXPORT
Interestingly, this retains a quadratic form, of sorts.
However, the expected energy for a trajectory, given by,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-}
\E_{\stateTraj, \controlTraj, \jacTraj} \left[ \costFunc(\stateTraj, \controlTraj) \right]
= \sum_{\timeInd=0}^\TimeInd
\E_{\stateDist}\left[ \stateDiff_\timeInd^T
\E_{\Jac \mid \state_{\timeInd}}\left[ \Jac(\state_\timeInd) \Jac(\state_\timeInd)^T \right]
 \stateDiff_\timeInd \right]
...
A simple approach is to assume that the expected metric $\E[\metricTensor]$
...
#+END_EXPORT
The control as inference algorithm presented in this section was evaluated in a simulated environment.
The simulation environment models a velocity controlled quadcopter in an environment subject to spatially
...
This enabled the trajectory optimisation's performance at achieving Goal 2 to be evaluated.
...
In all experiments a $\TimeInd=20$ step horizon was used and the controls were optimised using
...
*Riemannian Energy*
- compare different covariance weights
...
\end{minipage}
\caption{(\subref{fig-mode-conditioning-no-max-entropy-traj-opt-over-prob}) shows the trajectories overlayed on the desired mode's mixing probability
and (\subref{fig-mode-conditioning-no-max-entropy-traj-opt-over-svgp}) shows them overlayed on the desired mode's gating function posterior
mean (left) and posterior variance (right).}
...
\end{minipage}
\caption{(\subref{fig-mode-conditioning-max-entropy-traj-opt-over-prob}) shows the trajectories overlayed on the desired mode's mixing probability
and (\subref{fig-mode-conditioning-max-entropy-traj-opt-over-svgp}) shows them overlayed on the desired mode's gating function posterior
mean (left) and posterior variance (right).}
...
\begin{minipage}[r]{\columnwidth}
\centering
\includegraphics[width=\textwidth]{./images/traj-opt/mode-remaining/conditioning-energy-1-no-max-entropy/trajectories_over_desired_prob.pdf}
\subcaption{}
\label{eq-geometric-traj-opt-over-prob}
\end{minipage}
\begin{minipage}[r]{0.99\columnwidth}
\centering
\includegraphics[width=\textwidth]{./images/traj-opt/mode-remaining/conditioning-energy-1-no-max-entropy/trajectories_over_desired_gating_gp.pdf}
\subcaption{}
