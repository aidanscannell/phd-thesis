...
making it a hard parameter to set.
Especially in this environment, where the inoperable dynamics mode intersects a region of high epistemic
uncertainty.
...
Environment 1.
cref:fig-dataset-scenario-5 show the environment, data set and navigation problem.
...
This was due to the collocation solver failing with $\lambda=0.01$.
...
This is because it is a property of the collocation solver.
However, none of the experiments were able to reach the target state under the desired mode's dynamics, indicated
by none of the magenta trajectories reaching the target state.
This is because the variational inference strategy was not able to recover the controls from the collocation solver's
state trajectory.
It is likely that the ELBO was not able to recover the controls due to a local optima.
...
in this region.
As a result, the ELBO is low when trajectories pass through this region.
Further to this, the collocation state trajectory (yellow) for the experiment
in cref:fig-geodesic-mid-point-traj-opt-5
does not pass through the turbulent dynamics mode and it is still not able to recover the correct controls.
Observing the control trajectory during optimisation showed that this is due to the control trajectory
hitting the local optima attributed to the turbulent dynamics mode during optimisation.
...
cref:fig-geodesic-traj-opt-5,fig-geodesic-high-traj-opt-5,fig-geodesic-low-traj-opt-5,
which were initialised with straight line trajectories, the collocation solver was not able to escape local optima.
...
In these experiments, the influence of the gating function mean has a higher impact on
...
Because the indirect control experiments failed to remain in the desired dynamics mode and to recover
controls to follow their collocation state trajectory (yellow), they are not considered here.
...
3. The collocation solver is susceptible to local optima around its initial state trajectory.
...
4. The variational inference strategy used to recover controls for the indirect control method cannot always
   recover controls corresponding to the same state trajectory in the desired mode's GP dynamics,
   - because the collocation solver's state trajectory does not satisfy the dynamics constraints, or,
   - due to the optimisation getting stuck in a local optima.
