...
#+BEGIN_EXPORT latex
\begin{SingleSpace}
In order to mitigate risk due to Covid-19 lab closures, many of the methods in this dissertaion were validated in
simulation, as opposed to in real-world experiments.
...
This data set was used to train the \acrshort{mosvgpe} model in \cref{sec-brl-experiment}
and to train a \acrshort{mosvgpe} model to test the mode remaining trajectory optimisation algorithm presented in
\cref{sec-traj-opt-collocation}.
These results are a step towards validating that the methods presented in this dissertaion work on real-world systems.
However, this constant controls data set could not be used to learn a dyanmics model for the control methods
in \cref{chap-traj-opt-inference,sec-traj-opt-energy} (due to the lack of controls).
Due to Covid-19 lab closures, it was also not possible to collect a new real-world data set that could be used
to train the dynamics model (with controls).
Instead, we decided to use a simulator to generate a data set.
This had the added benefit that the simulator could be used to test the mode remaining trajectory optimisation
algorithms in \cref{chap-traj-opt-inference,sec-traj-opt-energy},
as well as the explorative trajectory optimisation algorithm in \cref{chap-active-learning},
...
%However, this real-world data set is not applicable for learning the dyanmics model needed for the control methods
...
Thank you for taking me on as a student and your continued support.
Your expertise and willigness to explore new ideas has made my PhD extrememly enjoyable.

I am profoundly grateful to Arthur Richards for believing in me,
It is thanks to you I am

giving me the opportunity to explore.

sharing many interesting discussions and their continued support

I'd also like to thank Carl for his countelss tips with bikes and Emacs.
I especially enjoyed wathcing your transition to the Emacs X Window Manager.
I hope that one day I can live in a fully

I would also like to thank the Reddit users that supported me through my journey with RSI.
It seems like a life time ago since I was consiering giving up my PhD and all hopes of a career in programming.
Your support gave me the confidence that I needed to overcome my RSI. I am eteranlly grateful.

I am grateful to my friends, especially everyone in VCH. You have made my time in Bristol so much fun!
I also want to thank my family for be amazing and always being their to support me.
I am especially grateful to my mum for th giving me

# Thank you for the endless proof reading

%have made so many amazing memories and I no
%it will always put a smile on my face.

# Without you I am not sure I would have maintained a healthy work life
...
I acknowledge the financial support toward this PhD from EPSRC
...
# -- to the target state $\targetState$, whilst guaranteeing that the controlled
# system remains in the desired dynamics mode.
All of the methods in this dissertation relate back to this mode remaining navigation problem.
For this reason, the problem is formally defined here.

This dissertation considers nonlinear, stochastic, multimodal dynamical systems, given by,
#+BEGIN_EXPORT latex
...
- cref:chap-traj-opt-control: investigates model-based control techniques that leverage the probabilistic model
  ...
  and used to train the model. It presents three trajectory optimisation algorithms that
...
Following other \acrshort{mogpe} methods, \acrshort{mosvgpe} is evaluated on the motorcyle data set citep:Silverman1985.
...
** Problem Statement
This work considers learning representations of /unknown/, or /partially unknown/,  multimodal,
stochastic, nonlinear dynamical systems.
That is, it seeks to learn the dynamics from the problem statement in cref:problem-statement-main.
This chapter modifies the dynamics from cref:eq-dynamics-main to consider mapping
a state and control at a given time step to the state difference at the next time step, as opposed to the next state.
...
\begin{align}
\Delta \state_{\timeInd+1} &= \dynamicsFunc(\state_\timeInd, \control_\timeInd; \Delta t = t_*) + \bm\epsilon \\
&= \mode{\dynamicsFunc}(\state_\timeInd, \control_\timeInd; \Delta t = t_*) + \mode{\bm\epsilon}
\quad \text{if} \quad \modeVar(\state_{\timeInd})=\modeInd \nonumber \\
\mode{\bm\epsilon} &\sim \mathcal{N}(\mathbf{0}, \Sigma_{\mode{\bm\epsilon}}),
\end{align}
\end{subequations}
...
resembles an indirect optimal contro method as it solves the necessary conditions which /indirectly/ represent the
...
An initial version of the indirect optimal control via latent geodesics method presented in cref:sec-traj-opt-collocation
...
However, this chapter only considers mode remaining trajectory optimisation and does not consider
...
This chapter further assumes prior access to the environment, such that a data set of state transitions has previously
...
cref:sec-prob-geo then extends these concepts to probabilistic geometries in order to encode Goal 3.
...
and to 2) avoid entering regions where it is uncertain which mode governs the dynamics, i.e. Goal 3.
...
\end{align}
#+END_EXPORT
is equivalent to the \acrfull{soc} problem, with a modified integral cost,
#+BEGIN_EXPORT latex
...
cref:chap-traj-opt-control were evaluated on a simulated version
of the illustrative example from cref:illustrative_example, i.e.
...
The methods were further evaluated on a second simulated velocity controlled quadcopter navigation problem.
...
They are also subject to higher diffusion (aka process noise) resulting from the turbulence induced by the fan.
...
The collocation solver from the indirect optimal control via latent geodesics method in cref:chap-traj-opt-geometry
was tested on a real-world quadcopter state transition data set.
Note that the data set was collected with constant controls so the controls could not be recovered from
the full \acrshort{mosvgpe} dynamics model, nor could the
direct optimal control method from cref:chap-traj-opt-geometry or the control-as-inference method from
...
All three control methods were then tested in the two simulated environments so that they could be compared.
...
large can omit any mode remaining behaviour.
...
\label{fig-dataset-scenario-7}
\end{minipage}
\caption{\textbf{Simulated Environments} - Visualisation of the simulated environments with their true mode boundaries
...
3. Goal 3.1 was measured using the state variance accumulated from cascading single-step predictions,
   #+BEGIN_EXPORT latex
...
4. Goal 3.2 was measured using the gating function variance accumulated from cascading single-step predictions,
   #+BEGIN_EXPORT latex
...
in the desired dynamics mode (Goal 3.1), is measured by summing the state variance cref:eq-metric-state-var
...
is measured by summing the gating function variance cref:eq-metric-gating-var over each trajectory.
...
*** Table :ignore:
#+begin_table
#+LATEX: \caption{\textbf{Results in simulated environments} Comparison of the  indirect optimal control via latent geodesics method (\ig) from \cref{sec-traj-opt-collocation}, the direct optimal control via Riemannain energy method (\dre) from \cref{sec-traj-opt-energy} and the control-as-inference method from \cref{sec-traj-opt-inference}, with max entropy (\cai gauss) and without max entropy (\cai dirac). All methods are evaluated in the two simulated environments. The performance measures are summed over the collocation points for the indirect optimal control method and over each time step for the direct optimal control method and the control-as-inference method. The "Mode prob" column calculates the probability of being in the desired dynamics mode at each time step without marginalising the state and gating function uncertainty over the trajectory. In contrast the "Mode prob with unc." marginalises both the state and gating function uncertainty at each time step. The target state column indicates if the trajectory reached the target state in the environment ($\checkmark$ both), only under the desired mode's dynamics \acrshort{gp} ($\checkmark$ dynamics) or not at all ($\times$).}
#+LATEX: \label{tab-results-sim-envs}
#+LATEX: \resizebox{\textwidth}{!}{
#+ATTR_LATEX: :center t :placement [!b] :align clccccccc
|-----+-----------------+-----------+------------------------------------+-------------------------------------+--------------------------+------------------------------------------------------+-----------------------------------------------------+------------------------------------|
| Env | Method          | $\lambda$ | Mode prob                          | Mode prob with unc.                 | State unc.               | Gating unc.                                          | Riemannian energy                                   | Target state                       |
|     |                 |           | Eq. ref:eq-mode-probability-no-unc | Eq. ref:eq-mode-probability-all-unc | $\mathbb{V}[\stateTraj]$ | $\mathbb{V}[\gatingFunc_{\desiredMode}(\stateTraj)]$ | Eq. ref:eq-approximate-trajectory-riemannian-energy | $\state_{\TimeInd}==\targetState$? |
|-----+-----------------+-----------+------------------------------------+-------------------------------------+--------------------------+------------------------------------------------------+-----------------------------------------------------+------------------------------------|
|     | \ig             | $0.5$     | $16.96$                            | $16.22$                             | $8.69$                   | $346.62$                                             | $238.29$                                            | $\times$ dynamics              |
|     | \ig             | $1.0$     | $18.51$                            | $16.98$                             | $8.70$                   | $367.16$                                             | $222.07$                                            | $\times$ dynamics              |
|     | \ig             | $20.0$    | $15.32$                            | $14.62$                             | $\mathbf{8.66}$          | $201.45$                                             | $627.74$                                            | $\times$ dynamics              |
|   1 | \dre            | $0.5$     | $20.91$                            | $16.77$                             | $8.70$                   | $404.51$                                             | $180.10$                                            | $\checkmark$ both                  |
|     | \dre            | $1.0$     | $\mathbf{20.98}$                   | $18.10$                             | $8.72$                   | $388.51$                                             | $\mathbf{173.84}$                                   | $\checkmark$ both                  |
|     | \dre            | $20.0$    | $12.64$                            | $13.00$                             | $8.69$                   | $\mathbf{157.83}$                                    | $349.08$                                            | $\times$ dynamics              |
|     | \cai (gauss)    | N/A       | $20.94$                            | $\mathbf{19.99}$                    | $8.74$                   | $208.62$                                             | $287.52$                                            | $\checkmark$ both                  |
|     | \cai (dirac)    | N/A       | $20.96$                            | $19.58$                             | $8.73$                   | $258.27$                                             | $237.32$                                            | $\checkmark$ both                  |
|-----+-----------------+-----------+------------------------------------+-------------------------------------+--------------------------+------------------------------------------------------+-----------------------------------------------------+------------------------------------|
|     | \ig             | $0.5$     | $18.98$                            | $19.22$                             | $1.53$                   | $495.11$                                             | $249.85$                                            | $\times$                           |
|     | \ig             | $1.0$     | $18.98$                            | $19.16$                             | $1.39$                   | $522.60$                                             | $285.92$                                            | $\times$                           |
|     | \ig (mid point) | $1.0$     | $17.55$                            | $17.80$                             | $1.57$                   | $553.00$                                             | $213.82$                                            | $\times$                           |
|     | \ig             | $5.0$     | $18.98$                            | $19.17$                             | $1.44$                   | $503.94$                                             | $215.75$                                            | $\times$                           |
|     | \dre            | $0.01$    | $\mathbf{20.98}$                   | $\mathbf{20.80}$                    | $1.79$                   | $497.47$                                             | $275.00$                                            | $\checkmark$ both                  |
|   2 | \dre            | $0.5$     | $20.96$                            | $20.34$                             | $1.42$                   | $367.47$                                             | $182.06$                                            | $\checkmark$ both                  |
|     | \dre            | $1.0$     | $\mathbf{20.98}$                   | $20.68$                             | $\mathbf{1.28}$          | $\mathbf{334.47}$                                    | $144.16$                                            | $\checkmark$ both                  |
|     | \dre            | $5.0$     | $20.81$                            | $20.13$                             | $1.37$                   | $374.13$                                             | $\mathbf{134.33}$                                   | $\checkmark$ both                  |
|     | \cai (gauss)    | N/A       | $\mathbf{20.98}$                   | $20.72$                             | $1.75$                   | $596.67$                                             | $442.88$                                            | $\checkmark$ both                  |
|     | \cai (dirac)    | N/A       | $\mathbf{20.98}$                   | $20.61$                             | $1.56$                   | $644.35$                                             | $534.39$                                            | $\checkmark$ both                  |
#+LATEX: }
#+end_table
...
each of the geometry-based methods in Environment 1.
In Environment 2, four different settings of the $\lambda$ parameter were tested for the
direct optimal control method and three settings were tested for the indirect optimal control method.
This was due to the indirect optimal control method's collocation solver failing with $\lambda=0.01$.
The control-as-inference (\cai) method from cref:sec-traj-opt-inference was tested with
Gaussian controls (gauss) and with deterministic (dirac) controls.
This enabled the maximum entropy control resulting from Gaussian controls to be compared to a baseline
without maximum entropy control.
...
This is because the indirect geodesic control experiments struggled to find solutions in Environment 2
-- due to local optima -- without adjusting the initial solution.

cref:fig-geodesic-low-traj-opt-7,fig-geodesic-traj-opt-7,fig-geodesic-high-traj-opt-7,fig-geodesic-low-traj-opt-5,fig-geodesic-traj-opt-5,fig-geodesic-high-traj-opt-5,fig-geodesic-mid-point-traj-opt-5
show the results using the indirect geodesic control method from cref:sec-traj-opt-collocation,
cref:fig-riemannian-energy-low-traj-opt-7,fig-riemannian-energy-traj-opt-7,fig-riemannian-energy-high-traj-opt-7,fig-riemannian-energy-low-traj-opt-5,fig-riemannian-energy-traj-opt-5,fig-riemannian-energy-high-traj-opt-5,fig-riemannian-energy-low-2-traj-opt-5
show the results using the direct optimal control method from cref:sec-traj-opt-energy and
cref:fig-mode-conditioning-max-entropy-traj-opt-7,fig-mode-conditioning-no-max-entropy-traj-opt-7,fig-mode-conditioning-max-entropy-traj-opt-5,fig-mode-conditioning-no-max-entropy-traj-opt-5
show the results using  the control-as-inference method from cref:sec-traj-opt-inference.
...
The indirect optimal control method's collocation solver was able to find state trajectories to the target state in all
experiments.
This is indicated by the collocation trajectories (yellow) in
cref:fig-geodesic-traj-opt-5,fig-geodesic-high-traj-opt-5,fig-geodesic-low-traj-opt-5,fig-geodesic-mid-point-traj-opt-5,fig-geodesic-low-traj-opt-7,fig-geodesic-traj-opt-7,fig-geodesic-high-traj-opt-7
reaching the target state  $\targetState$.
This is because the collocation solver is guaranteed to satisfy the boundary conditions.

In Environment 1, the inference strategy successfully recovered controls that drive the system along the
collocation solver's state trajectory (yellow).
This is indicated by the dynamics trajectory (magenta) following the collocation trajectory (yellow) in
...
This can be seen by the dynamics trajectories (magenta) differing from the collocation trajectories (yellow) in
...
All of the direct optimal control experiments, in
cref:fig-riemannian-energy-low-2-traj-opt-5,fig-riemannian-energy-low-traj-opt-5,fig-riemannian-energy-traj-opt-5,fig-riemannian-energy-high-traj-opt-5,fig-riemannian-energy-low-traj-opt-7,fig-riemannian-energy-traj-opt-7,fig-riemannian-energy-high-traj-opt-7,fig-mode-conditioning-max-entropy-traj-opt-7,fig-mode-conditioning-no-max-entropy-traj-opt-7
were able to find trajectories
that navigate to the target state $\targetState$ under the desired mode's GP dynamics (magenta).
It is worth noting that these methods do not guarantee that trajectories will satisfy the boundary conditions.
...
In practice the mode chance constraints inform us that
the trajectory is not $\delta-\text{mode remaining}$ so the trajectory would not be executed in the environment.
...
All of the control-as-inference experiments successfully navigated to the target state under the
dynamics (magenta) and in the environment (cyan).
This is indicated by the magenta and cyan trajectories in
cref:fig-mode-conditioning-max-entropy-traj-opt-5,fig-mode-conditioning-no-max-entropy-traj-opt-5,fig-mode-conditioning-max-entropy-traj-opt-7,fig-mode-conditioning-no-max-entropy-traj-opt-7
successfully navigating to the target state $\targetState$.
Similarly to the direct optimal control method, this approach is not guaranteed
to find trajectories that will end at the target state.
However, in practice, setting a high terminal state cost matrix $\terminalStateCostMatrix$ works well.
...
*Indirect geodesic control \ig*
None of the indirect geodesic control experiments were able to successfully remain in the desired dynamics mode.
This is shown by the collocation trajectories (yellow) passing over the mode boundary
into the turbulent dynamics mode in
cref:fig-geodesic-low-traj-opt-7,fig-geodesic-traj-opt-7,fig-geodesic-high-traj-opt-7,fig-geodesic-low-traj-opt-5,fig-geodesic-traj-opt-5,fig-geodesic-high-traj-opt-5,fig-geodesic-traj-opt-5,fig-geodesic-high-traj-opt-5,fig-geodesic-low-traj-opt-5.
In Environment 1, although none of the indirect geodesic control
experiments successfully remained in the desired dynamics mode, they did exhibited mode remaining behaviour.
...
This trajectory is named $\ig (mid point)$.
...
\newline
*Direct optimal control \dre*
All but one of the \dre experiments were able to successfully remain in the desired dynamics mode.
...
cref:fig-riemannian-energy-low-2-traj-opt-5,fig-riemannian-energy-low-traj-opt-5,fig-riemannian-energy-traj-opt-5,fig-riemannian-energy-high-traj-opt-5.
cref:tab-results-sim-envs shows that the trajectory found with \dre $\lambda=1.0$ obtained
the highest probability of remaining in the desired dynamics mode for Environment 1.
Similarly, in Environment 2 the trajectories found with \dre $\lambda=0.01$ and \dre $\lambda=1.0$ in
cref:fig-riemannian-energy-low-traj-opt-5 and cref:fig-riemannian-energy-traj-opt-5
respectively, obtained the highest probability of remaining in the desired dynamics mode.
From visual inspection of cref:fig-riemannian-energy-low-traj-opt-7,fig-riemannian-energy-traj-opt-7,
the trajectory found with \dre $\lambda=1.0$ has more clearance from the turbulent dynamics
mode than for \dre $\lambda=0.5$ and \dre $\lambda=20.0$.
...
Close inspection of
cref:fig-riemannian-energy-low-2-traj-opt-5,fig-riemannian-energy-low-traj-opt-5,fig-riemannian-energy-traj-opt-5,fig-riemannian-energy-high-traj-opt-5
shows that the trajectory found with \dre $\lambda=0.01$ achieved a lower probability
because it has the most clearance from the desired dynamics mode.
Interestingly, although the trajectory found with \dre $\lambda=1.0$
...
Further to this, the trajectory found with \dre $\lambda=0.01$ obtained the lowest probability even though it
...
The trajectory found with \dre $\lambda=5.0$ in Environment 2 obtained the lowest probability of
remaining in the desired dynamics mode.
...
show the contour following that achieves the mode remaining behaviour in Environment 2.
...
the trajectory found with \dre $\lambda=20.0$ in cref:fig-riemannian-energy-high-traj-opt-7
...
It is clear from
cref:fig-mode-conditioning-no-max-entropy-traj-opt-7,fig-mode-conditioning-max-entropy-traj-opt-7,fig-mode-conditioning-no-max-entropy-traj-opt-5,fig-mode-conditioning-max-entropy-traj-opt-5,
...
This observation is confirmed in cref:tab-results-sim-envs, where the experiments with maximum entropy
...
In Environment 2, both of the control-as-inference experiments
in  cref:fig-mode-conditioning-no-max-entropy-traj-opt-5,fig-mode-conditioning-max-entropy-traj-opt-5
...
In contrast, the direct optimal control method requires little to no human input to initialise the optimisation.
Overall, the direct optimal control and the control-as-inference with maximum entropy control experiments
achieved the best mode remaining behaviour.
This is indicated by them consistently obtaining the highest probabilities of remaining in the desired
...
each trajectory.
In all Environment 1experiments,
...
*Indirect optimal control*
In the \ig experiments in Environment 1 with $\lambda=1.0$ and $\lambda=0.5$, the trajectories navigate
...
In contrast, the \ig experiment with $\lambda=20.0$ (in cref:fig-geodesic-high-traj-opt-7)
avoided this region.
However, it should be noted that
this uncertainty avoiding behaviour came at the cost of passing straight through the turbulent dynamics mode.
In Environment 2 the \ig experiments did not remain in the desired dynamics mode, nor did they exhibit
any uncertainty avoiding behaviour.
As mentioned previously, this is likely due to the collocation solver getting stuck in local optima.

***** Direct optimal control :ignore:
\newline
*Direct optimal control*
In Environment 2 the trajectory found with \dre $\lambda=1.0$ accumulated the
least state and gating function variance.
In contrast, the result for \dre $\lambda=0.01$ accumulated the highest state and gating variance, whilst also
...
indirect optimal contro method working, without engineering the initial solution.
...
In contrast, setting the parameters for the indirect optimal contro method is not.
...
# - The indirect optimal contro method's collocation solver is susceptible to local optima around its initial state trajectory,
#    - Although this can be overcome by engineering the initial solution, in practice, it makes it difficult to
#      get the method working.
# - The variational inference strategy used to recover the controls for the indirect optimal contro method cannot always
...
# The main drawback of the indirect optimal contro method is that it does not enforce the dynamics constraints.


#+begin_table
#+LATEX: \caption{Comparison of the indirect optimal contro via latent geodesics method from \cref{sec-traj-opt-collocation}, the direct optimal control via Riemannian energy method from \cref{sec-traj-opt-energy} and the control-as-inference method from \cref{chap-traj-opt-inference}.}
#+LATEX: \label{tab-geometry-control-comparison}
#+LATEX: \resizebox{\textwidth}{!}{
#+ATTR_LATEX: :center t :placement [!t] :align lccc
|                                                    | Control as Inference | Indirect Optimal Control | Direct Optimal Control |
|----------------------------------------------------+----------------------+--------------------------+------------------------|
...
In contrast, the indirect optimal contro method does not find trajectories that are guaranteed to satisfy the dynamics
constraints.
This is a limiting factor that makes the indirect optimal contro method less appealing.
...
# Future work could consider extending the indirect optimal contro method to enforce the dynamics constraints.
# Further to this,
# *Uncertainty aware*
...
In contrast, the indirect optimal contro method ignores the uncertainty accumulated by cascading single-step predictions through
...
In contrast, the indirect optimal contro method uses a collocation solver where the cost can be evaluated at arbitrary
...
the impact of different gating entropy objectives evaluated.
...
This section presents initial results of \modeOpt's exploration strategy.
The results show the strategy successfully exploring the simulated Environment 1 in cref:sec-traj-opt-results-simulated.
...
Each \modeOpt iteration $i$ is visualised with a set of four subfigures showing how the desired mode's
...
2. The second subfigure overlays the trajectory found by the explorative controller $\explorativeController$
   ...
   show the explorative trajectory found at the first \modeOpt iteration.
...
the first iteration of \modeOpt and cref:fig-explorative-traj-opt-over-variance-7-1 shows how the gating
...
# visualise the first iteration of \modeOpt.
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_0_epoch_0.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_1_epoch_0.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_1_epoch_last.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_0_epoch_0.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/trajectories_over_desired_gating_gp_step_1.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_1_epoch_0.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_1_epoch_last.pdf}
...
*Initial model*
cref:fig-explorative-data-over-prob-7-step-0-epoch-0-expansion,fig-explorative-data-over-variance-7-step-0-epoch-0-expansion
shows the initial data set $\dataset_0$ (blue crosses) overlayed on the gating network after training on it.
cref:fig-explorative-data-over-prob-7-step-0-epoch-0-expansion
...
cref:fig-explorative-data-over-variance-7-step-0-epoch-0-expansion
shows that the gating function's posterior variance is low around the initial data set and high everywhere else.
Intuitively, \modeOpt seeks to expand the explorable region so that
...
expanding as \modeOpt collects more data and trains on it.

*Iteration $\mathbf{i=1}$*
cref:fig-explorative-traj-opt-over-prob-7-1,fig-explorative-traj-opt-over-prob-7-1
visualises the first iteration $i=1$ of \modeOpt.
Given the dynamics model after training on the initial data set $\dataset_0$,
the explorative controller $\explorativeController$ found the trajectory in
cref:fig-explorative-data-over-variance-7-step-1-epoch-0-expansion,fig-explorative-data-over-prob-7-step-1-epoch-0-expansion
...
cref:fig-explorative-data-over-variance-7-step-1-epoch-0-expansion
...
cref:fig-explorative-data-over-prob-7-step-1-epoch-1494-expansion,fig-explorative-data-over-variance-7-step-1-epoch-1494-expansion
show how the gating network changes after training on $\dataset_1$.
The gating function's posterior variance has decreased around the new observations.
The purple lines representing the
$\Pr(\alpha=\desiredMode \mid \state, \dataset_{0}) = 0.8$ contour in
cref:fig-explorative-data-over-prob-7-step-1-epoch-0-expansion,fig-explorative-data-over-variance-7-step-1-epoch-0-expansion
and the $\Pr(\alpha=\desiredMode \mid \state, \dataset_{0:1}) = 0.8$ contour in
cref:fig-explorative-data-over-prob-7-step-1-epoch-1494-expansion,fig-explorative-data-over-variance-7-step-1-epoch-1494-expansion
...
This result indicates that in this environment, setting $\delta=0.2$ enables exploration outside of the initial
...
*** Step 2:3 Prob Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{2}
\renewcommand{\stepNumNext}{3}
\renewcommand{\lastEpoch}{last}
\begin{figure}
\centering
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_\stepNum_epoch_\lastEpoch.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/trajectories_over_mixing_probs_step_\stepNumNext.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_\stepNumNext_epoch_0.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_\stepNumNext_epoch_last.pdf}
...
*** Step 2:3 Variance Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{2}
\renewcommand{\stepNumNext}{3}
\renewcommand{\lastEpoch}{last}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_\stepNum_epoch_last.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/trajectories_over_desired_gating_gp_step_\stepNumNext.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_\stepNumNext_epoch_0.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_\stepNumNext_epoch_last.pdf}
...
*** Step 3:4 Prob Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{3}
\renewcommand{\stepNumNext}{4}
\renewcommand{\lastEpoch}{last}
\begin{figure}
\centering
\begin{minipage}[r]{0.48\columnwidth}
...
*** Step 3:4 Variance Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{3}
\renewcommand{\stepNumNext}{4}
\renewcommand{\lastEpoch}{last}
...
*** iteration 3 text :ignore:
\newline

# cref:fig-explorative-traj-opt-over-prob-7-1,fig-explorative-traj-opt-over-prob-7-1
*Iteration $\mathbf{i=3}$*
cref:fig-explorative-traj-opt-over-prob-7-2 shows the third iteration
...
As a result, the trajectory found by the explorative controller in
cref:fig-explorative-traj-opt-over-prob-7-step-3-expansion leaves the desired dynamics mode and is subject to the
turbulent dynamics mode.
This is shown by the trajectory crossing the mode boundary (purple line) and also by
...
\end{remark}
#+END_EXPORT
cref:fig-explorative-data-over-prob-7-step-3-epoch-last-expansion
...
However, the explorable domain still intersected the mode boundary after training.

*** iteration 4 text :ignore:
\newline

*Iteration 4*
cref:fig-explorative-traj-opt-over-prob-7-3,fig-explorative-traj-opt-over-variance-7-3
shows the fourth iteration where once again
the explorative controller found a trajectory which left the desired dynamics mode.
Although not desirable, this is necessary for inferring the mode boundary.
cref:fig-explorative-data-over-prob-7-step-4-epoch-last-expansion,fig-explorative-data-over-variance-7-step-4-epoch-last-expansion
show that after training on the new
data set $\dataset_{0:4}$ the explorable domain no longer intersects with the mode boundary.
As a result, the explorative trajectory found at the next iteration (in
cref:fig-explorative-traj-opt-over-prob-7-step-5-expansion) did not leave the desired dynamics mode.
...
*** Step 4:5 Prob Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{4}
...
*** Step 4:5 Variance Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{4}
\renewcommand{\stepNumNext}{5}
\renewcommand{\lastEpoch}{last}
...
*** Step 1:6 Prob Figs :ignore:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{0}
\renewcommand{\stepNumNext}{1}
\renewcommand{\stepNumNextNext}{2}
\renewcommand{\stepNumNextNextNext}{3}
\renewcommand{\lastEpoch}{last}
\begin{figure}
\centering
\begin{minipage}[r]{0.32\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_\stepNum_epoch_0.pdf}
%\subcaption{$\dataset_{0:\stepNum}$ overlayed on $\Pr(\modeVar = 2 \mid \state, \dataset_{0:\stepNum})$}
\label{fig-explorative-data-over-prob-7-step-\stepNum-epoch-0-expansion}
\end{minipage}
\begin{minipage}[r]{0.32\columnwidth}
...
\begin{minipage}[r]{0.32\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/trajectories_over_mixing_probs_step_\stepNumNextNext.pdf}
%\subcaption{Explorative trajectory over $\Pr(\modeVar = 2 \mid \state, \dataset_{0:\stepNumNext})$}
\label{fig-explorative-traj-opt-over-prob-7-step-\stepNumNext-expansion}
\end{minipage}
\begin{minipage}[r]{0.32\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_\stepNumNextNext_epoch_0.pdf}
%\subcaption{$\dataset_{0:\stepNumNextNext}$ overlayed on $\Pr(\modeVar = 2 \mid \state, \dataset_{0:\stepNumNext})$}
\label{fig-explorative-data-over-prob-7-step-\stepNumNext-epoch-0-expansion}
\end{minipage}
\begin{minipage}[r]{0.32\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_\stepNumNextNext_epoch_last.pdf}
%\subcaption{$\dataset_{0:\stepNumNextNext}$ overlayed on $\Pr(\modeVar = 2 \mid \state, \dataset_{0:\stepNumNextNext})$}
\label{fig-explorative-data-over-prob-7-step-\stepNumNextNext-epoch-\lastEpoch-expansion}
\end{minipage}
\begin{minipage}[r]{0.32\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/trajectories_over_mixing_probs_step_\stepNumNextNextNext.pdf}
%\subcaption{Explorative trajectory over $\Pr(\modeVar = 2 \mid \state, \dataset_{0:\stepNumNextNext})$}
\label{fig-explorative-traj-opt-over-prob-7-step-\stepNumNext-expansion}
\end{minipage}
\begin{minipage}[r]{0.32\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_mixing_probs_step_\stepNumNextNextNext_epoch_0.pdf}
%\subcaption{$\dataset_{0:\stepNumNextNextNext}$ overlayed on $\Pr(\modeVar = 2 \mid \state, \dataset_{0:\stepNumNextNext})$}
\label{fig-explorative-data-over-prob-7-step-\stepNumNext-epoch-0-expansion}
\end{minipage}
\caption{\textbf{\modeOpt iterations $\mathbf{i=\stepNum:\stepNumNextNextNext}$ over mode probability}
Visualisation of \modeOpt iterations $i=\stepNum:\stepNumNextNextNext$ in Environment 1 overlayed
on the desired mode's mixing probability.
It shows the explorable region -- indicated by the boundary of the mode chance constraints (purple 0.8 contour)
-- expanding after training on data collected using the explorative controller $\explorativeController$.
Each row shows the data collection process associated with an iteration $i$
overlayed on the desired mode's mixing probability \textbf{after} training on the data set collected
at the previous iteration.
The left plots show the data set collected at the previous iteration $\dataset_{0:i}$ (blue crosses).
The middle plots show the trajectory found by the explorative controller rolled out in the desired mode's GP
dynamics (magenta) and in the environment (cyan).
The right plots show the updated data set $\dataset_{0:i+1}$ (blue crosses) after collecting data with the explorative
controller.
Reading left to right shows the data collection process whilst top to bottom
shows the dynamics model updating after training on the new data sets.}
\label{fig-explorative-traj-opt-over-prob-7-\stepNum-\stepNumNextNextNext}
\end{figure}
#+END_EXPORT
...
\renewcommand{\stepNum}{3}
\renewcommand{\stepNumNext}{4}
\renewcommand{\stepNumNextNext}{5}
\renewcommand{\stepNumNextNextNext}{6}
\renewcommand{\lastEpoch}{last}
\begin{figure}
\centering
\begin{minipage}[r]{0.32\columnwidth}
...
*** Step 5:6 Prob Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{5}
\renewcommand{\stepNumNext}{6}
\renewcommand{\stepNumNextNext}{7}
\renewcommand{\lastEpoch}{last}
\begin{figure}
\centering
\begin{minipage}[r]{0.48\columnwidth}
...
*** Step 5:6 Variance Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{5}
\renewcommand{\stepNumNext}{6}
\renewcommand{\lastEpoch}{last}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_\stepNum_epoch_\lastEpoch.pdf}
\subcaption{$\dataset_{0:\stepNum}$ overlayed on $\mathbb{V}\left[\desiredGatingFunction(\state) \mid \state, \dataset_{0:\stepNum} \right]$}
\label{fig-explorative-data-over-variance-7-step-\stepNum-epoch-0-expansion}
\end{minipage}
\begin{minipage}[r]{0.48\columnwidth}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} {.12\height} 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/data_over_desired_gating_gp_step_\stepNumNext_epoch_\lastEpoch.pdf}
\subcaption{$\dataset_{0:\stepNumNext}$ overlayed on $\mathbb{V}\left[\desiredGatingFunction(\state) \mid \state, \dataset_{0:\stepNumNext} \right]$}
\label{fig-explorative-data-over-variance-7-step-\stepNumNext-epoch-\lastEpoch-expansion}
\end{minipage}
\caption{\textbf{\modeOpt iteration $\mathbf{i=\stepNum:\stepNumNext}$ over gating function variance}
...
*** Step 10:11 Prob Figs :ignore:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{10}
\renewcommand{\stepNumNext}{11}
\renewcommand{\lastEpoch}{last}
\begin{figure}
\centering
\begin{minipage}[r]{0.48\columnwidth}
...
*** Step 10:11 Variance Figs :ignore:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{10}
\renewcommand{\stepNumNext}{11}
\renewcommand{\lastEpoch}{last}
...
*** Step 11:12 Prob Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{11}
\renewcommand{\stepNumNext}{12}
\renewcommand{\lastEpoch}{last}
\begin{figure}
\centering
\begin{minipage}[r]{0.48\columnwidth}
...
*** Step 11:12 Variance Figs :ignore:noexport:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{11}
\renewcommand{\stepNumNext}{12}
\renewcommand{\lastEpoch}{last}
...
show the final iteration of the exploration phase.
The mode chance constraints have expanded such that the target state $\targetState$ lies within the explorable domain.
...
exploitative trajectory leaving the desired dynamics mode during training.
Only two iterations of \modeOpt left the desired dynamics mode during training and
arguably it is necessary in order to learn the location of the mode boundary.

# During training \modeOpt was able to expand the chance constraints in the positive $y$ direction.
...
is interesting to analyse how it changes during each \modeOpt iteration.
Intuitively, as \modeOpt gathers more data, the gating function's variance decreases.
However, the mode boundary, i.e. the mode chance constraints, introduce some interesting behaviour.
For example, at the start of \modeOpt the shape of the region of low gating function variance is very
similar to the shape of the desired mode's mixing probability.
This can be seen in
cref:fig-explorative-traj-opt-over-prob-7-1,fig-explorative-traj-opt-over-variance-7-1 where they are both either
high or low around the data set.
However, when the gating network starts learning the mode boundary at iteration $i=3:4$
(in cref:fig-explorative-traj-opt-over-prob-7-3,fig-explorative-traj-opt-over-variance-7-3)
the shape of the
desired mode's mixing probability changes and no longer matches the shape of the gating function's posterior variance.
...
Nevertheless the results shown here are an initial step showing that \modeOpt can work in practice.
...
However, this would make \modeOpt applicable with a wider range of \acrshort{mogpe} dynamics models and
...
*Closed-loop control*
Although the trajectories found by the explorative controller are $\delta-\text{mode remaining}$, often when
the trajectories are executed in open-loop in the environment the resulting trajectories are not
$\delta-\text{mode remaining}$.
This behaviour is expected when executing open-loop controllers.
Although this did not pose an issue in the simulated experiments, it may pose an issue in real-world environments.
Future work should explore methods for obtaining closed-loop controllers.
A simple approach is to embed the trajectory optimisation algorithm into an \acrshort{mpc} loop to obtain a
closed-loop controller.
Alternatively, an explorative state-feedback controller could be learned using policy search.
