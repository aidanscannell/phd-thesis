...
$\mode{\stateDomain} = \{ \state \in \stateDomain \mid \modeVar = \modeInd \}$,
and
$\stateDomain_{i} \cap \stateDomain_{j} = \emptyset \quad \text{for} \quad i \neq j$.
...
The learned model may not be able to confidently predict which mode governs the dynamics at any given region.
...
flatness citep:milamNew2000,rossPseudospectral2004.
...
*** Remarks :ignore:
First of all, this approach does do not necessarily find trajectories that satisfy the dynamics constraints,
...
\end{definition}
#+END_EXPORT
The conditions to be $\delta-\text{mode remaining}$ in cref:def-delta-mode-remaining can
be enforced with chance constraints,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-mode-chance-constraint}
\Pr(\modeVar_{\timeInd} = \desiredMode \mid \state_{0}, \control_{0:\timeInd}, \bm\modeVar_{0:\timeInd}=\desiredMode)
...
\end{subequations}
#+END_EXPORT
using Sequential Least Squares Programming (SLSQP) in SciPy cite:2020SciPy-NMeth.
...
- Goal 2 :: Avoid regions of the learned dynamics with high /epistemic uncertainty/,
  - in the desired dynamic mode.
  - in the gating network,
To test their ability at avoiding regions of high epistemic uncertainty, a subset of the environment was intentionally
not observed. The trajectories are evaluated using two performance indicators:
1. The desired mode's ($\modeVar=1$) mixing probability,
2. The posterior variance associated with its gating function \acrshort{gp}.
...
The initial (cyan) trajectory in cref:fig-geometric-traj-opt-over-svgp is initialised with
...
As such, Mode 1 represents the operable dynamics mode and Mode 2 represents the turbulent dynamics mode.
...
|--------------------------+------------------------------------------------------------------------------+-------------------------------------------------------------------------------|
| Trajectory               | Mixing Probability                                                           | Epistemic Uncertainty                                                         |
|                          | $\sum_{\timeInd=0}^{\TimeInd} \Pr(\modeVar_\timeInd=1 \mid \state_\timeInd)$ | $\sum_{\timeInd=0}^{\TimeInd} \mathbb{V}[\gatingFunc^{(1)}(\state_\timeInd)]$ |
|--------------------------+------------------------------------------------------------------------------+-------------------------------------------------------------------------------|
| Optimised $\lambda=0.1$  | $$                                                                           | $$                                                                            |
| Optimised $\lambda=1.0$  | $$                                                                           | $$                                                                            |
| Optimised $\lambda=10.0$ | $$                                                                           | $$                                                                            |
...
The main drawbacks of the indirect control method are:
1. the fact it does not enforce the dynamics constraints,
2. its limitation to systems that are differentially flat.
...
The chance constraints enforce that the controlled system is $\delta$ - mode remaining,
ensuring this approximate dynamics integration is valid.
...
However, future work could attempt to enforce the dynamics constraints in the indirect methods collocation solver.
...
| Applicable in non differentially flat systems?     | \times           | \checkmark     |
...
In contrast, the direct control method does not make such assumptions and is applicable in a much larger range of systems.
