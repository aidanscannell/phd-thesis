...
- cref:chap-traj-opt-control: investigates model-based control methods that leverage the probabilistic model
  from cref:chap-dynamics to solve the mode remaining control problem in cref:eq-main-problem.
  Due to the complexity of the problem, it
...
First of all, it should be noted that more experiments are required to fully test $\modeOpt$.

Compare myopic vs non-myopic entropy in MPC loop vs mutual information
...
works in binary setting.
What about general mulitmodal setting?
