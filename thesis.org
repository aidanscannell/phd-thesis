...
# As the underlying dynamics modes and how the system switches between them, are \textit{not fully known a priori}.
# To this end, this dissertation is interested in synergising model learning and model-based control (or planning) to
# solve the quadcopter control problem.

The main goal of this work is to navigate the quadcopter from an initial state $\state_0$, to the target
state $\targetState$, without entering the inoperable, turbulent dynamics mode.
...
Given such a data set, this dissertation addresses learning environment models that can be exploited for
model-based control (or planning).
# To this end, cref:chap-dynamics investigates learning a factorised representation of multimodal dynamical
# systems.

cref:chap-traj-opt-geometry,chap-traj-opt-inference then investigate trajectory optimisation
algorithms that leverage the latent structure
of the probabilistic model from Chapter ref:chap-dynamics -- after performing Bayesian inference
...
# This work first considers having prior access to the environment, where it can perform prior flight
# trials and observe state transitions over the domain, i.e. collect a data set of state transitions $\mathcal{D}$.
# Given such a data set, this dissertation is interested in synergising model learning and model-based control
# (or planning)
# Chapter ref:chap-dynamics investigates learning a factorised representation of the transition
# dynamics.
...
# and how the dynamics switches between the modes, are \textit{not fully known a priori}.
...
This work derives novel evidence lower bounds based on sparse \acrshort{gp} approximations, that provide
...
\end{figure}
#+END_EXPORT
The work in this chapter first uses the \acrshort{mosvgpe} method from cref:chap-dynamics to learn a factorised
...
\end{subequations}
#+END_EXPORT
and solved using Sequential Least Squares Programming (SLSQP) in SciPy.
...
differential flatness to obtain the controls from the optimised state trajectory and geodesic \acrshort{ode}.
...
As the \acrshort{mosvgpe} model assumes that the mode indicator variable $\modeVar$, depends on the state via the gating function,
...
using the illustrative example from cref:illustrative_example, i.e.
flying a quadcopter from an initial state $\state_0$, to a target state $\state_{f}$, whilst avoiding the turbulent
dynamics mode.
See cref:fig-problem-statement for a schematic of the environment and details of the problem.
The turbulent dynamics mode is subject to higher drift in the negative $y$ direction due to the
...
This is due to high process noise which may lead to catastrophic failure.
It is therefore desirable to find trajectories that avoid entering this turbulent dynamics mode.

This section evaluates the trajectory optimisation algorithms' performance at achieving the two goals:
- Goal 1 :: Remain in the desired operable dynamics mode,
- Goal 2 :: Avoid regions of the gating network with high epistemic uncertainty.
To test their ability to avoid regions of high epistemic uncertainty a subset of the environment was intentionally
not observed. The trajectories are evaluated using two metrics.
The desired mode's ($\modeVar=1$) mixing probability and the
posterior variance associated with its gating function \acrshort{gp}.
...
The state-space of the velocity controlled quadcopter example consists of the 2D Euclidean coordinates $\state = (x, y)$,
i.e. the $x$ and $y$ coordinates.
...
\end{figure}
#+END_EXPORT
The trajectory optimisation presented in cref:sec-traj-opt-collocation is evaluated on the real-world quadcopter
control problem detailed in cref:sec-brl-experiment.
This method recovers the controls via differential flatness citep:rossPseudospectral2004,
so the controls were kept constant during data collection, reducing the dynamics to
$\Delta \state_{\timeInd+1} = \dynamicsFunc(\state_{\timeInd}; \control_{\timeInd}=\fixedControl)$.

**** Model Learning
\todo{Add comment about older ELBO?}
...
Each \acrshort{gp} associated with the dynamics modes used a squared exponential kernel
with \acrfull{ard} and a constant mean function.
...
the desired mode ($\modeVar = 1$) is responsible for predicting over the domain.
cref:fig-geometric-traj-opt-over-svgp shows the \acrshort{gp} posterior mean (left) and variance (right) of the
gating function $\gatingFunc_1$ associated with the desired dynamics mode ($\modeVar 1$).
...
%\label{}
\subcaption{Posterior variance associated with the desied mode's (\modeVar=1) gating function over the trajectories.}
\label{fig-epistemic_var_vs_time}
\end{minipage}
\caption{\label{fig-metric-vs-time}Comparision of the intial and optimised trajectories' performance -- for two
...
that 1) remain in the desired operable mode and 2) avoid regions of the gating network with high epistemic uncertainty.
...
Table ref:tab-results compares the initial trajectory to the results obtained
...
The collocation solver guarantees that trajectories will end at the target state.
However, trajectories are not guaranteed to remain in a desired dynamics, nor are they guaranteed to
...
underlying dynamics are given by simple Newtonian mechanics (velocity times time).
The two dynamics modes are governed by different "wind fields" which are implemented using different
drift $\windDrift{\modeVar}$ and process noise $\windTurbulence{\modeVar}$ terms.
Each mode's dynamics are given by,
#+BEGIN_EXPORT latex
\begin{align}
\dynamicsFunc_{\modeVar}(\state_\timeInd, \control_\timeInd; \Delta\timeInd=0.25)
&= \control_{\timeInd} \times \Delta\timeInd + \windDrift{\modeVar} + \windTurbulence{\modeVar}
\quad \text{if} \quad \modeVar_\timeInd = \modeInd \\
\windTurbulence{\modeVar} &\sim \mathcal{N}(\bm0, \windTurbulenceNoise{\modeVar}),
\end{align}
#+END_EXPORT
with
...
represent the operable dynamics mode and one to represent the turbulent dynamics mode.
...
For example, in a scenario where the main goal is to remain in the desired dynamics mode and avoiding regions of high
epistemic uncertainty is a secondary goal.
...
posterior mean's contours, once passed the uncertain region.
As a results, the trajectories successfully remain in the desired dynamics mode.
...
Remember that chance constraints are governed by the desired mode's mixing probability,
...
Again, this may be due to either the desired dynamics mode being uncertain, or the gating network being
uncertain.
In this scenario, it is desirable to explore the environment and update the learned dynamics mode
...
This motivates the work in cref:chap-active-learning, which addresses active learning in
...
|                                    | Indirect Control | Direct Control | Comments |
|------------------------------------+------------------+----------------+----------|
| Dynamics Constraints               | \times           | \checkmark     |          |
| Decouple Goals                     | \checkmark       | \checkmark     |          |
| Boundary Conditions Guaranteed     | \checkmark       | \times         |          |
| Non Differential Flat Systems      | \times           | \checkmark     |          |
| Mode Chance Constraints Guaranteed | \times           | \times         |          |
| Remain in *multiple* modes         | \times           | \times         |          |
#+LATEX: }
#+end_table
...
The main drawbacks of the indirect control method that motivated the direct control method, are 1) its limitation
to systems that are diffentially flat and 2) the fact it does not enforce the dynamics constraints.

*Differential Flatness*
The indirect control method is only applicable to systems that are differentially flat.
For example, it is not applicable to thrust/torque controlled quadcopters.
Further to this, it relies upon learning a slice of the transition dynamics with constant controls.
In practice, collecting a data set with constant controls is awkward.
In contrast, the direct control method does not make assumptions on the data collection process
and it is applicable in systems which are not differentially flat.

*Dynamics Constraints Satisfaction*
The direct control method also ensures trajectories satisfy the learned dynamics.
...
The chance constraints ensure that the controlled system is $\delta$ - mode remaining,
which makes this approximate dynamics integration valid.

# Future work could consider extending the indirect control method to enforce the dynamics constraints.
Further to this, the combination of approximate dynamics integration and chance constraints
leads to closed-form expressions for the expected cost.
This expression considers all of the epistemic uncertainty associated with the learned dynamics model.
Both the uncertainty in the desired dynamics mode and the uncertainty in the gating network.
...
# In this case the uncertainty in the gating network may be low,
# the expected
...
The trajectory optimisation algorithms presented in this chapter successfully find trajectories that
remain in a desired dynamics mode.
However, the main goal is to avoid entering the inoperable dynamics mode.
As such, an interesting direction for future work, is to develop control methods that can remain in a set of
dynamics modes.

Although not tested, the approach is theoretically sound and should be applicable in systems
with more than two dynamics modes.
...
The desired dynamics mode was engineered to have a noise variance that we deemed operable.
...
*Decouple goals*
Both approaches also provide a mechanism to set the relevance of each goal, i.e. it decouples them.
This is achieved by augmenting the expected Riemannian metric tensor with a user-tunable weight
parameter $\lambda$, that can be adjusted to determine the relevance of the covariance term.

*Invariant sets*
Recent work by
cite:berkenkampSafe2019,berkenkampSafe2017,berkenkampSafe2015,kollerLearningBased2018
considers the use of invariant sets to synthesise closed-loop controllers that are guaranteed to remain in
given set of states.
Their work considers safe sets defined by Lyapanov stability but it would be interesting to extend their
concepts to synthesise $\delta$ -mode remaining controllers
...
*Continuous Time*
The direct control method considered in this dissertation consider
required control regularisation to prevent the controls leading to state transitions that
"jump" over the undesired mode.
That is, trajectories whose discrete time steps appear to satisfy the constraints/cost but the actual continuous time
...
* Mode Constrained Active Learning in Multimodal Dynamical Systems label:chap-active-learning
#+begin_export latex
