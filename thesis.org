...
However, the extending the method in cref:sec-exploration
...
** Preliminary Results
*** intro :ignore:
This section presents some initial results for \modeOpt on the illustrative quadcotper navigation problem
from cref:illustrative_example.
In particular, it is tested in the simulated Environment 1 from cref:sec-traj-opt-results-simulated.
...
cref:fig-explorative-data-over-prob-7-initial shows the probability mass function over the mode indicator variable
where the probability of being in the desired dynamics mode $\desiredMode=2$ is high around the initial data set.
It tends to $0.5$ away from the data, i.e. maximum entropy for a Categorical distribution.
cref:fig-explorative-data-over-svgp-7-initial shows the \acrshort{gp} posterior over the desired mode's gating function.
...
This section provides details on how the experiments were configured.
...
This resulted in an initial data set of $118$ state transitions.
...
\centering
%\includegraphics[width=0.78\textwidth]{./images/mode-opt/exploration/trajectories_over_mixing_probs_step_\stepNum.pdf}
\adjincludegraphics[trim={{.54\width} 0 0 0},clip, width=0.5\textwidth]{./images/mode-opt/exploration/state-diff-only/trajectories_over_mixing_probs_step_17.pdf}
...
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} 0 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-diag/trajectories_over_mixing_probs_step_1.pdf}
\subcaption{Factorised entropy \cref{eq-factorised-entropy}}
\label{fig-entropy-mvn-diag-traj-opt-over-prob-7}
\end{minipage}
\begin{minipage}[r]{0.48\columnwidth}
\centering
\adjincludegraphics[trim={{.54\width} 0 0 0},clip, width=\textwidth]{./images/mode-opt/exploration/mvn-full-cov/trajectories_over_mixing_probs_step_1.pdf}
\subcaption{Joint entropy \cref{eq-joint-entropy}}
\label{fig-entropy-mvn-full-cov-traj-opt-over-prob-7}
\end{minipage}
\caption{\textbf{Comparison of joint/factorised entropy objectives}
...
the sum of the marginal entropy at each time step (factorised entropy) and
(\subref{fig-entropy-mvn-full-cov-traj-opt-over-prob-7})
the full joint entropy over a trajectory.
The trajectory found with the factorised entroyp collapses onto a single location of high entropy.
The optimised controls are rolled out in the desired mode’s
GP dynamics (magenta) and in the environment (cyan) and are overlayed on each mode’s mixing probability.}
...
Intuitively, a strategy which favours searching regions of the state space which have not previously been observed
could likely avoid the local optima in cref:fig-explorative-state-diff-traj-opt-over-prob-7.
This intuition motivated adding the gating entropy term in cref:eq-explorative-traj-opt.
...
cref:fig-entropy-mvn-diag-traj-opt-over-prob-7, shows that the entire trajectory collapses onto a single state
of high entropy.
...
These results confirm that the joint gating entropy relieves the myopia of active learning
by considering the information gain over the trajectory.
...
2. It is then overlayed on the gating network posterior *after* training on it,
...
has a positive impact on performance. However, it is worth noting that this was only possible
because the gating network is based on GPs.
As such, this method is not widely applicable in all MoGPE dynamics models.
...
However, the joint distribution over the \acrshort{mosvgpe} mode indicator variable is factorised over time.
As such, the entropy at each time step does not consider the information gain along the rest of the trajectory.
However, the \acrshort{mosvgpe}'s gating function \acrshort{gp} can be conditioned on all other time steps to
achieve this behaviour.
For example, consider using the entropy of the following distribution,
#+BEGIN_EXPORT latex
\begin{align}
...
The entropy of this distribution considers the information gain along the rest of trajectory via the
...
In some cases, they tend to a uniform distribution at the boundary between modes, even though they are very confident,
i.e have low epistemic uncertainty.
As such, this objective may favour trajectories at mode boundaries which it has already observed.
...
How often does the agent fail when leaving the desired dynamics mode in practice?
...
approximately solve the problem in cref:eq-mode-explore-problem.
...
Further testing in different environments and analysis of how the optimisation parameters impacts exploration,
...
* Conclusion
** Future Work
*** Ideas for continuing the work in this dissertation
...
Higher-dimensional state-control spaces, for example, models with image inputs.

*Real-time control?*
The trajectory optimisation algorithm in this chapter is only concerned with
...
This dissertation solely focused on model-based algorithms.

Reactive strategy which would automatically turn back when encountering hard to control dynamics
