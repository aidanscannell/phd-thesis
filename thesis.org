...
\end{align}
#+END_EXPORT
where $\state_t \in \stateDomain$, $\control_t \in \controlDomain$ and $\alpha_t \in \modeDomain$
...
with mini-batches,
and secondly, the expectation over the log-likelihood is calculated using Monte Carlo samples.
...
*** Evaluation on Motorcycle Data Set :noexport:
...
\subcaption{Gating network's predictive posterior $\predictiveGatingPrior$ where each row corresponds to a single GP posterior, $q(h_{k}(\state_*))$, corresponding to expert $k$. The mean $\E[h_{k}(\state_*)]$ is on the left and the variance $\V[h_{k}(\x_*)]$ is on the right.}
...
#+ATTR_LATEX: :width 1\textwidth :placement [hbt!] :center t
#+caption: Moment matched predictive posterior $p(\y_* \mid \x_*)$ after training on the quadcopter data set.
...
The model was then trained on the velocity controlled quadcopter data set
using the initial parameters in Table ref:tab-params-quadcopter.

# Figure ref:fig-experts-y-quadcopter-subset shows the predictive posteriors $q(\Delta\state_{kd} \mid \state_*)$
# associated with each dimension $d$
# of each expert $k$ and Figure ref:fig-gating-mixing-probs-quadcopter-subset shows the gating network's
# predictive mixing probabilities.
Figure ref:fig-experts-f-quadcopter-subset shows the predictive posteriors $q(f_{kd}(\state_*))$
associated with each dimension $d$ of each expert $k$ and
...
That is, select each expert's inducing inputs, $\expertInducingInput$, to support only a subset of the data set.
...
It is also high in the region where the model has had no training observations, as
expected.
...
The posterior variance is high everywhere except for the region in from of the fan.
...
the low turbulence region and low in front of the fan, i.e. the high turbulence mode.
The predictive mixing probabilities
tend to a uniform distribution
${\Pr(\modeVar=1 \mid \state_*)=\Pr(\modeVar=2 \mid \state_*)=0.5}$
...
It is worth noting that the mixing probabilities can tend to a uniform distribution for multiples reasons.
...
Figure ref:fig-gating-gps-quadcopter-subset shows the gating network's predictive posterior does not lose a degree
of freedom. The gating function associated with Expert 1 is high in the low turbulence regions and low in front of
the fan, i.e. the high turbulence mode. The posterior variance is high where the model has not observed
...
Figure ref:fig-y-mm-quadcopter-subset shows it approximated as a unimodal Gaussian density
(via moment matching) for visualisation purposes.
Although an approximation to the true density, the predictive variance has clearly accounted for both
modelled the
The variance is high where there are no training observations, indicating that is has successfully represented
...
the high process noise induced by the fan.
