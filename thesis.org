...
# cref:sec-problem-statement-inference-traj-opt
...
# \todo[inline]{Do I need to recap or shall I just remove this section?}
...
full stochastic nature of the learned dynamics model. In particular, it considers uncertainty propagation
through the dynamics.
Given the \acrshort{soc} problem in cref:eq-mode-soc-problem, the trajectory optimisation algorithm presented here
builds off of the control-as-inference framework citep:toussaintProbabilistic2006,kappenOptimal2013,
which frames optimal control as probabilistic inference.
Translating optimal control to inference can be viewed as a subset of probabilistic numerics
citep:hennigProbabilistic2015, where statistical methods are used to solve numerical problems,
...
*** text :ignore:noexport:
Following cref:sec-traj-opt-results-energy the optimised trajectories are overlayed on the
mixing probabilities and the GP posterior over the desired mode's gating function.
cref:tab-results-sim-envs-inf summarises the trajectory optimisation results.

The control-as-inference method in cref:eq-traj-opt-inference is tested with and without maximum entropy behaviour,
i.e. with Gaussian and deterministic controls.
cref:fig-mode-conditioning-max-entropy-traj-opt-7,fig-mode-conditioning-max-entropy-traj-opt-5
show the results using Gaussian controls,
i.e. with maximum entropy control, in Environment 1 and Environment 2 respectively.
cref:fig-mode-conditioning-no-max-entropy-traj-opt-7,fig-mode-conditioning-no-max-entropy-traj-opt-5
show the results without maximum entropy control.

*Target state*
All of the experiments were able to find trajectories that successfully navigate to the target state $\targetState$
under the dynamics (magenta) and in the environment (cyan).
Similarly to the results for the direct Riemannian energy method in cref:sec-traj-opt-energy,
setting the terminal state cost matrix $\terminalStateCostMatrix$
to be very high, appears to work well in practice.
...
that all of the experiments found discrete-time trajectories remained in the desired dynamics mode.
This is indicated by none of the trajectories time steps being in the turbulent dynamics mode.
However, when interpolating the discrete-time trajectory for the experiment in Environment 2 without
maximum entropy control, shown in
cref:fig-mode-conditioning-no-max-entropy-traj-opt-5,
the trajectory crosses the mode boundary.
This is an undesirable behaviour, as in a non simulated environment this corresponds to the trajectory
entering the turbulent dynamics mode.
Visual inspection of
cref:fig-mode-conditioning-no-max-entropy-traj-opt-5,fig-mode-conditioning-max-entropy-traj-opt-5
show that in Environment 2, the maximum entropy control term resulted in a trajectory with more clearance from the
mode boundary.
This is a desirable behaviour that is expected from the maximum entropy control term.
This can also be seen in the results for Environment 1, shown in
cref:fig-mode-conditioning-no-max-entropy-traj-opt-7,fig-mode-conditioning-max-entropy-traj-opt-7,
where the trajectory found with maximum entropy control has more clearance from the mode boundary.
This observation is confirmed in cref:tab-results-sim-envs-inf, where the experiments with maximum entropy
control obtained higher probabilities of remaining in the desired dynamics mode when considering the
model's epistemic uncertainty.
...
compare to geometry methods

# Although subtle, the trajectory found with maximum entropy control appears to have more clearance from
# the turbulent dynamics mode.
# cref:tab-results-sim-envs-inf provides more insights into this observation.

# The experiment with maximum entropy control obtained a higher probability of remaining in the desired
# dynamics mode when marginalising the models epistemic uncertainty.
# This higher probability indicates that the maximum entropy term

# Although the experiment without maximum entropy control obtained a higher probability of remaining in the desired
# dynamics mode without marginalising the models epistemic uncertainty,
# Both methods

# Before evaluating their performance at avoiding regions of high epistemic uncertainty,
...
# However, in multimodal systems where the underlying dynamics are fairly simple, each mode's GP
# may confidently interpolate/extrapolate into regions that belong to another mode.
# In this case, the epistemic uncertainty is not represented in the dynamics GP but
# in the gating network.
# In the \acrshort{mosvgpe} model, this form of epistemic uncertainty is modelled by the gating function's posterior variance.
*Epistemic uncertainty*
The methods ability to avoid regions of high epistemic uncertainty is now evaluated.
From visual inspection, in Environment 1, the trajectory found with maximum entropy control,
shown in cref:fig-mode-conditioning-max-entropy-over-svgp-traj-opt-7
appears to remain in regions of the model with lower epistemic uncertainty in the gating network.
This is shown by the trajectory navigating to the left of the region of high epistemic uncertainty in
the center of the environment.
This is confirmed in cref:tab-results-sim-envs-inf, as it is the trajectory with
the lowest accumulation of gating function variance.
In Environment 2, the experiment with maximum entropy control also obtained a
lower accumulation of gating function variance than the experiment without maximum entropy control.

Relative to the geometry-based methods,
the trajectories found with the control-as-inference method, in Environment 2, shown in
cref:fig-mode-conditioning-max-entropy-traj-opt-5,fig-mode-conditioning-no-max-entropy-traj-opt-5,
do not exhibit as much uncertainty avoiding behaviour in the gating network.
In contrast, in Environment 1, the trajectories found with the control-as-inference method
obtained some of the most uncertainty avoiding behaviour, indicated by some of the
lowest accumulated gating function variance.
This change in behaviour is due to the relevance of the uncertainty avoiding behaviour being
automatically set by the marginalisation of the gating function
in the mode remaining term from cref:eq-traj-opt-elbo.
...
It is worth noting that only the trajectories found with the geometry-based methods and
$\lambda=20.0$ were able to accumulate less gating function variance.

Without decoupling the uncertainty avoiding behaviour from the mode remaining behaviour,
it is not possible to find trajectories which avoid entering the region of high epistemic uncertainty in the
gating network.
However, as mentioned previously, it is not always clear how to balance these two behaviours.
As shown in cref:chap-traj-opt-geometry, increasing the uncertainty avoiding behaviour can have a detrimental
impact on the mode remaining behaviour.
In contrast, the method presented in this chapter does not provide a mechanism for adjusting the relevance
of each behaviour.
However, the marginalisation of uncertainty in the \acrshort{elbo}
(cref:eq-traj-opt-inference) is principled and requires no human input.
# Issues regarding trajectories not avoiding regions of high epistemic uncertainty in the gating network
# are attributed to the \acrshort{mosvgpe} underestimating uncertainty.
# As such, in regions without observations should
# be overcome by
...
# In contrast, cref:fig-mode-conditioning-no-max-entropy-traj-opt-7 shows results when using deterministic
...
# Relative to the geometry-based methods in cref:chap-traj-opt-geometry,
# the trajectories found with the control-as-inference in
# cref:fig-mode-conditioning-max-entropy-traj-opt,fig-mode-conditioning-no-max-entropy-traj-opt
# do not avoid the region of high uncertainty in the gating network.
