...
The state difference outputs $\stateDiffTraj = \{\stateDiff_{\timeInd}\}_{\timeInd=0}^{\TimeInd-1}$ are first
calculated from the state trajectory $\stateTraj = \{\state_{\timeInd}\}_{\timeInd=0}^{\TimeInd-1}$.
The control trajectory $\controlTraj$ is then inferred from the state trajectory by extending
the ELBO for a single SVGP expert to latent variable inputs.
...
The control inputs $\controlTraj$ are recovered by treating them as latent variables and extending the lower bound,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-control-elbo}
\log p(\stateDiffTraj \mid \stateTraj) &= \log \int p(\stateDiffTraj \mid \stateTraj, \controlTraj) p(\controlTraj) \text{d}\controlTraj \\
&\geq q(\controlTraj) \left( \mathcal{L}_{\text{SVGP}} + \log p(\controlTraj) - \log q(\controlTraj) \right) \text{d}\controlTraj,
\end{align}
#+END_EXPORT
where the latent controls are assumed to be Normally distributed,
#+BEGIN_EXPORT latex
\begin{align}
p(\controlTraj) = \prod_{\timeInd=0}^{\TimeInd-1} \mathcal{N}(\control_{\timeInd} \mid \mathbf{0}, \mathbf{I}),
\end{align}
#+END_EXPORT
and the variational posterior over the latent control trajectory is given by,
#+BEGIN_EXPORT latex
\begin{align}
...
This section evaluates the performance of the methods presented in this chapter using two performance indicators.
1. The desired mode's mixing probability,
   ...
   - The direct control method also calculates the probability of being in the desired dynamics mode
    ...
    \end{align}
     #+END_EXPORT
2. The posterior variance associated with the desired mode's gating function \acrshort{gp}
   #+BEGIN_EXPORT latex
    \begin{align}
    \sum_{\timeInd=1}^{\TimeInd}
...
Both control methods are tested in the simulated environment so that they can be compared.
To aid comparison to the real-world experiments, the layout of the environment was kept consistent
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.9\textwidth]{./images/mode-opt-traj-opt/scenario_7/trajectory_optimisation/env/env_with_dataset_start_end_pos.pdf}
...
\end{figure}
#+END_EXPORT
$\NumData$ state transitions were sampled from the simulator with $\Delta \timeInd = 0.25s$.
cref:fig-dataset-scenrio-7 shows the state transition data set and also indicates the separation of the modes.
...
\begin{minipage}[r]{\columnwidth}
\centering
\includegraphics[width=0.9\textwidth]{./images/mode-opt-traj-opt/scenario_7/trajectory_optimisation/riemannian-energy/trajectories_over_mixing_probs.pdf}
\subcaption{}
\label{fig-riemannian-energy-traj-opt-over-prob-7}
\end{minipage}
\begin{minipage}[r]{0.99\columnwidth}
\centering
\includegraphics[width=0.9\textwidth]{./images/mode-opt-traj-opt/scenario_7/trajectory_optimisation/riemannian-energy/trajectories_over_desired_gating_gp.pdf}
\subcaption{}
...
\begin{minipage}[r]{\columnwidth}
\centering
\includegraphics[width=0.9\textwidth]{./images/mode-opt-traj-opt/scenario_7/trajectory_optimisation/riemannian-energy-low/trajectories_over_mixing_probs.pdf}
\subcaption{}
\label{fig-riemannian-energy-low-traj-opt-over-prob-7}
\end{minipage}
\begin{minipage}[r]{0.99\columnwidth}
\centering
\includegraphics[width=0.9\textwidth]{./images/mode-opt-traj-opt/scenario_7/trajectory_optimisation/riemannian-energy-low/trajectories_over_desired_gating_gp.pdf}
\subcaption{}
...
**** Riemannian energy 40 :ignore:
#+BEGIN_EXPORT latex
\begin{figure}
\centering
\begin{minipage}[r]{\columnwidth}
\centering
\includegraphics[width=0.9\textwidth]{./images/mode-opt-traj-opt/scenario_7/trajectory_optimisation/riemannian-energy-high/trajectories_over_mixing_probs.pdf}
\subcaption{}
\label{fig-riemannian-energy-low-traj-opt-over-prob-7}
\end{minipage}
\begin{minipage}[r]{0.99\columnwidth}
\centering
\includegraphics[width=0.9\textwidth]{./images/mode-opt-traj-opt/scenario_7/trajectory_optimisation/riemannian-energy-high/trajectories_over_desired_gating_gp.pdf}
\subcaption{}
\label{fig-riemannian-energy-high-traj-opt-over-svgp-7}
\end{minipage}
\caption{\textbf{Riemannian Energy} $\bm\lambda\mathbf{=40.0}$
Trajectory optimisation results after finding trajectories from the start state $\state_0$,
to the target state $\targetState$,
with the Riemannian energy cost function from \cref{eq-approximate-trajectory-riemannian-energy}
with $\lambda=40.0$
...
cref:fig-riemannian-energy-traj-opt-7 shows the gating network posterior after training the model.
...
The chance constraints enforce that the controlled system is $\delta-\text{mode remaining}$,
