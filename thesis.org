...
- [ ] Finish writing up mcycle results
- [ ] Add section on gating networks?
  + [ ] show how GMM isn't useful for specifying informative priors
  + [ ] Results section called "Improving Identifiability through GP Gating Networks"
- [ ] Related work session at end?
  + [ ] intro from neurips paper
  + [ ] deep structured mixtures, etc
- [ ] Write discussion
  - [ ] compare to time series methods
...
dynamics modes, and how the dynamics switches between them, are /not fully known a priori/.
...
This chapter further assumes access to a data set $\dataset$ of state transitions, that
...
Probabilistic models have been used to account for epistemic uncertainty and also
provide a principled approach to modelling stochasticity i.e. aleatoric uncertainty
citep:deisenrothPILCO2011,schneiderExploiting1996,chuaDeep2018,hafnerLearning2019.
For example, cite:cutlerEfficient2015,deisenrothPILCO2011,panProbabilistic2014 use Gaussian processes (GPs) to learn
...
cite:mckinnonLearning2017 use a Mixture of Gaussian Process Experts (MoGPE) method,
cite:moerlandLearning2017 use deep generative models and
cite:kaiserBayesian2020 use a Bayesian model that learns independent
dynamics modes whilst maintaining a
probabilistic belief over which mode is responsible for predicting at a given input location.
# This work differs is primarily interested in correctly identifying the underlying dynamics modes
# and inferring latent spaces that are convenient for control.

# This requires correctly identifying the underlying dynamics modes and which is often difficult with
...
state-space models
cite:doerrProbabilistic2018,schonSystem201cite:eleftheriadisIdentification20171


switching state-space models
cite:ghahramaniVariational2000,ghahramaniSwitching1996

# Other models such as d
# cite:kaiserData2019
...
They are able to model non-stationary functions as
each expert learns separate hyperparameters (e.g. lengthscales, noise variances) and the gating
network can turn each expert "on" and "off" in different regions of the input space.
Many MoGPE methods have been proposed and in general they differ via
the formulation of their gating network and their approximate inference algorithms citep:yukselTwenty2012.

As highlighted in cite:rasmussenInfinite2001,
...
(bottom left plot in Figure [[ref:gating_network_comparison]]).
Although this gating function divides up the input space cite:rasmussenInfinite2001 argue that
data not assigned to a GP expert will lead to bias near the boundaries.
...
Each expert's inputs are then Gaussian distributed
(middle top plot of Figure [[ref:gating_network_comparison]]) giving rise to the gating
network in the middle bottom plot of Figure [[ref:gating_network_comparison]].
This gating network is not capable of turning an expert "on" in multiple regions
of the input space. It will instead generate a new cluster and assign it a new expert.
...
They assume that the expert indicators $\alpha$ are generated by a Dirichlet distribution and
that given the expert indicator the inputs follow a Gaussian mixture model (GMM), i.e. they
introduce a cluster indicator variable $z$.
This contrasts earlier approaches that let the expert indicator act as the input cluster
indicator.
This is illustrated in the right hand plots of Figure [[ref:gating_network_comparison]] which
show two Gaussian mixtures over the inputs (one for each expert) and
...
cite:trappDeep2020,nguyenStochastic2018
...
the choice of mean function.
We may also be interested in exploiting techniques from differential geometry
e.g. finding probabilistic geodesics citep:tosiMetrics2014 on the gating network.
Again, our choice of covariance function can be used to encode how differentiable
the gating network should be.
Learning better representations will also improve the models
ability to extrapolate as it will be more true to the underlying system.
...
# *Identifiability* We note that mixture models are inherently unidentifiable as
# different combinations of mixture components
# and mixing coefficients can generate the same predictive distributions.
# The gating network can be considered a handle for encoding prior knowledge that can be used to constrain
# the set of admissible functions.
...
** Problem Statement
This work is interested in learning multimodal, stochastic, nonlinear dynamical systems,
...
\dot{\state}(t) &= \f(\state(t), \control(t)) + \bm\epsilon(t) \\
&= \fk(\state(t), \control(t)) + \mode{\bm\epsilon}(t) \quad \text{if} \quad \alpha(t)=\modeInd, \\
\bm\mode{\epsilon}(t) &\sim \mathcal{N}\left(0, \bm\Sigma_{\mode{\epsilon}} \right),
\end{align}
#+END_EXPORT
where $\dot{\state}$ denotes the state derivative w.r.t. time, i.e. Newton's dot notation.
One of the $\ModeInd$ dynamics modes,
$\{\fk : \stateDomain \times \controlDomain \rightarrow \stateDomain \}_{\modeInd=1}^\ModeInd$, and associated noise models,
$\bm\mode{\epsilon}(t) &\sim \mathcal{N}\left(0, \bm\Sigma_{\bm\mode{\epsilon}} \right)$, where
...
Thus, the $\mode{\epsilon}$ term represents the process noise.
...
time to get the data set $\mathcal{D}=\{(\state_{\timeInd-1},\control_{\timeInd-1}),\Delta \state_t\}^\NumData_{\numData=1}$.
...
\Delta \state_{\timeInd} = \fk (\state_{\timeInd}, &\control_{\timeInd} ; \Delta t = t_*) + \mode{\epsilon}
...
To help with understanding and ease of notation,
only a single output dimension is considered in this chapter,
i.e. $\fk$ is assumed to be $\mode{\latentFunc}$.
...
$\inputDomain = \stateDomain \times \controlDomain \in \R^{\InputDim}$,
giving a single input
$\singleInput = (\state_{\timeInd-1},\control_{\timeInd-1})$.
A single state difference output is denoted as
$\singleOutput = \Delta \state_t = \state_t - \state_{t-1}$.
...
Given this notation, our task is to learn the function $\f$, mapping inputs $\singleInput$ to
...
\todo{cite cite:trappDeep2020 cite:nguyenStochastic2018 and others. Check NeurIPS paper intro?}
...
$\GatingFunc = \{\mode{\gatingFunc} : \stateDomain \times \controlDomain \rightarrow \R \}_{\modeInd=1}^\ModeInd$,
...
citep:hensmanGaussian2013,hensmanScalable2015.
...
# has been dropped.
where $\expertInducingOutput = \mode{\latentFunc}(\expertInducingInput)$ and
$\gatingInducingOutput = \mode{\gatingFunc}(\gatingInducingInput)$
are the inducing points associated with each expert and each gating function respectively.
...
The conditional distributions $\singleExpertGivenInducing$ and $\singleGatingGivenInducing$
...
a variational lower bound on Eq. ref:eq-marginal-likelihood-factorised can be obtained,
#+BEGIN_EXPORT latex
...
Jensen's inequality can be applied to the conditional probability $\singleGatingGivenInducing$,
...
&- \gatingsKL - \expertsKL := \furtherBound,
\end{align}
#+END_EXPORT
This is possible because the mixing probabilities are dependent on *all* of the gating functions and
not just their associated gating function.
This is not the case for the experts' as their Gaussian likelihoods only depend on their corresponding latent
function.
Moving the experts variational posterior $\expertsVariational$ outside of the sum over the indicator variable
corresponds to changing the underlying model.
In particular, it is
\todo{finish making this point about further bounding the experts}
...
marginalise $\gatingsInducingVariational$ and $\expertsInducingVariational$, in closed-form,
with Gaussian convolutions.
This removes the undesirable approximate $M$ dimensional integrals from
Eq. ref:eq-lower-bound-tight, at the cost of loosening the bound.
...
This further bound relies on the inequality,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-log-sum-product-bound}
\log \left(\sum_{i=1}^N a_i \right) \leq \sum_{i=1}^N \log (a_i),
\end{align}
#+END_EXPORT
which is equivalent to,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-sum-product-bound}
\prod_{i=1}^N a_i \geq \sum_{i=1}^N a_i.
\end{align}
#+END_EXPORT
In cref:eq-lower-bound-further, $a_i$ corresponds to conditional probabilities,
so will always be below one, $a_i < 1 \forall i \in N$.
In this case it is trivial to show that the inequality in cref:eq-sum-product-bound holds true.
\todo{Do I need to include a proof or reference for this?}
However, it does necessarily provide a tight bound.

*** Optimisation
\todo{add unbiased estimate of ELBO}
The bounds in  cref:eq-lower-bound-further,eq-lower-bound-tight meet the necessary conditions to perform stochastic
...
At each iteration $i$, a random subset of $\NumData_b$ data points are sampled from the data set $\mathcal{D}$,
to get a minibatch $\mathcal{D}_i = \{d_n\}_{n=1}^{\NumData_b}$, where $d_n$ represents a single
input-output pair.
The ELBO is then approximated by,
# #+BEGIN_EXPORT latex
...
\renewcommand{\GatingSampleInd}{\ensuremath{\hat{S}}}
\renewcommand{\batchSampleInd}{\ensuremath{n}}

\newcommand{\singleExpertLikelihoodSampleTight}{\ensuremath{p\left(\y_{\batchSampleInd} \mid \expertInducingOutput^{(\expertSampleInd)}\right)}}
\newcommand{\singleGatingLikelihoodSample}{\ensuremath{\Pr\left(\alpha_\batchSampleInd = \modeInd \mid \gatingInducingOutput^{(\gatingSampleInd)} \right)}}

\newcommand{\singleExpertLikelihoodSample}{\ensuremath{p\left(\y_{\batchSampleInd} \mid \mode{f}(\x_{\batchSampleInd-1})^{(\expertSampleInd)}\right)}}
\newcommand{\singleGatingLikelihoodSample}{\ensuremath{\Pr\left(\alpha_\batchSampleInd = \modeInd \mid \GatingFunc(\x_{\batchSampleInd-1})^{(\gatingSampleInd)} \right)}}
...
&- \gatingsKL - \expertsKL, \\
\text{where} \quad &\expertInducingOutput^{(\expertSampleInd)} \sim \expertInducingVariational, \\
&\gatingInducingOutput^{(\gatingSampleInd)} \sim \gatingInducingVariational
\end{align}
\normalsize
#+END_EXPORT
#+BEGIN_EXPORT latex
...
\renewcommand{\singleGatingLikelihoodSample}{\ensuremath{\Pr\left(\alpha_\batchSampleInd = \modeInd \mid \GatingFunc(\x_{\batchSampleInd-1})^{(\gatingSampleInd)} \right)}}
\newcommand{\gatingsVariationalSample}{\ensuremath{q(\GatingFunc(\x_{\batchSampleInd-1}))}}
\newcommand{\expertVariationalSample}{\ensuremath{q(\mode{\latentFunc}(\x_{\batchSampleInd-1}))}}
\small
%\sum_{\singleInput, \singleOutput \in \mathcal{D}_i}
...
and lower triangular $\mode{\mathbf{L}}$ of the covariance matrix
$\mode{\mathbf{S}} = \mode{\mathbf{S}} \mode{\mathbf{S}}^T$.
...
$M(\StateDim+\ControlDim)$ optimisation parameters.
...
*Computational Complexity* Assuming that each of the experts have the same number of inducing points $M$,
the cost of computing the KL divergences and their derivatives, is $\mathcal{O}\left( \ModeInd M^{2} \right)$.
The cost of computing the expected likelihood term
$\E_{\gatingsVariational \expertsVariational} \left[ \text{log} \sum_{\modeInd=1}^{\ModeInd} \singleGatingLikelihood \singleExpertLikelihood \right]$,
is dependent on the batch size $\NumData_b$ and has complexity
$\mathcal{O}\left( \NumData_{b} \ModeInd M^{3} \right)$ to evaluate.
When the number of inducing points $M$ is smaller than the batch size, most of the cost will arise from
computing the expected likelihood term.
The bound has complexity
$\max\left(\mathcal{O}\left( \ModeInd M^{2} \right),\mathcal{O}\left( \NumData_{b} \ModeInd M^{3} \right)\right)$.
...
a mixture of $\ModeInd$ Gaussains.
...
The distribution over the output is then obtained by marginalising the expert indicator variable,
#+BEGIN_EXPORT latex
\newcommand{\predictivePosterior}{\ensuremath{p(\output_* \mid \input_*)}}
\newcommand{\predictiveProb}{\ensuremath{\Pr(\modeVar_*=\modeInd \mid \input_*)}}
\newcommand{\predictiveExpert}{\ensuremath{q(\output_* \mid \input_*, \modeVar_*=\modeInd)}}
\newcommand{\predictiveExpertLikelihood}{\ensuremath{p(\output_* \mid \mode{f}(\input_*))}}
\newcommand{\predictiveExpertPrior}{\ensuremath{q(\mode{f}(\input_*))}}
\newcommand{\predictiveExpertsPrior}{\ensuremath{q(\mathbf{f}(\input_*))}}
\newcommand{\predictiveGatingLikelihood}{\ensuremath{\Pr(\modeVar_*=\modeInd \mid \mathbf{\gatingFunc}(\input_*))}}
\newcommand{\predictiveGatingLikelihoodBernoulli}{\ensuremath{\Pr(\modeVar=1 \mid \gatingFunc_1(\input_*))}}
\newcommand{\predictiveGatingPrior}{\ensuremath{q(\mathbf{\gatingFunc}(\input_*))}}
\newcommand{\predictiveGatingPriorBernoulli}{\ensuremath{q(\gatingFunc_1(\input_*))}}
...
is obtained by taking the expectation of it's Gaussian likelihood $\predictiveExpertLikelihood$
under its predictive posterior,
#+BEGIN_EXPORT latex
...
#+END_EXPORT
where the predictive posterior $\predictiveExpertPrior$ is given by Eq. ref:eq--variational-posteriors-functional.
# and $\predictiveExpertLikelihood$ is the Gaussian likelihood associated with the expert (Eq. ref:eq-expert-likelihood).

*Gating Network* The mixing probabilities associated with the gating network are obtained
by taking the expectation of the gating likelihood, $\predictiveGatingLikelihood$ (Bernoulli/Softmax),
...
\predictiveGatingLikelihood = \text{softmax}(\gatingFunc(\input_*)),
\end{align}
#+END_EXPORT
...
\predictiveGatingLikelihoodBernoulli = \Phi(\gatingFunc_1(\input_*)),
\end{align}
#+END_EXPORT
...
With this in mind,
the model and approximate inference scheme are evaluated on three data sets.
...
Although these data sets do not represent state transitions from a dynamical system,
they do contain non-stationary points, heterogeneous noise and complex switching behaviour,
which make them interesting data sets to study from the MoGPE perspective.
Finally, they are tested on a data set collected onboard a DJI Tello quadcopter flying in the Bristol Robotics
Laboratory.
...
- [-] Finish writing up mcycle results
  - [-] compare bounds
    - [X] show tighter bound is better
    - [ ] compare complexity of bounds? or speed to run?
    - [ ] compare how they treat gating network
      + max entropy in indicator var dist (further) vs gating function dist (tight)
  - [ ] compare batch size
  - [ ] finish writing two experts
  - [ ] finish writing three experts
  - [X] update mcycle table with correct NLML
    + NLML/num_data on test_dataset not full_dataset
- [ ] example showing why we want GP based gating network?
  + add artificial dataset from nerurips paper
  + or sigmoid kernel?
- [ ] Update quadcopter results
  + [ ] Add quiver
- [ ] Add comment about not optimising inducing points leading to better performance
- [ ] Add discussion
  - [ ] compare to time series methods
    + [ ] Switching State-Space model
...
The two lower bounds ($\mathcal{L}_{1}$ and $\furtherBound$),
...
In order to evaluate and compare the full predictive posteriors the Negative Log Marginal Likelihood (NLML)
...
Given a test data set $\{(\testSingleInput, \testSingleOutput)\}_{\numTest=1}^{\NumTest$,
they are calculated as follows,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-scores}
\text{RMSE} &= \sqrt{\frac{1}{\NumTest} \sum_{\numTest=1}^{\NumTest} (\predictedSingleOutput - \testSingleOutput)^2} \\
\text{MAE} &= \frac{1}{\NumTest} \sum_{\numTest=1}^{\NumTest} | \predictedSingleOutput - \testSingleOutput | \\
\text{NLML} &= \frac{1}{\NumTest} \sum_{\numTest=1}^{\NumTest} - \log (\testSingleOutput \mid \testSingleInput, \mathcal{D}, \expertParams, \gatingParams)
\end{align}
#+END_EXPORT
where $\predictedSingleOutput$ is the models prediction at $\testSingleInput$.
...
$\allOutput \in \stateDomain \subseteq \R^{133 \times 1}$)
...
cref:tab-mcycle-metrics summarises the results and compares them to a standard GP regression model
...
The NLML indicates the probability of the data given the
...
The tight lower bound $\tightBound$ achieved better NLML than the further lower bound $\furtherBound$
for both two and three experts.
...
Interestingly, the standard GP regression model achieved the best RMSE, followed by the SVGP models and then the
MoSVGPE models.

**** Results table :ignore:

#+Name: tab-mcycle-metrics
#+Caption: Results on the Motorcycle data set cite:Silverman1985.
#+Caption: Comparison of the root mean squared error (RMSE) mean absolute error (MAE)
#+Caption: and the negative log marginal likelihood (NLML) on the test data set for different
#+Caption: instantiations of our model (MoSVGPE).
...
#+Caption: All models were instantiated with a Squared Exponential kernel and were
...
| MoSVGPE $(k=2, \furtherBound)$ | $0.4472$          | $0.5271$          | $\mathbf{0.3218}$ |
| MoSVGPE $(k=3, \tightBound)$   | $0.4569$          | $\mathbf{0.2634}$ | $0.3301$          |
| MoSVGPE $(k=3 ,\furtherBound)$ | $0.4575$          | $0.5467$          | $0.3270$          |

**** Results table old :ignore:noexport:
...
#+Caption: The SVGP and MoSVGPE models were trained with Adam citep:kingmaAdam2017 using a learning rate
#+Caption: of $0.01$ and a minibatch size of $\NumData_b=16$.
#+Caption: The MoSVGPE expertiments used $\NumInducing=32$ inducing points for all each expert and gating network GPs.
|--------------------------------+-------------------+-------------------+-------------------|
|                                | RMSE              | NLML              | MAE               |
|--------------------------------+-------------------+-------------------+-------------------|
| GP                             | $\mathbf{0.4554}$ | $1.1154$          | $0.3405$          |
|--------------------------------+-------------------+-------------------+-------------------|
| SVGP $(M=16)$                  | $0.4580$          | $1.0700$          | $0.3407$          |
| SVGP $(M=32)$                  | $0.4584$          | $1.0737$          | $0.3414$          |
|--------------------------------+-------------------+-------------------+-------------------|
| MoSVGPE $(k=2, \tightBound)$   | $0.4598$          | $0.4639$          | $0.3394$          |
| MoSVGPE $(k=2, \furtherBound)$ | $0.4597$          | $0.5760$          | $\mathbf{0.3381}$ |
| MoSVGPE $(k=3, \tightBound)$   | $0.4634$          | $\mathbf{0.2903}$ | $0.3388$          |
| MoSVGPE $(k=3 ,\furtherBound)$ | $0.4688$          | $0.5686$          | $0.3398$          |
...
\includegraphics[width=\textwidth]{./images/model/mcycle/K=2_L1/y_means.pdf}
...
\end{minipage}
\begin{minipage}[r]{0.49\textwidth}
\includegraphics[width=\textwidth]{./images/model/mcycle/K=2_L1/y_samples.pdf}
\subcaption{}
...
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{./images/model/mcycle/K=2_L1/experts_f.pdf}
%\subcaption{Expert's latent function posteriors $\predictiveExpertPrior$ with $\ModeInd=2$.}
\subcaption{}
...
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{./images/model/mcycle/K=2_L1/gating_gps.pdf}
...
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{./images/model/mcycle/K=2_L1/gating_mixing_probs.pdf}
...
Motorcycle data set \citep{Silverman1985}, with $\tightBound$ (left column) and with $\furtherBound$ (right column).
...
their associated mixing probabilities $\predictiveProb$ in
...
**** Two Experts
The two lower bounds are first compared by instantiating the model with two experts $\ModeInd=2$
and comparing their performance.
# All experiments use $\NumInducing=32$ inducing points for all GPs and are trained for $25,000$ iterations
# of Adam citep:kingmaAdam2017, with a learning rate of $0.01$ and a batch size of $\NumData_b=16$.

The results are show in cref:fig-y-mcycle-two-experts,
where the motorcycle data set is represented by the black crosses.
The left column shows results for the tight bound $\tightBound$ and the right column shows
results for the further bound $\furtherBound$.
...
*Predictive distribution* Both MoSVGPE results are capable of modelling the non-stationarity
...
show the GP posteriors over the experts' latent functions $q(\mode{\latentFunc}(\state_*))$.
cref:fig-gating-gps-mcycle-two-experts-tight,fig-gating-gps-mcycle-two-experts-further
show the GP posteriors over the latent gating functions $q(\mode{\gatingFunc}(\state_*))$
...
*Gating network* The lengthscale of the gating network kernel governs how fast the model can shift responsibility from
expert one to expert two.
For the further lower bound $\furtherBound$,
...
*Experts* Optimising with both bounds resulted in expert one learning a
...
The noise variance inferred by expert one is larger for the further lower bound $\furtherBound$
than for the tight lower bound $\tightBound$.
...
\centering
\begin{minipage}[r]{0.49\textwidth}
\includegraphics[width=\textwidth]{./images/model/mcycle/K=3_L1/y_means.pdf}
\subcaption{}
...
\end{minipage}
\begin{minipage}[r]{0.49\textwidth}
\includegraphics[width=\textwidth]{./images/model/mcycle/K=3_L1/y_samples.pdf}
\subcaption{}
...
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{./images/model/mcycle/K=3_L1/experts_f.pdf}
\subcaption{}
...
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{./images/model/mcycle/K=3_L1/gating_gps.pdf}
\subcaption{}
...
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=0.8\textwidth]{./images/model/mcycle/K=3_L1/gating_mixing_probs.pdf}
\subcaption{}
...
This section instantiates the model with three experts $\ModeInd=3$ and follows the same procedure
as the two exerts experiments, i.e. it trains the model with the same initial model and training parameters.

The results are show in cref:fig-y-mcycle-three-experts,
where the top row visualises the predictive mean and
the bottom row the predictive density, for the tight bound (left column) and the further bound (right column).
Again, the red lines show plus or minus two standard deviations of the SVGP's posterior variance for comparison.
...
each model/bound combo.

Interestingly, the two lower bounds explain the data completely differently.
From cref:tab-mcycle-metrics, it is clear that the predictive posterior associated with the tight bound
is the most accurate.
Instantiating the model with three experts $\ModeInd=3$ and training with the tight lower bound $\tightBound$,
leads to the third expert fitting to the data at $x \geq 1.5$ and the gating network assigning responsibility to it
as this region.
In contrast, instantiating the model with three experts $\ModeInd=3$ and training with the further
lower bound $\tightBound$, results in the gating network "turning off" the third expert at $x \geq 1.5$.
...
The learning curve for batch size $N_b=32$ in cref:fig-mcycle-training-curve-133 shows an example of this behaviour.
In this case, the learning rate must be adjusted, i.e. made smaller.

*Number of Inducing Points*
Our variational inference scheme models the joint distribution over the data via the inducing variables
($\expertsInducingOutput$ and $\gatingInducingOutput$).
In practice, the number of inducing points should be less than the number
of data points, $\NumInducing \lll \NumData$, offering improved computational performance.
...
impact of  offer a comparison an example of this behaviour.
\todo{M=32 is shown twice. Are they with different learning rates?}
\todo{what else do these curves show? Looks like batch size needs to be high as well as number of inducing inputs in (a).}

**** two vs three expert latent fig OLD :ignore:noexport:
#+BEGIN_EXPORT latex
\begin{figure}[t!]
\centering
\begin{minipage}[r]{0.49\textwidth}
\includegraphics[width=\textwidth]{./images/model/mcycle/two-experts/experts_f.pdf}
%\subcaption{Expert's latent function posteriors $\predictiveExpertPrior$ with $\ModeInd=2$.}
\subcaption{}
\label{fig-gating-gps-mcycle-two-experts}
\end{minipage}
\begin{minipage}[r]{0.49\textwidth}
\includegraphics[width=\textwidth]{./images/model/mcycle/three-experts/experts_f.pdf}
%\subcaption{Expert's latent function posteriors $\predictiveExpertPrior$ with $\ModeInd=3$.}
\subcaption{}
\label{fig-gating-gps-mcycle-three-experts}
\end{minipage}
\begin{minipage}[r]{0.49\textwidth}
\includegraphics[width=\textwidth]{./images/model/mcycle/two-experts/gating_gps.pdf}
%\subcaption{Gating function posteriors $\predictiveGatingPrior$ with $\ModeInd=2$.}
\subcaption{}
\label{fig-gating-gps-mcycle-two-experts}
\end{minipage}
\begin{minipage}[r]{0.49\textwidth}
\includegraphics[width=\textwidth]{./images/model/mcycle/three-experts/gating_gps.pdf}
%\subcaption{Gating function posteriors $\predictiveGatingPrior$ with $\ModeInd=3$.}
\subcaption{}
\label{fig-gating-gps-mcycle-three-experts}
\end{minipage}
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{./images/model/mcycle/two-experts/gating_mixing_probs.pdf}
%\subcaption{Gating network's predictive mixing probabilities $\predictiveProb$ with $\ModeInd=2$.}
\subcaption{}
...
\begin{minipage}[r]{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{./images/model/mcycle/three-experts/gating_mixing_probs.pdf}
%\subcaption{Gating network's predictive mixing probabilities $\predictiveProb$ with $\ModeInd=3$.}
\subcaption{}
...
In order to verify that our method works on real world systems,
...
(see Figure ref:fig-quadcopter-environment for a diagram of the environment).


This data set represents samples from a dynamical system with constant controls, i.e.
...
\includegraphics[width=\textwidth]{./images/model/quadcopter/subset/dataset_quiver.pdf}
\subcaption{Quiver plot showing the data set $\mathcal{D}$ -- state transitions from 9 trajectories each flown 7 times.}
\label{fig-quiver}
\end{minipage}
\caption{Illustration of the environment (a room in the Bristol Robotics Laboratory) and the data set of state transitions that was used to train the dynamics model. A turbulent dynamics mode was induced by a desk top fan at the right hand side of the room and a subset of the enivornment has not been observed.}
\end{figure}
#+END_EXPORT
...
enabled pre-planned trajectories to be flown, with a simple PID controller using
...
**** Results
The model was configured to have two experts, with the goal of each expert learning a separate dynamics mode and the
gating network learning a representation of how the underlying dynamics modes vary over the state space.
The model was then trained on the velocity controlled quadcopter data set using the model and training
parameters shown in Table ref:tab-params-quadcopter.
...
#+ATTR_LATEX: :width 1\textwidth :placement [hbt!] :center t
#+caption: Moment matched predictive posterior $p(\y_* \mid \x_*)$ after training on the quadcopter data set. Each row corresponds to an output dimension $d$ and the left plots shows the moment matched means $\E[\Delta \state_d]$ and the right plots show the moment matched variances $\V[\Delta \state_d]$.
[[file:./images/model/quadcopter/subset/y_moment_matched.pdf]]

#+NAME: fig-experts-f-quadcopter-subset
#+ATTR_LATEX: :width 1\textwidth :placement [hbt!] :center t
#+caption: Visualisation of the experts' predictive posteriors $\predictiveExpertsPrior$ after training on the quadcopter data set. Each row corresponds to a single GP posterior, $q(f_{kd}(\state_*))$, corresponding to dimension $d$ of expert $k$. The mean $\E[f_{kd}(\state_*)]$ is on the left and the variance $\V[f_{kd}(\x_*)]$ is on the right. The noise variances learned by Expert 1 and Expert 2 were $\Sigma_1 = \diag\left([0.259,0.386]]\right)$ and $\Sigma_2 = \diag\left([0.869, 1.243]\right)$.
[[file:./images/model/quadcopter/subset/experts_f.pdf]]
...
\centering
\includegraphics[width=\textwidth]{./images/model/quadcopter/subset/gating_mixing_probs.pdf}
\subcaption{Gating network's predictive mixing probabilties $\predictiveProb$.}
\label{fig-gating-mixing-probs-quadcopter-subset}
\end{minipage}
\begin{minipage}[r]{1.0\textwidth}
\includegraphics[width=\textwidth]{./images/model/quadcopter/subset/gating_gps.pdf}
\subcaption{Gating network's predictive posterior $\predictiveGatingPrior$ where each row corresponds to a single GP posterior, $q(h_{k}(\state_*))$, corresponding to expert $k$. The left hand plots show the means $\E[h_{k}(\state_*)]$ and the right hand plots show the variances $\V[h_{k}(\state_*)]$.}
\label{fig-gating-gps-quadcopter-subset}
\end{minipage}
\caption{Visualisation of the gating network after training on the quadcopter data set. The plots in a) show the predictive mixing probabilities $\predictiveProb$ for Expert 1 (left) and Expert 2 (right). The plots in b) show the predictive GP posteriors $q(h_{k}(\state_*))$ associated with Expert 1 (top) and Expert 2 (bottom).}
...
Given a new input $\input_*$ the predictive density over the output, $p(\output_* \mid \input_*)$,
follows a mixture of $\ModeInd$ Gaussian's.
Visualising a mixture of two Gaussian's with a two-dimensional input space and a two-dimensional output space
...
Figure ref:fig-experts-f-quadcopter-subset shows the predictive posteriors $q(f_{kd}(\state_*))$
associated with each dimension $d$ of each expert $k$.
The method has successfully learned a factorised representation of the underlying dynamics, where
Expert 1 has learned a dynamics mode with low process noise $\Sigma_1 = \diag\left([0.259,0.386]]\right)$
and Expert 2 to has learned a dynamics mode with high process noise $\Sigma_2 = \diag\left([0.869, 1.243]\right)$.
...
the first dimension $(x)$ of Expert 1.
...
expected.
However, the posterior variance associated with the second dimension $(y)$ of Expert 1, is not high in this region.
This is due to the lengthscale of the second output dimension.
\todo{is this because of the lengthscale or is it due to gating network}

The bottom two right plots in Figure ref:fig-experts-f-quadcopter-subset show the posterior variance
associated with the first and second dimensions ($x$ and $y$) of Expert 2.
...
Figure ref:fig-gating-network-quadcopter-subset shows the predictive gating network after training on the
data set.
Figure ref:fig-gating-mixing-probs-quadcopter-subset shows that the probability of Expert 1
$\Pr(\modeVar=1 \mid \state_*)$ is high in
the non-turbulent region and low in front of the fan, i.e. the high-turbulence region.
The predictive mixing probabilities
tend to a uniform distribution
${\Pr(\modeVar_*=k \mid \state_*)=0.5}$
...
Figure ref:fig-gating-gps-quadcopter-subset shows the gating network's GP predictive posterior.
The mean of the gating function associated with Expert 1 $\E[h_{1}(\state_*)]$ is
...
network GPs have successfully inferred the epistemic uncertainty.
...
It is worth noting that in contrast to other MoGPE models, this model does not partition the data set,
i.e. each expert depends on **all** of the training inputs.
...
Therefore, the number of inducing points $\NumInducing$ associated with each expert,
...
Optimising some of the parameters instead marginalising all parameters is knwon as Type-II marginal likelihood.
In Bayesian model selection, it is well known that Type-II marginal likelihood can lead to overfitting
if the number of parameters being optimised in large.
...
This often leads to lower NLML being achieved as well.
This observation highlights the importance of a full Bayesian treatment of the inducing inputs.
Specifying priors and performing efficient posterior inference over the inducing inputs is an interesting
direction for future work -- although a challenging problem.

** Related Work
Switching state-space models (HMM)
** Conclusion
...
\end{align}
#+END_EXPORT
To simply notation the state and control trajectories are denoted
...
- Show optimised state trajectory
  + Show trajectory resulting from applying the controls in the simulator
...
full stochastic nature of the learned dynamics model. In particular, it considers uncertainties propagation
...
This section firsts recaps the control-as-inference framework. It then details how the framework is extended
...
\jointDist =
&\underbrace{\terminalCostDist}_{\text{Terminal Cost}} \\
\prod_{\timeInd=0}^{\TimeInd-1} \big[&
\underbrace{\optimalProb}_{\text{Integral Cost}}
\underbrace{\modeProb}_{\text{Mode Remaining Term}} \\
&\underbrace{\transitionDist}_{\text{Dynamics}}
\underbrace{\controlDist}_{\text{Controller}} \big]. \nonumber
\end{align}
\normalsize
#+END_EXPORT
...
* Trajectory Optimisation in Desired Modes as Probabilistic Inference
