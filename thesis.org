...
# #+LATEX_HEADER: \DeclareMathOperator{\inputs}{\mathbf{X}}
# #+LATEX_HEADER: \DeclareMathOperator{\outputs}{\mathbf{Y}}
#+LATEX_HEADER: \newcommand{\x}{\ensuremath{\mathbf{x}}}
#+LATEX_HEADER: \newcommand{\y}{\ensuremath{\mathbf{y}}}

*** Mixture Vars
#+LATEX_HEADER: \newcommand{\dataSub}{\ensuremath{t}}
#+LATEX_HEADER: \newcommand{\dimSub}{\ensuremath{d}}
#+LATEX_HEADER: \newcommand{\modeSup}{\ensuremath{k}}

# Macros for single/all data notation
#+LATEX_HEADER: \newcommand{\singleData}[1]{\ensuremath{#1_{n}}}
#+LATEX_HEADER: \newcommand{\allData}[1]{\ensuremath{\MakeUppercase{#1}}}
# #+LATEX_HEADER: \newcommand{\singleData}[1]{\ensuremath{#1_{\dataSub}}}
# #+LATEX_HEADER: \newcommand{\allData}[1]{\ensuremath{#1_{1:T}}}

#+LATEX_HEADER: \newcommand{\singleDataDim}[1]{\ensuremath{#1_{\dataSub, \dimSub}}}
#+LATEX_HEADER: \newcommand{\singleDim}[1]{\ensuremath{#1_{\dimSub}}}

# Macros for mode k notation
#+LATEX_HEADER: \newcommand{\mode}[1]{\ensuremath{#1^{(\modeSup)}}}

# Expert gating function notation
...
# #+LATEX_HEADER: \newcommand{\Fk}{\ensuremath{\mode{\allData{\mathbf{\latentFunc}}}}}
#+LATEX_HEADER: \newcommand{\Fk}{\ensuremath{\mode{\mathbf{\latentFunc}}}}
#+LATEX_HEADER: \newcommand{\Fkd}{\ensuremath{\singleDim{\Fk}}}
#+LATEX_HEADER: \newcommand{\F}{\ensuremath{\mathbf{\latentFunc}}}
...
\singleOutput &= \latentFunc(\singleInput) \\
\allOutput &= \latentFunc(\allInput)  \\
\allOutput &= \fk(\allInput) \\
\fkn &= \fk(\singleInput) = \{ \fkd(\singleInput) \}_{d=1}^{D} \\
\Fk &= \fk(\allInput) \\
\Fkd &= \fkd(\allInput) \\
\F &= \f(\allInput)
\end{align}
...
p(\Fkd \mid \allInput) &= \mathcal{N}\left( \Fkd \mid \singleDim{\mode{\mu}}(\allInput), \singleDim{\mode{k}}(\allInput, \allInput) \right)
\Kkxx
\end{align}
#+END_EXPORT
#+BEGIN_EXPORT latex
\begin{align} \label{eq-expert-prior}
\pFk &= \prod_{d=1}^{D} \pFkd
\end{align}
#+END_EXPORT
...
#+BEGIN_EXPORT latex
\begin{align} \label{eq-unimodal-dynamics}
...
In the time-independent case the policy is denoted $\pi(\mathbf{x}(t))$
...
\text{s.t. }&\text{Eq. \ref{eq-unimodal-dynamics}} \\
...
This work considers continuous-time, continuous-state, nonlinear stochastic dynamics, given by,
#+BEGIN_EXPORT latex
\begin{align*} \label{eq-dynamics-cont}
\dot{\mathbf{x}}(t) &= f(\mathbf{x}(t), \mathbf{u}(t)) + \epsilon(t) \numberthis \\
&= f^{(k)}(\mathbf{x}(t), \mathbf{u}(t)) + \epsilon^{(k)}(t)
\quad \text{if} \quad \alpha(t)=k
%\quad \text{if} \quad \alpha(\mathbf{x}(t), \mathbf{u}(t))=k
\end{align*}
#+END_EXPORT
with states $\mathbf{x} \in \R^D$ and controls $\mathbf{u} \in \R^F$.
One of $K$ dynamics modes $\{f^{(k)}\}_{k=1}^K$ and associated noise models
$\epsilon^{(k)}(t) \sim \mathcal{N}(0, (\sigma^{(k)})^2)$
are selected by a switching (or gating) variable
$\alpha(t) \in \{1,\ldots,K\}$.
...
This work learns a discrete-time representation of Eq. ref:eq-dynamics-cont,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-dynamics-k}
\Delta\mathbf{x}_{t}
= f^{(k)}(\mathbf{x}_{t-1}, &\mathbf{u}_{t-1} ; \Delta t = t_*) + \epsilon_{t-1}^{(k)}
\quad \text{if} \quad \alpha_t=k,
\end{align}
#+END_EXPORT
where $\mathbf{x}_t \in \R^D$ and $\mathbf{u}_t \in \R^F$ are the states and controls
at time $t$ respectively, and $\alpha_t \in \{1, \dotsc, K\}$ is a mode indicator variable that
indicates one of $K$ dynamics modes at time $t$.
...
$\hat{\mathbf{x}}_{t-1} = (\mathbf{x}_{t-1}, \mathbf{u}_{t-1})$, all inputs are denoted as
$\hat{\mathbf{x}}_{1:T}$, and the set of all outputs as $\Delta\mathbf{x}_{1:T}$.
The $d^{\text{th}}$ dimension of the $k^{\text{th}}$ mode's latent transition dynamics
function $f^{(k)}$, evaluated at $\hat{\mathbf{x}}_{t-1}$,
is denoted $f_{t,d}^{(k)} = f^{(k)}(\hat{\mathbf{x}}_{t-1})$,
