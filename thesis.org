...
Diagram showing a top down view of a simulated system, representing a quadcopter
...
The goal is to find trajectories from the start state $\state_0$, to the target state
$\targetState$, whilst avoiding 1) the turbulent dynamics mode and 2) regions of the learned dynamics that
cannot be predicted confidently due to lack of observations.}
\end{figure}
#+END_EXPORT
cref:fig-point-mass-motivation illustrates the problem that this chapter seeks to solve.
...
\label{eq-traj-opt-over-gating-mask-svgp-baseline}
\caption{This figure demonstrates the shortcomings of learning a transition dynamics model using a SVGP.
The SVGP has not been able to learn a representation of the dynamics which is true to the underlying system.
This is because it cannot model the mulitmodal behaviour present in the state transitions data set,
i.e. the multimodality of the system.
The optimised trajectory in the dynamics SVGP (magenta) and in the environment (cyan),
are overlayed on the gating mask, indicating where each mode governs the dynamics.
White indicates the turbulent dynamics mode and red indicates the operatable mode.
The trajectory optimised in the learned SVGP dynamics,
successfully controls the dynamics from a start state $\state_0$, to a target state $\targetState$.
However, as the learned SVGP dynamics do not correspond to the true underlying dynamics,
applying the optimised controls in the environment does not result in the trajectory successfully navigating
...
\centering
\includegraphics[width=0.8\textwidth]{./images/traj-opt/mode-remaining/baseline-traj-opt/trajectories_over_desired_gating_mask.pdf}
...
To demonstrate the methods shortcomings, the optimised controls are rolled out in the desired mode's GP dynamics (magenta)
...
It is clear from cref:fig-traj-opt-baseline that dynamics mode one $\modeInd=1$ is subject to higher drift in the
...
however, this is not visualised in cref:fig-traj-opt-baseline.
    ...
    # In particular, trajectories that lead to catastrophic failure or end in far away from the target state $\targetState$.
