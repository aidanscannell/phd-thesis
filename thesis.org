...
#+LATEX_HEADER: \newcommand{\pFku}{\ensuremath{p\left(\Fku \right)}}
#+LATEX_HEADER: \newcommand{\qFk}{\ensuremath{q\left(\Fk \right)}}
#+LATEX_HEADER: \newcommand{\qfk}{\ensuremath{q\left(\fk \right)}}
#+LATEX_HEADER: \newcommand{\qfkn}{\ensuremath{q\left(\fkn \right)}}
#+LATEX_HEADER: \newcommand{\pFkuGivenX}{\ensuremath{p\left(\Fku \mid \allInput \right)}}
...
represented as a variational distribution, as seen in cite:hensmanGaussian2013 for regression and in
cite:hensmanScalable2015 for classification.
We follow cite:hensmanGaussian2013 for each expert's GP and cite:hensmanScalable2015 for the gating network
GPs.

Let us first apply Jensen's inequality to the conditional probability $\pYkGivenFku$, i.e.
let us bound each expert's conditional given its inducing points,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-expert-bound-1}
\text{log} \pYkGivenFku&\geq \E_{\pFkGivenUk}\left[ \text{log} \pYkGivenFk  \right].
\end{align}
#+END_EXPORT
This is the standard bound from cite:hensmanGaussian2013 ($\mathcal{L}_{1}$ defined in Eq. 1).
Instead of collapsing the inducing points here (as seen in cite:titsiasVariational2009)
they can be explicitly represented as a variational distribution,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-expert-inducing-dist}
\qFku = \mathcal{N}\left(\Fku \mid \mode{\mathbf{m}}, \mode{\mathbf{S}} \right),
\end{align}
#+END_EXPORT
and used to lower bound $\pYkGivenX$,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-expert-bound-2}
\text{log} \pYkGivenX &\geq \text{log} \E_{\qFku} \left[ \pYkGivenFku \right] - \text{KL}\left[\qFku \mid\mid \pFku\right],
\end{align}
#+END_EXPORT
which is equivalent to $\mathcal{L}_{3}$ defined in Eq. 4 cite:hensmanGaussian2013.
Substituting Eq. ref:eq-expert-bound-1 into Eq. ref:eq-expert-bound-2 results in a further bound,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-expert-bound-3}
\text{log} \pYkGivenX
&\geq \E_{\qFku} \left[ \E_{\pFkGivenFku} \left[ \text{log} \pYkGivenFk \right] \right]
- \text{KL}\left[\qFku \mid\mid \pFku\right] \nonumber \\
&= \E_{\qFk} \left[ \text{log} \pYkGivenFk \right]
- \text{KL}\left[\qFku \mid\mid \pFku\right],
\end{align}
#+END_EXPORT
where $\qFk = \int \qFku \pFkGivenFku \text{d} \Fku$.
Since the likelihood factorizes across data $\pYkGivenFk = \prod_{\numData=1}^{\NumData} \pykGivenfk$
only the marginals of $\qFk$ are needed to calculate Eq. ref:eq-expert-bound-3,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-expert-bound-3-fact}
\text{log} \pYkGivenX
&\geq \sum_{\numData=1}^{\NumData}\E_{\qfkn} \left[ \text{log} \pykGivenfk \right]
- \text{KL}\left[\qFku \mid\mid \pFku\right].
\end{align}
#+END_EXPORT

This is the standard variational equation from cite:hensmanGaussian2013 ($\mathcal{L}_{1}$ defined in Eq. 1).

\todo{start by bounding mixture of SVGP experts with any gating network.}


Let us first derive a bound for the model with a general MoE gating network, that is,
#+BEGIN_EXPORT latex
\begin{align*} \label{eq-variational-dist}
\text{log} \pYGivenX = \text{log} \sum_{k=1}^{K} \PrA \pYkGivenFk \pFk
\end{align*}
#+END_EXPORT
#+BEGIN_EXPORT latex
\begin{align*} \label{eq-variational-dist}
\text{log} \pYGivenU \geq \E_{\prod_{k=1}^{K}\pFkGivenUk} \left[ \text{log} \sum_{k=1}^{K} \PrA \pYkGivenFk \right]
\end{align*}
#+END_EXPORT


For notational conciseness the dependency on the inputs is dropped.
The MoE log marignal likelihood is given by,
#+BEGIN_EXPORT latex
\begin{align*} \label{eq-moe-log-marginal-likelihood}
\text{log} \pY &= \text{log} \sum_{k=1}^{K} \PrA \E_{\pFku} \left[ \pYkGivenFku \right]
\end{align*}
#+END_EXPORT
Eq. ref:eq-expert-bound can be used to bound Eq. ref:eq-moe-log-marginal-likelihood
for each expert,
#+BEGIN_EXPORT latex
\begin{align*} \label{eq-variational-dist}
\text{log} \pY &= \text{log} \sum_{k=1}^{K} \PrA \E_{\pFku} \left[ \pYkGivenFku \right] \\
&\geq \text{log} \sum_{k=1}^{K} \PrA \E_{\qFku} \left[ \pYkGivenFku \right] - \sum_{k=1}^{K} \text{KL}\left[\qFku \mid\mid \pFku\right] \\
&\geq \E_{\prod_{k=1}^{K}\qFku} \left[ \text{log} \sum_{k=1}^{K} \PrA \pYkGivenFku \right] - \sum_{k=1}^{K} \text{KL}\left[\qFku \mid\mid \pFku\right] \\
&\geq \E_{\prod_{k=1}^{K}\qFku} \left[ \E_{\pFkGivenFku} \left[ \text{log} \sum_{k=1}^{K} \PrA \pYkGivenFk \right] \right] - \sum_{k=1}^{K} \text{KL}\left[\qFku \mid\mid \pFku\right] \\
&= \E_{\qF} \left[ \text{log} \sum_{k=1}^{K} \PrA \pYkGivenFk \right] - \sum_{k=1}^{K} \text{KL}\left[\qFku \mid\mid \pFku\right] \\
\end{align*}
#+END_EXPORT
where $\qF := \prod_{k=1}^{K} \int \pFkGivenFku \qFku \text{d} \Fku$.
...
Following cite:hensmanGaussian2013 the variational parameters are introduced
#+BEGIN_EXPORT latex
...
#+BEGIN_EXPORT latex
\begin{align} \label{eq-expert-bound}
\text{log} \pYGivenX &\geq \E_{\qFu}\left[ \text{log} \pYkGivenUk \PrA \right] - \sum_{k=1}^{K} \text{KL}\left( \qFku \mid \mid \pFku \right)
\end{align}
#+END_EXPORT
