...
each time step.
Given Normally distributed states, calculating the expected quadratic terminal/control cost terms in
...
# 1) Simulating the MoSVGPE dynamics model to obtain multi-step predictions is not trivial,
...
#+END_EXPORT
# *Approximate Inference for Dynamics Predictions*
Multi-step predictions in the MoSVGPE dynamics model have no closed-form solution  because the state
...
by assuming that the controlled system remains in the desired dynamics mode.
#+BEGIN_EXPORT latex
...
Making the assumption that the controlled system remains in the desired dynamics mode simplifies
...
Let $\dynamicsFunc : \stateDomain \times \controlDomain \rightarrow \stateDomain$ denote a dynamical system
...
\end{definition}
#+END_EXPORT
Conveniently, the conditions to be $\delta$ -mode remaining in cref:def-delta-mode-remaining can
be enforced with chance constraints,
#+BEGIN_EXPORT latex
\begin{align} \label{eq-mode-chance-constraint}
\Pr(\modeVar_{\timeInd} = \desiredMode \mid \state_{0}, \control_{0:\timeInd}, \modeVar_{0:\timeInd}=\desiredMode)
...
As the MoSVGPE model assumes that the mode indicator variable $\modeVar$, depends on the state via the gating function,
...
\underbrace{p(\state_\timeInd \mid \state_{0}, \control_{0:\timeInd-1}, \modeVar_{0:\timeInd-1}=\desiredMode)}_{\text{state dist}}
\text{d} \state_{\timeInd}
...
Given this approach for simulating the MoSVGPE dynamics model, the state at each time step is Normally distributed.
Unlike the quadratic terminal/control cost terms in cref:eq-quadratic-cost-with-energy, the expected Riemannian
energy,
#+BEGIN_EXPORT latex
...
However, this work simplifies the problem and calculates the Jacobian at the mean of each state along a trajectory, i.e.
...
and 2) avoid entering regions with high uncertainty in which mode governs the dynamics.
The expectation over the state then encourages trajectories to remain in regions of the desired dynamics mode with
low uncertainty.
\end{myquote}
#+END_EXPORT
...
\end{align}
#+END_EXPORT
This method then approximately solves the problem in cref:eq-mode-soc-problem-geometry by solving,
#+BEGIN_EXPORT latex
\begin{subequations} \label{eq-mode-soc-problem-geometry-approx}
\begin{align}
\min_{\controlTraj} &\approxExpectedCost \\
\text{s.t.}& \quad \cref{eq-state-unc-prop,eq-mode-chance-constraint}
\end{align}
\end{subequations}
#+END_EXPORT
using Sequential Least Squares Programming (SLSQP) in SciPy.
...
However, in practice, the optimisation is performed unconstrained, i.e. without enforcing the chance constraints at
every iteration.
This is because the chance constraints are:
1) expensive to evaluate,
2) often inhibit the optimisation and prevent it from finding a good trajectory.
