...
\renewcommand{\gatingKernelMN}{\ensuremath{\hat{\mathbf{K}}_{\modeInd \NumInducing \NumData}}}
...
For each set of assignments, there are then $\ModeInd$ GP models that need to be evaluated, each with
...
\begin{figure}[t]
  ...
  softly assigning them in the variational lower bound.}
...
# Each of the $\ModeInd$ gating function GPs and the $\ModeInd$ expert GPs has complexity
# $\mathcal{O}\left( \NumData_{b} \NumInducing^{2} \right)$ to evaluate.
...
\predictivePosterior &= \prod_{\numData=1}^{\NumTest} \sum_{\modeInd=1}^\ModeInd \predictiveProb \predictiveExpert
\end{align}
#+END_EXPORT
