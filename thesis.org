...
*Iteration 4*
cref:fig-explorative-traj-opt-over-prob-7-3 shows the fourth iteration where once again
...
cref:fig-explorative-data-over-prob-7-step-4-epoch-last-expansion shows that after training on the new
...
Further to this, the trajectory optimiser was able to explore around the mode boundary without leaving the desired
dynamics mode.
...
*** Step 11:12 Variance Figs :ignore:
#+BEGIN_EXPORT latex
\renewcommand{\stepNum}{11}
\renewcommand{\stepNumNext}{12}
\renewcommand{\lastEpoch}{last}
...
cref:fig-explorative-traj-opt-over-prob-7-11,fig-explorative-traj-opt-over-variance-7-11
is $\delta-\text{mode remaining}$, the continuous-time trajectory appears to intersect the end of the mode boundary.
...
Although not tested here, this could likely be solved by reducing how far a time step can travel.
This could be achieved by introducing constraints of the controls or by
...
the shape of the
desired mode's mixing probability changes and no longer matches the gating function's posterior variance.
