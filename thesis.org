...
To further aid visualisation, the $1-\delta$ contour is marked on all plots (pruple line).
...
The initial data set consists of $15$ random trajectories from the start state $\state_0$ that were
ended when the left the initial state domain $\initialStateDomain$.
...
This work initially considered summing the entropy at each time step, as is seen in cite:buisson-fenetActively2020.
This approach is compared to using the joint entropy over a trajectory as is used in
in cref:eq-explorative-traj-opt.
cref:fig-entropy-comparison-traj-opt-7 shows the initial trajectories found with the two approaches.
The factorised entropy approach show in
cref:fig-explorative-mvn-diag-traj-opt-over-prob-7, clearly shows the entire trajectory collapses onto a state
of high entropy.
...
Preliminary experiments suggest that balancing these terms
results in exploration to the target state $\targetState$ without getting stuck in a local minima.
# However, when these two terms are combined,
the objective favours maximum entropy trajectories whose centre of mass is closest
to the target state $\targetState$.
