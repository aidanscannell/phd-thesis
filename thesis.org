...
** Intro :ignore:
This chapter is interested in learning representations of multimodal dynamical systems for control.
The work in this chapter assumes access to a data set $\dataset$ of observations that
have previously been sampled from the system at a constant frequency,
(i.e. with a fixed time-step).
...
\normalsize
#+END_EXPORT
For notational conciseness the terminal values have been omitted.
Assuming a uniform prior for $\policy(\control_\timeInd \mid \state_\timeInd)$ leads to,
#+BEGIN_EXPORT latex
\small
