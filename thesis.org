...
Both methods are evaluated and compared to an alternative approach in cref:chap-traj-opt-results.
...
given a learned representation of the dynamics using \acrshort{mosvgpe}.
...
expert indicator variable can
be used to encode mode remaining behaviour for trajectory optimisation.
