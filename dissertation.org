...
This dissertation considers the \acrfull{mbrl} setting where an explicit model of the environment
...
The primary goal of this dissertation is to control /stochastic/, /multimodal/, /nonlinear/ dynamical systems
to a target state, whilst remaining in the desired dynamics mode.
...
at any given time.
This dissertation considers /stochastic/, /multimodal/, /nonlinear/ dynamical systems, given by,
#+begin_export latex
\begin{subequations} \label{eq-dynamics-main}
\begin{align}
\state_{\timeInd+1} &= \dynamicsFunc(\state_\timeInd, \control_\timeInd) + \bm\epsilon \\
...
at $x \approx -0.7$ better than the sparse variational Gaussian process (\acrshort{svgp}).
...
is the most accurate as it obtained the highest \acrshort{nlpp} score.
...
transitions belonging to another mode. As a result, the lengthscale of the gating network \acrshort{gps} increases.
