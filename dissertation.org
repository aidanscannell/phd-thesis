...
#+latex_class: mimosis
#+EXPORT_FILE_NAME: ./phd-dissertation.pdf
...
{{{acronym(cai,CaI,Control as Inference)}}}
...
\setcounter{page}{1}
\begin{singlespace}
The main goal of the work in this dissertation is to control \textit{unknown},
multimodal dynamical systems to a target state, whilst avoiding inoperable or undesirable dynamics modes.
Over the last decade, learning-based control has become a popular paradigm for controlling dynamical systems.
...
To this end, we consider the model-based reinforcement learning setting, where an explicit dynamics model -- that includes uncertainties -- is used to plan trajectories to a target state.
...
to a target state with mode remaining guarantees.
Initially, the agent’s dynamics model will be highly uncertain — due to a lack of training observations — so these algorithms cannot guarantee mode remaining navigation with high confidence.
When this is the case, the agent should actively explore its environment, collect data and update its dynamics model.
Based on this intuition, we introduce an explorative trajectory optimisation algorithm that explicitly reasons about
...
Finally, we consolidate the work in this dissertation into a model-based reinforcement learning algorithm, which solves the mode remaining navigation problem, with a high probability of remaining in the desired dynamics mode.
\end{singlespace}
#+END_EXPORT
...
# The main goal of the work in this dissertation is to control \textit{unknown} or \textit{partially unknown},
# multimodal dynamical systems, to a target state, whilst avoiding inoperable or undesirable dynamics modes.
...
# In this dissertation, we first consider learning representations of multimodal dynamical systems.
...
%From robotics, to industrial processing, to finanace, learning-based approaches to control
%help alleviate the dependence on domain exerts for system identification and controller design.
This dissertation is concerned with learning-based control for robotic systems subject to multiple dynamics modes,
...
This dissertation considers the model-based reinforcement learning setting where an explicit model of the environment
...
This dissertation first considers learning representations of multimodal dynamical systems.
...
% Given this learned dynamics model, this dissertation considers the mode remaining navigation problem,
Secondly, this dissertation considers the mode remaining navigation problem
...
% This dissertation develops three mode remaining trajectory optimisation algorithms.
% The first two approaches
% exploit concepts of Riemannian geometry -- extended to probabilistic geometries -- to encode the mode remaining
% behaviour, whilst the third approach formulates the control problem as probabilistic inference
% and encodes the mode remaining behaviour by conditioning on a mode assignment variable.
...
Finally, this dissertation considers the more realistic setting of not having prior access to the environment.
...
Secondly, this dissertation considers driving a dynamical system from an initial state (in the desired dynamics mode),
to a target state, whilst remaining in the desired dynamics mode.
For example, consider controlling a quadcopter in an environment subject to two dynamics modes: 1) a turbulent
...
that could be used to train the dynamics model (with controls).
...
%I have thoroughly enjoyed our disscusions,
%Arthur, I have especially enjoyed the many connections you've made between machine learning concepts and control theory.
%Carl,
%in particular, our many whiteboard sessions.
%and how to find there simple solutions

%I especially the ones where Arthur sees through the
...
This work was supported by the EPSRC Centre for Doctoral Training in Future
Autonomous and Robotic Systems (FARSCOPE) at the Bristol Robotics Laboratory.
...
It represents the knowledge about the phenomenon that we could know, but that we do \textit{not know a priori}.
...
As this dissertation assumes that complete knowledge of the dynamics are /not fully known a priori/,
...
# The methods developed throughout this dissertation are evaluated on an illustrative 2D quadcopter navigation example.
...
# The methods developed throughout this dissertation are evaluated on an illustrative 2D quadcopter example.
...
The primary goal of this dissertation is to control stochastic, nonlinear,
multimodal dynamical systems to a target state, whilst remaining in the desired dynamics mode.
...
it can navigate to a target state, whilst remaining in the desired dynamics mode.}
\end{displayquote}
#+END_EXPORT
...
# This section formally defines the mode remaining navigation problem that this dissertation seeks to solve.
...
This dissertation considers the /finite horizon problem/, given by,
#+BEGIN_EXPORT latex
...
These are closed-loop controllers and  in the reinforcement learning literature are referred to as policies.
Alternatively, controllers can depend on time $\control_t = \policy(t)$, in which case they are known
as open-loop controllers.
...
used to find optimal controllers given an accurate model of the environment as an \acrshort{mdp}.
...
These methods are out of the scope of this dissertation.
...
In contrast, this dissertation considers problems where
the underlying dynamics modes $\{\mode{\dynamicsFunc}\}_{\modeInd=1}^{\ModeInd}$
and how the system switches between them $\modeVar$, are \textit{not fully known a priori}.
...
#+BEGIN_EXPORT latex
\begin{algorithm}[!t]
\caption{Model-based reinforcement learning}\label{alg-mbrl-background}
\begin{algorithmic}[1]
...
As both \acrshort{mfrl} and \acrshort{mbrl} methods learn models,
...
The field of \acrfull{mbrl} seeks to solve many optimal control problems by incrementally learning a dynamics model
...
For example, \acrfull{ilqr} can be used to generate trajectories for non-linear systems by
...
However, in this case, iLQR is susceptible to converging
to terrible (local) optima if the true cost function is highly non-convex.
cite:boedeckerApproximate2014 present a real-time iLQR controller based on sparse GPs.
cite:rohrProbabilistic2021 propose a novel LQR controller synthesis for
...
and with \acrshort{gp}s citep:kamtheDataEfficient2018.
...
# However, these are out of the scope of this dissertation.
...
The most direct method to embed constraints is in \acrfull{mpc} citep:eduardof.Model2007.
...
Distinguishing these two sources of uncertainty is important in reinforcement learning.
...
Further to this, it may be desirable to avoid entering regions of
high /aleatoric uncertainty/ because it may result in poor performance or even catastrophic failure.
...
Gaussian processes (GP) are the state-of-the-art approach for Bayesian nonparametric regression and
are a building block for the methods presented in this dissertation.
\acrshort{gp}s have become a popular choice for learning representations of dynamical systems
...
# This section introduces \acrshort{gp}s and the sparse approximations that this work builds upon.
...
*** Gaussian Processes
**** intro :ignore:
The mathematical machinery underpinning \acrshort{gp}s is now detailed.
...
This structured handling of uncertainty makes \acrshort{gp}s extremely powerful for learning representations
...
\subcaption{GP dynamics model trained on state transitions sampled from \textbf{both} dynamics modes.}
\label{fig-traj-opt-over-gating-mask-svgp-all-baseline}
\end{minipage}
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{./images/traj-opt/mode-remaining/scenario_7_svgp_baseline_desired_mode.pdf}
\subcaption{GP dynamics model trained on state transitions sampled from \textbf{only the desired} (operable) dynamics mode.}
\label{fig-traj-opt-over-gating-mask-svgp-desired-baseline}
\end{minipage}
...
cite:kaiserBayesian2020 use the data association with \acrshort{gp}s model;
a Bayesian model that learns independent dynamics modes whilst maintaining a
probabilistic belief over which mode is responsible for predicting at a given input location.
cite:mckinnonLearning2017 also use an approach based on \acrshort{gp}s except that they use a
...
# These approaches are out of the scope of this dissertation.

# approaches but they are out of the scope of this dissertation.
# There are many \acrshort{mfrl} approaches but they are out of the scope of this dissertation.
...
their models, and /exploitation/, where agents make decisions which are optimal for the data observed so far.
...
\acrfull{mbrl} procedure shown in cref:alg-mbrl-background.
# This section recaps some relevant exploration strategies that fit into the general
...
function.
This approach is used in \acrshort{pilco} citep:deisenrothPILCO2011 and GP-\acrshort{mpc} citep:kamtheDataEfficient2018
...
*Active learning*
Active learning is another class of exploration algorithms which fit into the two-phase exploration approach.
...
# This dissertation hypothesises that it must
# be important to
...
based on GPs to improve identifiability.
...
# Motivated by identifiability this work adopts a GP-based gating network resembling a \acrshort{gp} classification model.
...
this work adopts a GP-based gating network resembling a \acrshort{gp} classification model,
...
explorative trajectory optimisation algorithm that considers information gain over trajectories,
as opposed to myopically maximising the information gain at the next state.
...
$\hat{S}$ samples are drawn from the gating network's variational posterior and $S$ are drawn from the experts'
...
#+Caption[\acrshort{mosvgpe} results on motorcycle data set]: Results on the Motorcycle data set cite:Silverman1985 with different instantiations of our model (\acrshort{mosvgpe}).
...
#+Caption: The GP's hyperparameters were optimised using SciPy's citep:2020SciPy-NMeth L-BFGS-B optimiser.
...
#+Caption: The \acrshort{mosvgpe} experiments used $\NumInducing=32$ inducing points for each expert GP
...
the standard \acrshort{gp} regression model achieved the best RMSE, followed by the SVGP models and then the
...
# superior generalisation and identifiability offered by the GP-based gating network.
...
The SVGP has explained the observations by increasing its single
...
expert two (magenta) learning a shorter lengthscale function to fit the wiggly section from $-0.5$ to $1.2$.
...
This is indicated by the extra expert's probability (cyan) remaining low over the region with training data in
...
\end{minipage}
\caption[Motorcycle training curves]{Training curves showing the negative ELBO (- $\furtherBound$) vs step when training
...
\label{fig-quiver}
\end{minipage}
\caption[Diagram of the real-world quadcotper environment and the state transition data set]{Illustration of (\subref{fig-quadcopter-environment}) the environment (a room in the Bristol Robotics Laboratory) and (\subref{fig-quiver}) the data set of state transitions. A turbulent dynamics mode is induced by a desk top fan at the right-hand side of the room and a subset of the environment has not been observed.}
\end{figure}
#+END_EXPORT
...
Just as sparse \acrshort{gp} methods can be viewed as methods that parameterise the full nonparametric GP,
...
its \acrshort{gp}s are used to develop an information-based exploration strategy in cref:chap-active-learning.
...
Instead, it extends the \acrfull{cai} framework and encodes mode remaining behaviour via conditioning on
...
First note that as the differential operator is linear, the derivative of a \acrshort{gp} is also a GP,
...
\end{subequations}
#+END_EXPORT
with the following objective function that minimises the Riemannian energy,
#+BEGIN_EXPORT latex
...
This section first recaps the control-as-inference framework.
...
In particular, it has shown how the control-as-inference framework citep:toussaintRobot2009,toussaintProbabilistic2006,kappenOptimal2013
...
3. Goal 3.1 is measured using the state variance accumulated from cascading single-step predictions,
   #+BEGIN_EXPORT latex
...
4. Goal 3.2 is measured using the gating function variance accumulated from cascading single-step predictions,
   #+BEGIN_EXPORT latex
...
|-----+-----------------+-----------+------------------------------------+-------------------------------------+--------------------------+------------------------------------------------------+-----------------------------------------------------+------------------------------------|
|     | \acrshort{ig}             | $0.5$     | $16.96$                            | $16.22$                             | $8.69$                   | $346.62$                                             | $238.29$                                            | $\times$ dynamics                  |
|     | \acrshort{ig}             | $1.0$     | $18.51$                            | $16.98$                             | $8.70$                   | $367.16$                                             | $222.07$                                            | $\times$ dynamics                  |
|     | \acrshort{ig}             | $20.0$    | $15.04$                            | $14.92$                             | $8.65$                   | $231.98$                                             | $501.26$                                            | $\times$ dynamics                  |
|   1 | \acrshort{dre}            | $0.5$     | $\mathbf{20.98}$                   | $18.04$                             | $8.71$                   | $388.71$                                             | $171.75$                                            | $\checkmark$ both                  |
|     | \acrshort{dre}            | $1.0$     | $\mathbf{20.98}$                   | $18.10$                             | $8.72$                   | $370.94$                                             | $\mathbf{156.66}$                                   | $\checkmark$ both                  |
|     | \acrshort{dre}            | $20.0$    | $13.47$                            | $13.10$                             | $\mathbf{8.64}$          | $\mathbf{164.88}$                                    | $363.98$                                            | $\times$ dynamics                  |
|     | \acrshort{cai} (gauss)    | N/A       | $20.94$                            | $\mathbf{19.99}$                    | $8.74$                   | $208.62$                                             | $287.52$                                            | $\checkmark$ both                  |
|     | \acrshort{cai} (Dirac)    | N/A       | $20.96$                            | $19.58$                             | $8.73$                   | $258.27$                                             | $237.32$                                            | $\checkmark$ both                  |
|-----+-----------------+-----------+------------------------------------+-------------------------------------+--------------------------+------------------------------------------------------+-----------------------------------------------------+------------------------------------|
|     | \acrshort{ig}             | $0.5$     | $18.98$                            | $19.22$                             | $1.53$                   | $495.11$                                             | $249.85$                                            | $\times$                           |
|     | \acrshort{ig}             | $1.0$     | $18.98$                            | $19.16$                             | $1.39$                   | $522.60$                                             | $285.92$                                            | $\times$                           |
|     | \acrshort{ig} (mid point) | $1.0$     | $17.55$                            | $17.80$                             | $1.57$                   | $553.00$                                             | $213.82$                                            | $\times$                           |
|     | \acrshort{ig}             | $5.0$     | $18.98$                            | $19.17$                             | $1.44$                   | $503.94$                                             | $215.75$                                            | $\times$                           |
|   2 | \acrshort{dre}            | $0.5$     | $20.96$                            | $20.34$                             | $1.42$                   | $367.47$                                             | $182.06$                                            | $\checkmark$ both                  |
|     | \acrshort{dre}            | $1.0$     | $\mathbf{20.98}$                   | $20.68$                             | $\mathbf{1.28}$          | $\mathbf{334.47}$                                    | $144.16$                                            | $\checkmark$ both                  |
|     | \acrshort{dre}            | $5.0$     | $20.81$                            | $20.13$                             | $1.37$                   | $374.13$                                             | $\mathbf{134.33}$                                   | $\checkmark$ both                  |
|     | \acrshort{cai} (gauss)    | N/A       | $\mathbf{20.98}$                   | $\mathbf{20.72}$                    | $1.75$                   | $596.67$                                             | $442.88$                                            | $\checkmark$ both                  |
|     | \acrshort{cai} (Dirac)    | N/A       | $\mathbf{20.98}$                   | $20.61$                             | $1.56$                   | $644.35$                                             | $534.39$                                            | $\checkmark$ both                  |
#+LATEX: }
#+end_table
...
One possible explanation is that the ELBO in cref:eq-control-elbo-svgp was not able
...
in this region.
This high uncertainty may have resulted in the ELBO being low when trajectories pass through this region and
as a result, the gradient-based optimiser may have got stuck in this local minima.
...
Therefore, it is possible that this experiment also got stuck in this local optima.
...
Overall, the \acrshort{dre} method performed significantly better than \acrshort{ig} method in the experiments.
...
Both the \acrfull{dre} and the control-as-inference with maximum entropy control
...
# Setting the bounds too loose results in trajectories not satisfying the geodesic ODE whilst
...
cref:sec-modeopt presents \acrshort{modeopt}, a \acrfull{mbrl} algorithm for
solving the mode remaining navigation problem in cref:problem-statement-main.
Finally, cref:sec-preliminary-results presents some preliminary results in a simulated
quadcopter environment and cref:sec-future-work-explore discusses \acrshort{modeopt} and details some directions for future work.
...
The method is named Mode Optimisation or \acrshort{modeopt} for short.
...
This section presents initial results of \acrshort{modeopt}'s exploration strategy.
...
\acrshort{modeopt} side-steps the exploration-exploitation trade-off which is common in MBRL.
...
This is because the \acrshort{gp}s have been able to  model the joint distribution over the
...
so that they could be used for real-time \acrshort{mpc}, for example, via locally linear dynamics approximations.
Alternatively, an explorative state-feedback controller could be learned.
For example, the explorative trajectory optimisation algorithm in cref:eq-explorative-traj-opt could be used for
guided policy search citep:levineGuided2013,
or, it could be used for policy optimisation by reformulating it as a sum of discounted rewards
